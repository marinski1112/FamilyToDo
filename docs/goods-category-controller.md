...
- At initial reconstruction, content with a missing or disabled catalog entry is displayed in its own kind's unclassified group. This is a read-only display fallback, repeated on reload; it neither re-enables a deleted catalog nor changes the other kind. Item set events first refresh catalog metadata before recovery, avoiding a race with category re-enable.
