...
Catalog maps and DOM identities are scoped by `shopping` or `item`. Goods input tabs select the kind for create/AI/unclassified add and reusable sets; Shopping and Item category rows, including archived-empty rows, remain co-visible under both tabs. Category operations continue to read each row’s own kind. Same-name categories are independent. The server rejects `shared` deletion: repository search found only the removed legacy Goods UI calling it. Cached old clients fail safely instead of deleting both domains.
...
- Category header counts reflect the selected pending/completed tab. Completed view shows zero-count named categories in the normal list and suppresses the empty cluster without changing persisted lifecycle state. Unclassified is hidden when it has no rows in the selected status; its global add button remains available. Pending view retains its existing lifecycle.
...
## Co-visible Goods display follow-up (2026-09-23)

Both Shopping and Item categories are displayed under either Goods input tab. The archived-empty cluster contains both kinds at once; each entry carries `data-goods-kind` for independent rename, delete and add. Input tabs control new category/item, AI and reusable-set entry only; Task/Event remains a separate section. Executed DOM regression in the standard CI manifest asserts both kinds on both tabs, the shared empty cluster, same-name independence and kind-specific mutation routing. Newly added Item rows use the same full-width action line as loaded rows; the info action is anchored at the far right.
