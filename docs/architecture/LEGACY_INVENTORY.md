...
| `public/assets/task-linked-items-ui.js` | DEAD | task edit renderer loads only `/assets/task-edit.js`; current `task-edit.js` owns shopping/item row editing; exact asset-name search found no callers; service worker has no explicit precache entry; only confirmed remaining reference was the standalone `check:browser-js` syntax check | `public/assets/task-edit.js` | removed in bounded W3 cleanup together with orphaned syntax-check entry |
