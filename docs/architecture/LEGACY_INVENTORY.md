...
| `CHANGELOG_CLOUDFLARE_WAVE31.md` | HISTORICAL | documentation-only Wave31 task-only/LIFF migration notes; current root README, route/owner maps, runtime source, migration chain and active contracts are authoritative; exact-filename search had no references as supplemental evidence only; referenced `migrations/0008_wave31_task_only.sql` exists separately and remains part of the migration chain | Git history; retained migration 0008 and current task/LIFF source/contracts | classified; keep changelog until a later bounded removal PR |
...
| `CHANGELOG_CLOUDFLARE_WAVE33.md` | HISTORICAL | documentation-only Wave33 legacy-event-removal/lifecycle/UI notes; current root README, runtime/owner maps, migration chain and active contracts are authoritative; exact-filename search had no references as supplemental evidence only; referenced physical-removal migration exists separately as `migrations/0009_wave33_drop_legacy_events.sql` | Git history; retained migration 0009 and current task/schema/contracts | classified; keep changelog until a later bounded removal PR |
...
| `CHANGELOG_CLOUDFLARE_WAVE107.md` | HISTORICAL | documentation-only Wave107 Family AI signed-write architecture/operations notes; current Worker entrypoint is `src/index.ts`; Workers assets are `./public`; root README does not link it; CI/regression consume executable source/contracts rather than this Markdown; exact-filename search had no references as supplemental evidence; referenced migration `migrations/0035_wave107_family_ai_actions.sql` exists separately and is preserved | Git history; retained migration/current source/contracts | remove changelog only; keep migration 0035 |
