...
assert.ok(oneWay.includes('received: 0')&&oneWay.includes('inbound_more: false'),'normal Calendar projection lifecycle must remain outbound-only');
for(const retired of ['processCalendarInbound','applyInbound','syncCalendarAccount'])assert.ok(!calendar.includes(retired),`retired inbound lifecycle must stay absent: ${retired}`);
