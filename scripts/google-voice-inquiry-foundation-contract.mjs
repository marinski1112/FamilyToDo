...
must(/export function parseMarkedGoogleVoiceInquiryCommand\(value:unknown\):GoogleVoiceInquiryParseResult/.test(source),'typed marked-command adapter export is required');
...
must(source.includes("return inquiry?{marked:true,...inquiry}:{marked:true,type:'NEEDS_REVIEW',reason:'UNSUPPORTED_INQUIRY'}"),'marked inquiry adapter must fail closed to NEEDS_REVIEW');
