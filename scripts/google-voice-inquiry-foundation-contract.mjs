...
const extractStart=source.indexOf('export function extractMarkedGoogleVoiceInquiryBody');
const extractBound=source.indexOf('const raw=boundedInput(value);',extractStart);
const extractNormalize=source.indexOf("const normalized=raw.normalize('NFKC')",extractStart);
must(extractStart>=0&&extractBound>extractStart&&extractNormalize>extractBound,'marked body extraction must apply its input bound before NFKC normalization');
...
must(/export function extractMarkedGoogleVoiceInquiryBody\(value:unknown\):string\|null/.test(source),'bounded marked-command body extractor is required');
...
must(source.includes('const body=extractMarkedGoogleVoiceInquiryBody(value)'),'typed marked adapter must reuse the same bounded marker boundary as fallback classification');
...
console.log('google voice inquiry foundation contract: bounded preprocessing, shared marker boundary, exact matching, polite phrases, privacy boundary ok');
