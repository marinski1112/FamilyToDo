...
assert.ok(['12.122.0-wave103','12.123.0-wave104'].includes(pkg.version));
assert.match(ai,/GEMINI_MODEL_DEFAULT='gemini-3\.5-flash-lite'/);
assert.ok(!ai.includes('gemini-2.0-flash'));
assert.match(ai,/env\.GEMINI_MODEL\?\.trim\(\)\|\|GEMINI_MODEL_DEFAULT/);
for(const category of ['NOT_CONFIGURED','API_KEY_INVALID','MODEL_NOT_FOUND','RATE_LIMITED','UPSTREAM_UNAVAILABLE','UNKNOWN'])assert.ok(ai.includes(category));
