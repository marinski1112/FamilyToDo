...
for(const marker of ["WHERE msg.family_id=?","msg.id>?","msg.sender_id<>? AND msg.reminder_at IS NOT NULL AND msg.reminder_at>? AND msg.reminder_at<=?","ORDER BY msg.id ASC LIMIT ${PAGE_SIZE}","cache-control':'private, no-store"]){if(!syncApi.includes(marker))throw new Error(`message sync API contract lost: ${marker}`);}
