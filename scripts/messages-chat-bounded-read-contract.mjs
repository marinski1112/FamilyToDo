...
for(const marker of ["post('/api/task-rough-input'","primaryType:'shopping'","draft?.requiresConfirmation!==true","message_updated_at:String(row.dataset.updatedAt||'')","message_original_text:text","item.dueDate","item.dueTime","AIでタスクに追加","diag?.mark('OUTSIDE_TAP')","'CYCLE_2_DONE'","diag?.mark('THUMBNAIL_RESTORED')","stampTimers.set(img,setTimeout(()=>{stampTimers.delete(img);img.src=stamp.thumbnailUrl||frames[0].url;diag?.mark('THUMBNAIL_RESTORED');},duration))",'const firstFramePreloads=new Map();',"url=String(frames[0]?.url||'')",'if(frames.length<2||!url||firstFramePreloads.has(url))return;',"preload.decoding='async';preload.src=url;",'bubble.append(img);preloadFirstFrame(s);']) if(!client.includes(marker)) throw new Error(`messages chat AI/diagnostic/preload contract lost: ${marker}`);
...
const photoUi=fs.readFileSync('public/assets/message-photo-upload.js','utf8');if(photoUi.includes('画像の選択を取り消す'))throw new Error('photo draft must not expose a dedicated cancel button');if(!photoUi.includes("document.addEventListener('pointerdown'"))throw new Error('photo draft outside-tap cancellation contract lost');
console.log('messages chat bounded-read/media/LINE-composer/AI-confirmation/diagnostic/stamp contract ok');
