...
if(client.includes('cursor=Math.max(cursor,id)'))throw new Error('scheduled releases must not advance the normal message ID cursor');
for(const marker of ["releasedAfterId=0,serverNow=String(payload.now||'')","u.searchParams.set('released_after_id',String(releasedAfterId))","const nextAfter=Number(d.nextAfter)","releasedAfter=nextReleasedAfter;releasedAfterId=nextReleasedAfterId","now=serverNow"])if(!client.includes(marker))throw new Error('message release cursor contract lost: '+marker);
...
for(const marker of ["WHERE msg.family_id=? AND msg.id>? AND (msg.target_member_id IS NULL OR msg.target_member_id IN (?,msg.sender_id)) AND (msg.reminder_at IS NULL OR msg.reminder_at<=? OR msg.sender_id=?)","WHERE msg.family_id=? AND msg.sender_id<>? AND msg.reminder_at IS NOT NULL","(msg.reminder_at>? OR (msg.reminder_at=? AND msg.id>?)) AND msg.reminder_at<=?","const byId=new Map<number,SyncRow>();","ORDER BY msg.id ASC LIMIT ${PAGE_SIZE}","ORDER BY msg.reminder_at ASC,msg.id ASC LIMIT ${PAGE_SIZE}","released_after_id","releasedAfter:nextReleasedAfter","releasedAfterId:nextReleasedAfterId","cache-control':'private, no-store"]){if(!syncApi.includes(marker))throw new Error(`message sync API contract lost: ${marker}`);}
