...
assert.ok(liff.indexOf('await window.liff.init')<liff.indexOf('const current=runtime()'));assert.ok(liff.includes("new URL(location.href).searchParams.get('next')||String(payload.next"));
...
assert.ok(css.includes('--calendar-no-band-content-top:29px'));assert.ok(css.includes('[data-band-rows="0"]'));assert.ok(css.includes('var(--calendar-day-band-rows) * var(--calendar-band-step)'));assert.ok(app.match(/>移動<\/button>/g)?.length===2);assert.ok(css.includes('min-height:34px'));assert.ok(css.includes('width:min(300px,calc(100vw - 24px))'));
...
for(const asset of ['liff-auth.js?v=12.139.0-wave120','calendar.css?v=12.139.0-wave120','calendar.js?v=12.139.0-wave120','family-log.js?v=12.139.0-wave120','family.css?v=12.139.0-wave120'])assert.ok(app.includes(asset));
