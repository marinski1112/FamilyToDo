...
assert.match(loader,/location\.pathname==='\/app\/settings_family_log\.php'\)load\('\/assets\/family-log-management-ui\.js\?v=wave128-fix18'\);\s*else load\('\/assets\/family-log-compact-ui\.js\?v=compact4-dock1'\)/,'daily Family Log must load the fixed compact enhancer directly instead of routing through the management enhancer');
assert.match(shell,/const FAMILY_LOG_UI_REVISION = 'baby-food-photo2-observer1-form2-dock1'/,'Family Log loader must be cache-busted for the observer fix');
