...
for(const token of ["view==='family'","view==='private'","taskVisibilitySql('t')"])assert.ok(app.includes(token),token);
assert.ok(!app.includes("view==='assigned'"),'retired assignee Calendar filter must stay removed');
