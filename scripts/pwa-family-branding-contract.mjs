...
const staticAssets=(sw.match(/const STATIC_ASSETS=\[([^\n]+)\];/)||[])[1]||'';
assert.ok(!staticAssets.includes('/manifest.webmanifest'),'private manifest must not be precached');
assert.ok(!staticAssets.includes('/app-icon-'),'family-scoped icons must not be precached');
assert.doesNotMatch(sw,/url\.pathname==='\/manifest\.webmanifest'/,'service worker fetch handler must not cache private manifest');
assert.doesNotMatch(sw,/url\.pathname\.startsWith\('\/app-icon-'\)/,'service worker fetch handler must not cache family-scoped icons');
assert.ok(sw.includes("'/manifest.webmanifest','/app-icon-180.png','/app-icon-192.png','/app-icon-512.png'"),'activation must evict any legacy private branding cache entries');
assert.ok(sw.includes("const STATIC_CACHE='familytodo-static-shopping-task-fallback'"),'existing static cache generation contract must remain intact');
