...
const categoryGuard=source.indexOf('category.length>MAX_CATEGORY_UNITS',submitStart);
...
assert.ok(categoryGuard>dueDateGuard&&categoryGuard<bodyAssembly,'category length validation must run before add_batch request assembly');
assert.ok(memoGuard>categoryGuard&&memoGuard<bodyAssembly,'memo length validation must run before add_batch request assembly');
