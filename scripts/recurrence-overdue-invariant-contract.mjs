...
const expiredStart=checklist.indexOf('async function expiredTaskPageFor(ctx:AppContext,date:string,cursor?:OverdueTaskCursor):Promise<Row[]>');
const expiredEnd=checklist.indexOf('async function expiredTasksFor',expiredStart);
...
if(!expired.includes('LIMIT ${OVERDUE_TASK_PAGE_SIZE+1}'))throw new Error('expired physical task query must remain bounded');
...
const viewEnd=checklist.indexOf('const renderExpiredTaskRows',viewStart);
