...
if(taskBoundary.includes("from './daily-task-page'")) throw new Error('retired daily page handlers must not return');
...
if(shoppingBoundary.includes("export { shopping } from './shopping-root';")) throw new Error('retired standalone shopping page export must not return');
for(const marker of ["export { shoppingNew } from './shopping-new-page';","export { shoppingEdit } from './shopping-edit-page';"]) if(!shoppingBoundary.includes(marker)) throw new Error(`shopping retained page handler missing: ${marker}`);
...
for(const retiredRoute of ["'/today.php'","'/tomorrow.php'","'/app/shopping.php'"]){
  if(pages.includes(retiredRoute)) throw new Error(`retired legacy page route must not return: ${retiredRoute}`);
}
if(pages.includes('return await today(')||pages.includes('return await tomorrow(')||pages.includes('return await shopping(request,context)')) throw new Error('retired standalone page handler dispatch must not return');
