...
if(!shell.includes("const CALENDAR_STAMP_UI_REVISION = 'stamp-viewer-488';"))fail('Calendar stamp browser asset must carry an explicit revision when its UI changes without a global APP_VERSION release');
...
console.log('calendar animated stamps read API + month-cell consumer contract: bounded sequential PNG playback, safe storage resolution, idle media preloading, lightweight tap-dismiss viewer, editable placement projection, picker/create/update/delete placement API flow and legacy GIF/WebP fallback ok');
