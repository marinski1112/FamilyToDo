...
if(!processBody.includes('buildFactPayload(env,Number(setting.family_id),requesterMemberId,localDate)'))throw new Error('unique destinations must keep requester-scoped Task/Goods facts');
...
console.log('line-daily-digest-destination-contract: one push per LINE destination; individual PRIVATE facts and shared FAMILY-only facts remain scoped');
