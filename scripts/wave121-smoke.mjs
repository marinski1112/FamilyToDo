...
const read=p=>fs.readFileSync(p,'utf8'),oauth=read('src/oauth-continuation.ts'),index=read('src/index.ts'),target=read('src/liff-target.ts'),liff=read('public/assets/liff-auth.js'),app=read('src/app.ts'),family=read('public/assets/family-log.js')+(fs.existsSync('public/assets/family-log-core.js')?'\n'+read('public/assets/family-log-core.js'):''),config=read('wrangler.jsonc'),types=read('worker-configuration.d.ts'),docs=read('docs/GOOGLE_HOME_VOICE_SETUP.md'),pkg=JSON.parse(read('package.json'));
assert.ok(types.includes('LINE_LOGIN_CHANNEL_ID?:string')&&types.includes('LINE_LOGIN_CHANNEL_SECRET?:string'));
