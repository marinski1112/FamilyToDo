import fs from 'node:fs';import path from 'node:path';import { retainedAppContractSource } from './retained-app-contract-source.mjs';
...
const app=retainedAppContractSource(),index=fs.readFileSync('src/index.ts','utf8'),taskEntryPage=fs.readFileSync('src/task-entry-page.ts','utf8'),css=fs.readFileSync('public/assets/family.css','utf8');
...
if(!taskEntryPage.includes('date-option-row date-range-grid task-date-row')||!app.includes('date-option-row date-range-grid'))throw Error('task new/edit date pair missing');
