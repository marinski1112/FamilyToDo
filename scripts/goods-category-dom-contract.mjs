...
view=await render('item');assert.equal(find(view,'item','改名した空').dataset.categoryState,'FRESH_EMPTY');assert.equal(find(view,'item','改名した空').hidden,false);assert(find(view,'shopping','古い空'),'other kind unchanged');
...
await view.close();db.close();console.log('Goods DOM: actual legacy scripts + common controller + real APIs/SQLite; isolated catalogs, tabs, sets, rename/reload, both create/composers and single-kind delete passed');
