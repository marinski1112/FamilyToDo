...
assert.match(source,/const MAX_FRAMES_PER_ASSET=48/,'sequential PNG frame count must remain bounded');
...
assert.match(source,/safeCalendarStampFrame[\s\S]*?frame_index>=0&&row\.frame_index<MAX_FRAMES_PER_ASSET[\s\S]*?duration_ms>=40&&row\.duration_ms<=2000/,'PNG frame metadata must fail closed outside bounded index/duration limits');
...
assert.doesNotMatch(source,/row\.asset_kind==='ANIMATED'&&row\.mime_type==='image\/png'\)return false/,'supported sequential PNG assets must not be rejected by the placement metadata guard');
...
console.log('calendar stamp read-model metadata guard contract: unsafe placement/frame metadata fails closed without identity/log exposure or consuming the safe result limit');
