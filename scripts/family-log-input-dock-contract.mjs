...
const f=setup(),dock=f.page.querySelector(),[tabs,growth,chores]=dock.children;
...
assert.equal(tabs.children[1].attrs['aria-selected'],'true');
tabs.children[1].listeners.keydown({key:'ArrowLeft',preventDefault(){}});
assert.equal(growth.hidden,false);assert.equal(tabs.children[0].focused,true);
...
assert.equal(setup({growth:false}).page.querySelector().children[0].children.length,1);
...
console.log('Family Log input dock: node/listener identity, one click, category/keyboard state, idempotent setup, empty/management fallback PASS');
