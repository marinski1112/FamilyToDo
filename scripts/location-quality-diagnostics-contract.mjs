...
assert.match(api,/latestSelection:'NEWEST_SENSOR_TIME'/,'diagnostics must report the actual newest-sensor-time latest policy');
assert.match(api,/qualityAware:false/,'diagnostics must make clear that accuracy no longer gates latest selection');
assert.match(api,/poorThresholdMeters:100/,'diagnostics must retain the quality band threshold for observation only');
assert.match(api,/historyKeepsPoorPoints:true/,'diagnostics must make clear that poor fixes remain in history');
assert.doesNotMatch(api,/goodLatestProtectionSeconds/,'diagnostics must not advertise a removed latest protection window');
