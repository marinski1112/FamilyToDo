...
if(!exceptionRoutes.includes("import { webhook } from './line-webhook';")) throw new Error('exception routes must import LINE webhook module');
...
if(!exceptionRoutes.includes("if(url.pathname==='/webhook'||url.pathname==='/app/api/webhook'||url.pathname==='/app/api/webhook.php') return await webhook(request,env);")) throw new Error('LINE webhook route wiring changed');
