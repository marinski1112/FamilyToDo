...
assert.ok(ai.includes('listGeminiModels'));assert.ok(!ai.includes('GEMINI_FREE_CANDIDATES'));assert.ok(ai.includes('autoSwitch:false'));assert.ok(ai.includes('no user or family data'));
assert.ok(ai.includes('nextPageToken'));assert.ok(ai.includes("pageSize','1000'"));
...
assert.ok(index.includes('/api/family-ai/model-probe'));assert.ok(cal.includes('model-compatibility'));
