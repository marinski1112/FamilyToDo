...
node <<'JS'
const fs=require('fs');
const app=fs.readFileSync('src/app.ts','utf8'),js=fs.readFileSync('public/assets/family-log.js','utf8'),css=fs.readFileSync('public/assets/family.css','utf8'),mig=fs.readFileSync('migrations/0030_wave94_family_log_overview.sql','utf8');
const has=(s,x)=>{if(!s.includes(x))throw Error(`missing: ${x}`)};
has(app,'All-view intentionally does not union'); has(app,'familyLogOverviewQuickTypes'); has(app,"action==='sleep_start'"); has(app,"action==='sleep_stop'"); has(app,"action==='sleep_adjust'"); has(app,"subject_kind='CHILD'"); has(app,"log_type='SLEEP'"); has(app,'SLEEP_TIMER_WARNING_MINUTES'); has(app,'SLEEP_TIMER_CONFIRM_MINUTES'); has(app,'SLEEP_TIMER_MAX_ADJUST_MINUTES'); has(app,"type!=='TIMER'"); has(app,'family_log_import_batches'); has(app,"countFor('SLEEP')");
