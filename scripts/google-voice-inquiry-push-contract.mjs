...
must(source.includes("url:'/app/tasks.php'")&&source.includes("url:'/app/tasks.php?offset=1'")&&source.includes("url:'/app/tasks.php#shopping-checklist'"),'inquiry kinds must target canonical in-app destinations');
must(routes.includes("url.searchParams.get('offset')==='1'"),'tomorrow inquiry destination must resolve through the canonical task route');
...
console.log('Google voice inquiry push contract: canonical routes, bounded source preprocessing, bounded Unicode-safe output, privacy boundary ok');
