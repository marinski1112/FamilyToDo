...
for(const marker of [
  ...
  'function dueDateNeedsModel(block:RoughBlock,dueDate:string):boolean{',
  'if(httpUrlOnly.test(line))continue;',
  'if(temporalIntentHint(line)||(index>0&&continuationRelativeDateHint.test(line)))return true;',
  ...
  'if(dueDate&&dueDateNeedsModel(block,dueDate))return true;',
...
const fixtureTemporalIntent=value=>fixtureAbsoluteDateHint.test(value)||fixtureRelativeDateHint.test(value)||fixtureWeekdayHint.test(value)||fixtureTimeHint.test(value);
...
const extractExplicitDueFixture=lines=>{
  ...
  for(let index=0;index<lines.length;index++){
    ...
    if(index>0&&match?.[1]){
      if(!fixtureValidDate(match[1]))return null;
...
const dueNeedsModelFixture=lines=>{
  ...
  for(let index=0;index<lines.length;index++){
    ...
    if(fixtureTemporalIntent(line)||(index>0&&fixtureContinuationRelativeDateHint.test(line)))return true;
  }
  return false;
};
assert.equal(extractExplicitDueFixture(['牛乳','期限: 2026-09-10']),'2026-09-10','exact labeled ISO due date must be extractable');
assert.equal(dueNeedsModelFixture(['牛乳','期限: 2026-09-10']),false,'exact due-only block must stay deterministic');
assert.equal(extractExplicitDueFixture(['薬','締切：2026-09-11']),'2026-09-11','full-width colon must remain extractable');
assert.equal(extractExplicitDueFixture(['牛乳','期限: 2026-02-30']),null,'invalid calendar date must not be extracted');
assert.equal(dueNeedsModelFixture(['牛乳','期限: 2026-09-10 18:00']),true,'due time on the due line must remain model-eligible');
assert.equal(extractExplicitDueFixture(['会議','期限: 2026-09-10','18:00']),'2026-09-10','safe exact due date must survive fallback with a separate time');
...
assert.equal(dueNeedsModelFixture(['会議','期限: 2026-09-10','メモ: 明日確認']),true,'Japanese continuation relative date must remain model-eligible');
...
assert.equal(extractExplicitDueFixture(['牛乳','期限: 2026-09-10','https://example.com/2025/12/31']),'2026-09-10','URL metadata must not discard the safe exact due date');
assert.equal(dueNeedsModelFixture(['牛乳','期限: 2026-09-10','https://example.com/2025/12/31']),false,'URL-only metadata must not create temporal ambiguity');
...
console.log('rough-input AI cost guard contract: deterministic-first, safe ISO due extraction, continuation temporal eligibility, URL metadata exclusion, durable budgets, date-independent 429 circuit, and bounded model calls ok');
