...
const parseExplicitDueFixture=lines=>{
  ...
  for(let index=0;index<lines.length;index++){
    ...
    if(fixtureDueIntent.test(line)||fixtureTemporalIntent(line))return null;
  }
  return found;
};
...
console.log('rough-input AI cost guard contract: deterministic-first, explicit ISO due dates, continuation temporal guards, durable budgets, date-independent 429 circuit, and bounded model calls ok');
