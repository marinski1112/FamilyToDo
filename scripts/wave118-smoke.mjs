...
const app=read('src/app.ts'),css=read('public/assets/calendar.css'),calendar=read('public/assets/calendar.js'),log=read('public/assets/family-log.js')+(fs.existsSync('public/assets/family-log-core.js')?'\n'+read('public/assets/family-log-core.js'):''),familyCss=read('public/assets/family.css'),ci=read('.github/workflows/ci.yml'),regression=read('scripts/regression-suite.mjs');
assert.equal(inventory.version,pkg.version);
assert.ok(ci.includes('node scripts/regression-suite.mjs'));
for(const command of ['check:domain-smoke','check:wave117','check:wave118'])assert.ok(regression.includes(`npm run ${command}`));
...
for(const asset of ['calendar.css','calendar.js','family-log.js','family.css'])assert.ok(app.includes(asset+'?v='+pkg.version));
