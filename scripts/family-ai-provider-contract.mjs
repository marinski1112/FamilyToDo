...
for(const marker of ['quotaViolations','FREE_TIER_QUOTA_ZERO','RATE_LIMIT_RPD','RATE_LIMIT_RPM','RATE_LIMIT_TPM','RATE_LIMIT_TEMPORARY','RATE_LIMIT_UNKNOWN','quotaMetric','quotaId','quotaDimensions','quotaValue','retryDelay'])assert.ok(ai.includes(marker),marker);
...
console.log('family-ai-provider-contract: provider catalog, quota diagnostics, model probing, planner boundary, admin gate, and integration prerequisites ok');
