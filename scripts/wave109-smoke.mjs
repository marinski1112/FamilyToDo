...
assert.equal(pkg.version,'12.128.0-wave109');
assert.match(source,/ICS_APPLY_CHUNK=10/);assert.match(source,/ICS_ROLLBACK_CHUNK=15/);assert.match(source,/ICS_MAX_EXDATES=4/);
...
assert.equal(Math.ceil(634/10),64);assert.equal(Math.ceil(634/15),43);assert.ok(source.includes('query_budget_max:67'));assert.ok(source.includes('query_budget_max:64'));
