...
const dailyDigest=fs.readFileSync('src/line-daily-digest.ts','utf8');
...
assert.match(dailyDigest,/current<target\|\|current>target\+29/,'daily digest must tolerate a full 30-minute scheduler/retry window');
assert.match(dailyDigest,/ORDER BY CASE WHEN date\(COALESCE\(start_at,due_at\)\)=\? THEN 0 ELSE 1 END,COALESCE\(start_at,due_at\),id LIMIT 12/,'today rows must be selected before old overdue tasks can consume the digest row budget');
assert.match(dailyDigest,/INSERT OR IGNORE INTO line_daily_digest_receipts/,'daily digest retry tolerance must retain per-day idempotency receipts');
assert.match(dailyDigest,/String\(receipt\.status\)==='SENT'/,'daily digest must not resend after a successful receipt');
