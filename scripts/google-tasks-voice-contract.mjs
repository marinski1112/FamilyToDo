...
for(const value of ["type:'TASK_CREATE'","command.type==='TASK_CREATE'","command_type='TASK_CREATE'","target_type='task'",'MISSING_TASK_TITLE',"visibility==='PRIVATE'?a.member_id:null"])assert.ok(tasks.includes(value),value);
for(const value of ["type:'TASK_COMPLETE'","command.type==='TASK_COMPLETE'","command_type='TASK_COMPLETE'",'AMBIGUOUS_TASK','TASK_NOT_FOUND_OR_PRIVATE',"INSERT INTO task_completions(task_id,member_id,completed_at)","INSERT INTO task_completion_history(task_id,member_id,action,occurred_at)","target_type='task' AND target_id=? AND family_id=? AND status IN ('pending','retry')", "COALESCE(t.visibility_scope,'FAMILY')='FAMILY'",'t.private_owner_id=?'])assert.ok(tasks.includes(value),value);
assert.ok(!tasks.includes('task_assignees'),'Google Tasks must not read or write retired assignees');
for(const value of ["type:'SHOPPING_COMPLETE'","command.type==='SHOPPING_COMPLETE'","command_type='SHOPPING_COMPLETE'",'MISSING_SHOPPING_COMPLETE_NAME','AMBIGUOUS_SHOPPING','SHOPPING_NOT_FOUND_OR_NOT_VISIBLE',"INSERT INTO shopping_completions(shopping_item_id,member_id,completed_at)","INSERT INTO shopping_completion_history(shopping_item_id,member_id,action,occurred_at)","target_type='shopping'", "goodsVisibilitySql('s')",'am.family_id=s.family_id'])assert.ok(tasks.includes(value),value);
...
for(const value of ['Google Tasks ownership map','single Google Tasks list','date-only','PRIVATE by default','Child Journal voice delegation','Wave115/Wave116 setup notes are historical'])assert.ok(docs.includes(value),value);

console.log('google-tasks-voice-contract: bounded Google Tasks sync, private projection guards, typed task/shopping creation and completion, corrected SHOPPING_ADD idempotency, family-log voice bridge, inquiry-safe member Web Push foundation with UTC transport metadata, schema uniqueness, and canonical voice-import documentation boundaries ok');
