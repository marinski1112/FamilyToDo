...
assert.ok(
  checklist.includes('(t.start_at IS NOT NULL AND date(t.start_at)<=date(?) AND date(COALESCE(t.end_at,t.due_at,t.start_at))>=date(?))'),
...
assert.ok(
  !checklist.includes('(t.start_at IS NOT NULL AND date(t.start_at)<=date(?) AND (t.end_at IS NULL OR date(t.end_at)>=date(?)))'),
...
assert.ok(!/class="btn[^"`]*task-shopping-add[^`]*>＋ このタスクに買い物を追加/.test(checklist),'long expired-task shopping button must remain absent');
for(const marker of [
