...
node -e "const fs=require('fs');const s=fs.readFileSync('src/item-edit-page.ts','utf8');if(!s.includes('const taskId=privateParent?Number(item.task_id)'))process.exit(1)"
