...
assert.equal((await context.module.exports.archiveLocationHistory({DB:adapter})).length,1,'pre-fix archive without STAY is rebuilt once');
assert.equal((await context.module.exports.archiveLocationHistory({DB:adapter})).length,0,'rebuilt archive is not repaired repeatedly');
db.close();
console.log('journal archive overflow: full 2672-point day is archived, one-shot repaired, then left stable');
