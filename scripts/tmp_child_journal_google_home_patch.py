...
milestone_block = """    if(types.includes('SLEEP'))scenes.push({...scene(`ft:sleep:start:${s.id}`,`${prefix}寝た`,[`${possessive}睡眠開始`,`${possessive}寝かしつけ完了`]),category:'睡眠',operation:'SLEEP_START'},{...scene(`ft:sleep:stop:${s.id}`,`${prefix}起きた`,[`${possessive}起床記録`,`${possessive}睡眠終了`]),category:'睡眠',operation:'SLEEP_STOP'});
    if(journalReady){
      const journalMilestones=[
        ['stand','STAND','立った記録',single?'立った':`${s.name}が立った`],
        ['first_step','FIRST_STEP','歩いた記録',single?'歩いた':`${s.name}が歩いた`],
        ['first_tooth','FIRST_TOOTH','最初の歯記録',single?'最初の歯が生えた':`${s.name}に最初の歯が生えた`],
        ['tooth','TOOTH','歯記録',single?'歯を記録':`${s.name}の歯を記録`],
      ] as const;
      for(const [slug,code,label,alias] of journalMilestones)scenes.push({...scene(`ft:journal:${slug}:${s.id}`,`${prefix}${label}`,[alias,`${possessive}成長日記 ${label}`]),category:'成長日記',operation:`CHILD_JOURNAL_${code}`});
    }
"""
