...
assert.deepEqual(rows.map(row=>row.title),['今日のイベント','共有の提出','自分だけ','昨日から未完了']);
const shared=fixture.prepare(task.query).all(0,42,0,0,0,'2026-09-25','2026-09-25',0,'2026-09-25','2026-09-25',0);
assert.deepEqual(shared.map(row=>row.title),['今日のイベント','共有の提出','昨日から未完了']);
for(let id=10;id<70;id++)fixture.prepare("INSERT INTO tasks VALUES(?,42,?,'TASK','pending','2026-09-01',NULL,NULL,'FAMILY',NULL,0)").run(id,`期限超過${id}`);
const crowded=fixture.prepare(task.query).all(...task.args);
assert.ok(crowded.some(row=>row.title==='今日のイベント'),'today event survives 50 overdue tasks');
assert.ok(crowded.some(row=>row.title==='共有の提出'),'today task survives 50 overdue tasks');
assert.ok(!crowded.some(row=>row.title==='他人だけ'),'PRIVATE rows stay recipient-scoped');
fixture.close();
