...
export function layout(title: string, body: string, active = ''): string {
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.88-wave69">':''; return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.88-wave69">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],shopping:Row[],items:Row[]=[],detailTasks:Row[]=tasks):string{
  ...
  for(let weekStart=new Date(start);weekStart<=end;weekStart.setUTCDate(weekStart.getUTCDate()+7)){
    ...
    for(let i=0;i<7;i++){
      ...
      const d=cursor.toISOString().slice(0,10),inMonth=d.startsWith(month),dayItems=(map[d]||[]).filter(t=>Number(t._spanDays||1)<=1).sort((a,b)=>(Number(Number(a.id)<0)-Number(Number(b.id)<0))||(Number(a.sort_order||0)-Number(b.sort_order||0))||(Number(a.id)-Number(b.id))),holiday=jpHolidayName(d),wd=cursor.getUTCDay();
  ...
  const script='<script src="/assets/calendar.js?v=12.88-wave69"></script>';
...
export async function messages(request:Request,ctx:AppContext):Promise<Response>{
  ...
  <script src="/assets/messages.js?v=12.88-wave69"></script>`;
...
function shoppingBatchForm(ctx:AppContext, tasks:Row[], date='', members:Row[]=[], selectedTaskId=0): string {
  ...
  return `<div class="card form-card batch-shopping-card" id="addShopping">
    ...
    <form id="shopBatchForm">
      ...
      <div id="shoppingProducts">
        <div class="product-row batch-product" data-product-row><input type="text" name="product_name[]" maxlength="255" placeholder="商品名" required><input type="text" name="product_quantity[]" value="1" inputmode="text" placeholder="数量" aria-label="数量"><button type="button" class="product-url-toggle" aria-expanded="false">🔗 URL</button><span class="product-row-spacer" aria-hidden="true"></span><div class="product-url-popover" hidden><div class="product-url-popover-head"><strong>商品URL</strong><button type="button" class="product-url-close" aria-label="URL入力を閉じる">×</button></div><input type="url" name="product_url[]" placeholder="https://..." aria-label="商品URL"><p class="small">商品ページのURLがある場合だけ入力してください。</p></div></div>
      </div>
  ...
  <script src="/assets/shopping-new.js?v=12.88-wave69"></script>`;
}
...
export async function taskView(ctx:AppContext, id:number):Promise<Response>{
  ...
  <script>const csrf=${JSON.stringify(ctx.session.csrfToken||'')};document.getElementById('done').onchange=async e=>{const checked=e.target.checked;const payload={type:${isVirtual?'\'recurrence\'':'\'task\''},id:${id},occurrence_id:${occurrenceId},completed:checked,csrf};try{const r=await fetch('/api/toggle',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'更新に失敗しました');document.getElementById('taskStatus').textContent=d.status==='completed'?'完了':'未完了';}catch(err){e.target.checked=!checked;alert(err.message)}};document.querySelectorAll('.task-child-toggle').forEach(el=>el.addEventListener('change',async()=>{const checked=el.checked;el.disabled=true;try{const r=await fetch('/api/toggle',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({type:el.dataset.type,id:Number(el.dataset.id),completed:checked,csrf})});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'更新に失敗しました');el.nextElementSibling?.classList.toggle('done',checked);}catch(e){el.checked=!checked;alert(e.message)}finally{el.disabled=false}}));${canEdit?exceptionOrigin?`const exceptionModal=document.getElementById('exceptionDeleteModal');const closeExceptionDelete=()=>{exceptionModal?.classList.remove('open');exceptionModal?.setAttribute('aria-hidden','true')};document.getElementById('exceptionDeleteOpen')?.addEventListener('click',()=>{exceptionModal?.classList.add('open');exceptionModal?.setAttribute('aria-hidden','false')});document.getElementById('exceptionDeleteClose')?.addEventListener('click',closeExceptionDelete);exceptionModal?.addEventListener('click',e=>{if(e.target===exceptionModal)closeExceptionDelete()});const deleteException=async mode=>{const buttons=exceptionModal?.querySelectorAll('button')||[];buttons.forEach(b=>b.disabled=true);try{const r=await fetch('/api/task?id=${id}&exception_mode='+encodeURIComponent(mode),{method:'DELETE',headers:{'x-csrf':csrf}});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'削除に失敗しました。');location.href='/today.php?date=${encodeURIComponent(dateForChildren)}';}catch(err){alert(err.message||String(err));buttons.forEach(b=>b.disabled=false)}};document.getElementById('exceptionDeleteRestore')?.addEventListener('click',()=>deleteException('restore'));document.getElementById('exceptionDeleteExclude')?.addEventListener('click',()=>deleteException('exclude'));`:`document.getElementById('del').onclick=async()=>{if(!confirm('このタスクを削除しますか？'))return;const r=await fetch('/api/task?id=${id}',{method:'DELETE',headers:{'x-csrf':csrf}});const d=await r.json();if(d.ok)location.href='/today.php?date=${encodeURIComponent(dateForChildren)}';else alert(d.error||'削除に失敗しました。');};`:''}</script>`;
...
export async function taskApiLegacy(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(request.method==='DELETE'){
    ...
    for(const r of shops.results) stm.push(
      ...
      ctx.env.DB.prepare("INSERT INTO deleted_completion_history(family_id,entity_type,entity_id,member_id,action,occurred_at,source_type,source_id,archived_at) SELECT ?, 'shopping', shopping_item_id, member_id, action, completed_at, 'shopping_legacy_completion', shopping_item_id, ? FROM shopping_completions WHERE shopping_item_id=?").bind(m.family_id,nowJst(),Number(r.id)),ctx.env.DB.prepare('DELETE FROM shopping_completions WHERE shopping_item_id=?').bind(Number(r.id)),
...
export async function settingsDiagnostics(ctx:AppContext):Promise<Response>{
  ...
  const cards=labels.map((v,i)=>`<div class="diagnostic-row ${counts[i]?'has-issue':'is-ok'}"><div><strong>${esc(v[0])}</strong><div class="small">${esc(v[1])}</div></div><span class="diagnostic-count">${counts[i]}</span></div>`).join('');
...
export async function recurring(request: Request, ctx: AppContext): Promise<Response> {
  ...
  if (request.method === 'POST') {
    ...
    if (action === 'delete') {
      ...
      if (taskId) {
        ...
        statements.push(ctx.env.DB.prepare("INSERT INTO deleted_completion_history(family_id,entity_type,entity_id,member_id,action,occurred_at,source_type,source_id,archived_at) SELECT ?, 'shopping', c.shopping_item_id, c.member_id, c.action, c.completed_at, 'shopping_legacy_completion', c.shopping_item_id, ? FROM shopping_completions c JOIN shopping_items s ON s.id=c.shopping_item_id AND s.family_id=? WHERE s.task_id=?").bind(m.family_id,nowJst(),m.family_id,taskId),ctx.env.DB.prepare('DELETE FROM shopping_completions WHERE shopping_item_id IN (SELECT id FROM shopping_items WHERE task_id=? AND family_id=?)').bind(taskId,m.family_id));
  ...
  <script src="/assets/recurring.js?v=12.88-wave69"></script>
`;
