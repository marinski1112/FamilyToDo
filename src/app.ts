...
export function layout(title: string, body: string, active = ''): string {
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.81-wave62">':''; return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.81-wave62">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],shopping:Row[],items:Row[]=[]):string{
  ...
  function detailHtml(d){
    ...
    const rows=x.map(t=>{
      ...
      const check='<input type="checkbox" class="calendar-task-toggle" data-id="'+t.id+'" data-occurrence-id="'+(t.recurrence_occurrence_id||0)+'" data-recurrence="'+(t.recurring?'1':'0')+'" '+(t.status==='completed'?'checked':'')+'> ';
  ...
  async function moveCalendarTask(id,dir){
    ...
    const a=normalPositions[pos],b=normalPositions[next],[rows[a],rows[b]]=[rows[b],rows[a]];
    if(modalBody)modalBody.innerHTML=detailHtml(selectedDate);
    const ids=normalPositions.map(i=>Number(rows[i].id)).filter(n=>n>0);
    try{const r=await fetch('/app/api/reorder.php',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({csrf:${JSON.stringify(ctx.session.csrfToken??'')},date:selectedDate,ids})});const d=await r.json().catch(()=>null);if(!r.ok||!d?.ok)throw new Error(d?.error||'並べ替えを保存できませんでした。');}
    catch(e){[rows[a],rows[b]]=[rows[b],rows[a]];if(modalBody)modalBody.innerHTML=detailHtml(selectedDate);alert(e?.message||String(e));}
  }
