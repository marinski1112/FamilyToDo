...
export async function familyLog(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const payload=JSON.stringify({csrf:ctx.session.csrfToken||'',managementMode,selectedDate,selectedSubject:adultAggregate?Number(currentAdultSubject?.id||0):selectedSubject,adultAggregate,showAdultLogs,nowLocal,isAdmin:canManageQuickChores,sleepWarningMinutes:SLEEP_TIMER_WARNING_MINUTES,sleepConfirmMinutes:SLEEP_TIMER_CONFIRM_MINUTES,sleepMaxAdjustMinutes:SLEEP_TIMER_MAX_ADJUST_MINUTES,logs:logMap,subjects:subjectMap,quickChores:quickChores.results.map(x=>({id:Number(x.id),name:String(x.name),icon:String(x.icon||'✨'),active:Number(x.active)===1,weekday_mask:Number(x.weekday_mask??127)}))}).replaceAll('<','\\u003c').replaceAll('>','\\u003e').replaceAll('&','\\u0026');
