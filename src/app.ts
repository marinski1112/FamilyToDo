...
export async function familyLog(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(request.method==='POST'){
    ...
    if(action==='settings_update'){
      ...
      await ctx.env.DB.prepare(`INSERT INTO family_log_settings(family_id,show_adult_logs,created_at,updated_at) VALUES(?,?,?,?)
        ON CONFLICT(family_id) DO UPDATE SET show_adult_logs=excluded.show_adult_logs,updated_at=excluded.updated_at`).bind(m.family_id,showAdultLogs,now,now).run();
      await logActivity(ctx,'UPDATED','family_log_settings',m.family_id,{show_adult_logs:showAdultLogs});
      return json({ok:true,show_adult_logs:Boolean(showAdultLogs)});
    }
  ...
  const payload=JSON.stringify({csrf:ctx.session.csrfToken||'',selectedDate,selectedSubject:adultAggregate?Number(currentAdultSubject?.id||0):selectedSubject,adultAggregate,showAdultLogs,nowLocal,isAdmin:familyLogIsAdmin,logs:logMap,subjects:subjectMap,quickChores:quickChores.results.map(x=>({id:Number(x.id),name:String(x.name),icon:String(x.icon||'✨'),active:Number(x.active)===1}))}).replaceAll('<','\\u003c').replaceAll('>','\\u003e').replaceAll('&','\\u0026');
  ...
  const body=`<div class="page-head family-log-head"><div><div class="eyebrow">Family TODO LINE</div><h1>🐣 家族ログ</h1><div class="meta">${esc(selectedSubjectLabel)} ・ ${esc(selectedSubjectMode)}</div></div><div class="family-log-head-actions">${familyLogIsAdmin?'<a class="btn gray" href="/app/family_log_import.php">📥 データをインポート</a>':''}${selectedSubject?'<button type="button" class="btn gray" id="familyLogSubjectEdit">⚙️ 対象設定</button>':''}${familyLogIsAdmin?'<button type="button" class="btn gray" id="familyLogSettingsOpen">⚙️ 表示設定</button>':''}<button type="button" class="btn gray" id="familyLogSubjectOpen">＋ 対象追加</button></div></div>
  ${subjectChips}
  ...
  ${dashboardHtml}
  ${(adultAggregate||(!selectedSubject&&!adultAggregate))?`<form class="family-log-recorder-filter" method="get"><input type="hidden" name="date" value="${esc(selectedDate)}"><input type="hidden" name="subject" value="${adultAggregate?'adult':''}"><label>記録者 <select name="recorder" onchange="this.form.submit()"><option value="0">すべて</option>${members.results.map(member=>`<option value="${member.id}" ${recorderFilter===Number(member.id)?'selected':''}>${esc(member.name)}</option>`).join('')}</select></label></form>`:''}
  <div class="card family-log-timeline"><div class="section-head"><h2>履歴</h2><span class="meta">1ページ50件</span></div><form method="get" class="family-log-type-filter"><input type="hidden" name="date" value="${selectedDate}"><input type="hidden" name="subject" value="${adultAggregate?'adult':selectedSubject||''}"><input type="hidden" name="recorder" value="${recorderFilter}"><select name="type" onchange="this.form.submit()"><option value="">すべて</option>${quickTypes.map(type=>`<option value="${type}" ${timelineType===type?'selected':''}>${FAMILY_LOG_TYPE_META[type].icon} ${FAMILY_LOG_TYPE_META[type].label}</option>`).join('')}<option value="MEMO" ${timelineType==='MEMO'?'selected':''}>📝 その他</option></select></form>${rowHtml||'<p class="empty">この日の記録はありません。</p>'}<nav class="family-log-pagination">${timelinePage>1?`<a class="btn gray" href="?date=${selectedDate}${subjectQuery}&recorder=${recorderFilter}&type=${timelineType}&page=${timelinePage-1}${rangeQuery}">前へ</a>`:''}${timelineHasMore?`<a class="btn gray" href="?date=${selectedDate}${subjectQuery}&recorder=${recorderFilter}&type=${timelineType}&page=${timelinePage+1}${rangeQuery}">さらに読み込む</a>`:''}</nav></div>
  ...
  <script type="application/json" id="familyLogPayload">${payload}</script><script src="/assets/family-log.js?v=12.111-wave92"></script>`;
