...
export function layout(title: string, body: string, active = ''): string {
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.39-wave14">':''; return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.39-wave14">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
function shoppingBatchForm(ctx:AppContext, tasks:Row[], date='', members:Row[]=[], selectedTaskId=0): string {
  ...
  (()=>{
    ...
    form.addEventListener('submit',async e=>{
      ...
      const body={action:'add_batch',csrf:${JSON.stringify(csrf)},products:names.map((name,i)=>({name,quantity:quantities[i]||'1'})),category:form.category.value.trim(),due_date:form.due_date.value,task_id:Number(form.task_id.value||0),assignees:[...form.querySelectorAll('[name="assignees"]:checked')].map(x=>Number(x.value)),memo:form.memo.value.trim()};
