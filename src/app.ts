...
export async function webPushApi(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(action==='unsubscribe'){
    ...
    if(endpoint)await ctx.env.DB.prepare('DELETE FROM web_push_subscriptions WHERE member_id=? AND family_id=? AND endpoint=?').bind(m.id,m.family_id,endpoint).run();
...
export async function settingsDiagnostics(ctx:AppContext):Promise<Response>{
  ...
  const results=await Promise.all([
    ...
    )`).bind(familyId,familyId,familyId,familyId,familyId).first<Row>()
  ]);
  const labels=[
  ...
  const detailSamples=await Promise.all([
    ...
    ) LIMIT 20`).bind(familyId,familyId,familyId,familyId,familyId).all<Row>()
  ]);
