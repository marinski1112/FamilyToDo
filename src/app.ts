...
export async function familyLog(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const subjectChips=`<div class="family-log-subjects"><a class="${selectedSubject===0&&!adultAggregate?'active':''}" href="/app/family_log.php?date=${selectedDate}">すべて</a>${adultSubjects.length>1?`<a class="${adultAggregate?'active':''}" href="/app/family_log.php?date=${selectedDate}&subject=adult">👤 大人</a>`:adultSubjects.map(s=>`<a class="${selectedSubject===Number(s.id)?'active':''}" href="/app/family_log.php?date=${selectedDate}&subject=${s.id}">${esc(familyLogSubjectIcon(s))} ${esc(s.name)}</a>`).join('')}${subjects.results.filter(s=>familyLogSubjectKind(s.subject_kind)!=='ADULT').map(s=>`<a class="${selectedSubject===Number(s.id)?'active':''}" href="/app/family_log.php?date=${selectedDate}&subject=${s.id}">${esc(familyLogSubjectIcon(s))} ${esc(s.name)}</a>`).join('')}</div>`;
  ...
  <script type="application/json" id="familyLogPayload">${payload}</script><script src="/assets/family-log.js?v=12.106-wave87"></script>`;
