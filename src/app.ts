...
function renderDailyPage(ctx:AppContext,date:string,data:{tasks:Row[];items:Row[];shopping:Row[];expiredTasks:Row[]},tomorrow:boolean,unorganized:Row[]=[],unified=false):string {
  ...
  const rows=data.tasks.map(t=>{
    ...
    const shoppingBlock=`<div class="task-actions">${taskShopping}${childItems}<a class="task-shopping-add" href="/app/shopping_new.php?date=${encodeURIComponent(date)}&task_id=${templateId}" aria-label="このタスクに買い物を追加" title="買い物を追加"><span aria-hidden="true">🛒</span><span class="shopping-plus-badge" aria-hidden="true">＋</span></a></div>`;
    ...
    const mainHtml=isEvent?`<div class="task-main event-main"><span>${titleHtml} <small>(イベント)</small></span></div>`:`<label class="task-main"><input class="check toggle" type="checkbox" data-type="${Number(t.id)<0?'recurrence':'task'}" data-id="${esc(t.id)}" ${Number(t.id)<0?`data-occurrence-id="${esc(t.recurrence_occurrence_id)}"`:''} ${t.status==='completed'?'checked':''}><span class="${t.status==='completed'?'done':''}">${titleHtml}</span></label>`;
...
export async function familyLog(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(request.method==='POST'){
    ...
    if(action==='timer_start'){
      const type=String(b.log_type||'').toUpperCase();if(!['SLEEP','BREASTFEED','EXERCISE','WALK'].includes(type))throw new BadRequest('この種類ではタイマーを開始できません。');
      ...
      const now=nowJst();const r=await ctx.env.DB.prepare("INSERT INTO family_log_timers(family_id,subject_id,log_type,started_at,started_at_ms,status,note,created_by,created_at,updated_at) VALUES(?,?,?,?,?,'running',NULL,?,?,?)")
        .bind(m.family_id,subjectId,type,now,Date.now(),m.id,now,now).run();
      await logActivity(ctx,'STARTED','family_log_timer',Number(r.meta.last_row_id),{log_type:type});return json({ok:true,id:Number(r.meta.last_row_id)});
    }
    if(action==='timer_stop'){
      ...
      const r=await ctx.env.DB.prepare('INSERT INTO family_logs(family_id,subject_id,log_type,occurred_at,detail_code,amount,unit,duration_minutes,value_text,note,linked_task_id,linked_occurrence_id,created_by,created_at,updated_at,deleted_at) VALUES(?,?,?,?,NULL,NULL,NULL,?,NULL,?,NULL,NULL,?,?,?,NULL)')
        .bind(m.family_id,timer.subject_id||null,String(timer.log_type||'SLEEP'),String(timer.started_at||now),duration,timer.note||null,m.id,now,now).run();
