...
export async function familyLog(request:Request,ctx:AppContext):Promise<Response>{
  ...
  <script type="application/json" id="familyLogPayload">${payload}</script><script src="/assets/family-log.js?v=12.114-wave95"></script>`;
  const aiEntry=!managementMode?`<details class="family-ai-query"><summary>✨ 家族データに聞く</summary><form id="familyAiForm"><input name="question" maxlength="500" placeholder="例：今週未完了のタスクは何件？" required><button type="submit">聞く</button></form><div id="familyAiAnswer" class="small" aria-live="polite"></div></details><script src="/assets/family-ai.js?v=12.116-wave97"></script>`:'';
...
export async function settingsDiagnostics(ctx:AppContext):Promise<Response>{
  ...
  const calendarDiagnostics=await ctx.env.DB.prepare(`SELECT
    (SELECT COUNT(*) FROM external_calendar_links l WHERE l.family_id=? AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=l.task_id AND t.family_id=l.family_id)) orphan_link,
    (SELECT COUNT(*) FROM external_calendar_links l JOIN tasks t ON t.id=l.task_id WHERE l.family_id=? AND t.family_id<>l.family_id) cross_family_link,
    (SELECT COUNT(*) FROM external_calendar_links l JOIN tasks t ON t.id=l.task_id AND t.family_id=l.family_id WHERE l.family_id=? AND t.visibility_scope='PRIVATE' AND l.deleted_at IS NULL) private_link,
    (SELECT COUNT(*) FROM calendar_sync_outbox o WHERE o.family_id=? AND o.status='ERROR' AND o.retry_count>=5) stuck_outbox,
    (SELECT COUNT(*) FROM external_calendar_accounts a WHERE a.family_id=? AND a.status='REVOKED') revoked_account,
    (SELECT COUNT(*) FROM external_calendar_accounts a WHERE a.family_id=? AND a.status='ACTIVE' AND (a.calendar_id IS NULL OR a.calendar_id='')) missing_calendar`).bind(familyId,familyId,familyId,familyId,familyId,familyId).first<Row>();
  const calendarCard=`<div class="card"><h2>Family AI / Calendar診断</h2><div class="diagnostic-row"><span>Gemini configured</span><strong>${ctx.env.GEMINI_API_KEY?'true':'false'}</strong></div>${Object.entries(calendarDiagnostics||{}).map(([k,v])=>`<div class="diagnostic-row ${Number(v)?'has-issue':'is-ok'}"><span>${esc(k)}</span><strong>${Number(v)}</strong></div>`).join('')}<p class="small">秘密値・token・質問本文は表示しません。</p></div>`;
  const body=`<div class="page-head"><div><div class="eyebrow">管理</div><h1>🩺 データ診断</h1></div><a class="btn gray" href="/app/settings.php">戻る</a></div><div class="card"><div class="section-head"><h2>ライフサイクル整合性</h2><span class="${total?'diagnostic-summary-warn':'diagnostic-summary-ok'}">${total?`要確認 ${total}件`:'異常なし'}</span></div><p class="small">表示は現在の家族データに対する読み取り専用診断です。Cronのcleanupで安全に自動修復できる項目は通常0件になります。</p>${cards}</div>${importCard}${pushCard}${calendarCard}<div class="card"><h2>診断の扱い</h2><p class="small">削除完了履歴は、本体削除後も履歴を保存する目的で外部キーを持ちません。そのため「元タスクが存在しない」だけでは異常扱いしません。自動判断できないデータはこの画面やCloudflareログで確認してから修復します。</p></div>`;
