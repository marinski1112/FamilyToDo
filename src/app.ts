...
async function makeViewData(ctx: AppContext, date:string) {
  const [tasks,items,recurring,shopping,expiredTasks] = await Promise.all([
    ...
    ctx.env.DB.prepare(`SELECT i.*, (SELECT e.title FROM events e WHERE e.id=i.event_id LIMIT 1) AS event_title,
      ...
      FROM items i LEFT JOIN events ie ON ie.id=i.event_id AND ie.family_id=i.family_id
      WHERE i.family_id=? AND ((i.due_at IS NOT NULL AND date(i.due_at)=date(?)) OR (i.event_id IS NOT NULL AND date(ie.start_at)=date(?))) ORDER BY COALESCE(i.due_at,ie.start_at),i.status,i.id`).bind(ctx.member!.family_id,date,date).all<Row>(),
    recurringForDate(ctx,date),
    ctx.env.DB.prepare(`SELECT s.*, t.title AS task_title, et.title AS event_title,
      ...
      FROM shopping_items s LEFT JOIN tasks t ON t.id=s.task_id AND t.family_id=s.family_id LEFT JOIN events et ON et.id=s.event_id AND et.family_id=s.family_id
      WHERE s.family_id=? AND ((s.due_date IS NOT NULL AND s.due_date=?) OR (s.event_id IS NOT NULL AND date(et.start_at)=date(?)) OR (s.due_date IS NULL AND s.event_id IS NULL)) ORDER BY s.status,(s.due_date IS NULL),s.due_date,et.start_at,s.category,s.name,s.id`).bind(ctx.member!.family_id,date,date).all<Row>(),
...
export async function calendar(request:Request,ctx:AppContext,month:string):Promise<Response>{
  ...
  const [shopping,items]=await Promise.all([
    ctx.env.DB.prepare(`SELECT s.*,t.title task_title,(SELECT GROUP_CONCAT(m.name,'、') FROM shopping_assignees sa JOIN members m ON m.id=sa.member_id AND m.active=1 WHERE sa.shopping_item_id=s.id) assignees,(SELECT e.title FROM events e WHERE e.id=s.event_id LIMIT 1) event_title FROM shopping_items s LEFT JOIN tasks t ON t.id=s.task_id AND t.family_id=s.family_id LEFT JOIN events se ON se.id=s.event_id AND se.family_id=s.family_id WHERE s.family_id=? AND ((s.due_date BETWEEN ? AND ?) OR (s.due_date IS NULL AND se.start_at IS NOT NULL AND date(se.start_at) BETWEEN date(?) AND date(?))) ORDER BY COALESCE(s.due_date,date(se.start_at)),s.category,s.name,s.id`).bind(fid,from,to,from,to).all<Row>(),
    ctx.env.DB.prepare(`SELECT i.*,(SELECT e.title FROM events e WHERE e.id=i.event_id LIMIT 1) event_title,(SELECT GROUP_CONCAT(m.name,'、') FROM item_assignees ia JOIN members m ON m.id=ia.member_id AND m.active=1 WHERE ia.item_id=i.id) assignees FROM items i LEFT JOIN events ie ON ie.id=i.event_id AND ie.family_id=i.family_id WHERE i.family_id=? AND ((i.due_at IS NOT NULL AND date(i.due_at) BETWEEN date(?) AND date(?)) OR (i.due_at IS NULL AND ie.start_at IS NOT NULL AND date(ie.start_at) BETWEEN date(?) AND date(?))) ORDER BY COALESCE(i.due_at,ie.start_at),i.id`).bind(fid,from,to,from,to).all<Row>()
  ]);
...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],events:Row[],shopping:Row[],items:Row[]=[]):string{
  ...
  const cellsEl=document.querySelectorAll('.calendar-cell'),modal=document.getElementById('dayModal'),modalBody=document.getElementById('modalBody'),modalTitle=document.getElementById('modalTitle'),modalAdd=document.getElementById('modalAdd'),modalTaskAdd=document.getElementById('modalTaskAdd'),modalShoppingAdd=document.getElementById('modalShoppingAdd'),modalItemAdd=document.getElementById('modalItemAdd'),modalPrev=document.getElementById('modalPrev'),modalNext=document.getElementById('modalNext');
  let selectedDate='';
  ...
  function render(d){
    ...
    selectedDate=d;modalTitle.textContent=d+' の詳細';modalBody.innerHTML=detailHtml(d);modalAdd.href='/task/new.php?date='+d+'&return=calendar';if(modalTaskAdd)modalTaskAdd.href='/task/new.php?date='+d+'&return=calendar';if(modalShoppingAdd)modalShoppingAdd.href='/app/shopping_new.php?date='+d; if(modalItemAdd)modalItemAdd.href='/item/new.php?date='+d; if(modalPrev)modalPrev.setAttribute('aria-label',shiftDate(d,-1)+' の詳細');if(modalNext)modalNext.setAttribute('aria-label',shiftDate(d,1)+' の詳細');modal.classList.add('open');
  }
  ...
  const body='<div class="page-head calendar-page-head"><div><h1>📅 カレンダー</h1><div class="meta">'+month.slice(0,4)+'年'+Number(month.slice(5))+'月</div></div><div><a class="btn gray" href="/app/calendar.php?month='+prev+'">‹</a> <a class="btn gray" href="/app/calendar.php?month='+next+'">›</a></div></div>'+
    ...
    '<div class="modal-backdrop" id="dayModal"><div class="day-modal"><div class="modal-top"><button id="modalPrev" class="modal-day-nav" type="button">‹</button><h2 id="modalTitle"></h2><button id="modalNext" class="modal-day-nav" type="button">›</button><button id="modalClose" class="btn gray modal-close" type="button">×</button></div><div class="modal-scroll"><div id="modalBody" class="modal-body"></div><div class="modal-quick-add"><a class="btn small" id="modalTaskAdd" href="#">📝 タスク</a><a class="btn gray small" id="modalShoppingAdd" href="#">🛒 買い物</a><a class="btn gray small" id="modalItemAdd" href="#">🎒 持ち物</a><a class="btn gray small" id="modalMessageAdd" href="/app/message_new.php">💬 伝言</a></div></div><a id="modalAdd" class="modal-add-fab" href="#">＋</a></div></div>'+script;
...
function shoppingBatchForm(ctx:AppContext, tasks:Row[], date='', members:Row[]=[], selectedTaskId=0, events:Row[]=[]): string {
  const csrf=ctx.session.csrfToken??'';
  ...
  return `<div class="card form-card batch-shopping-card" id="addShopping">
    ...
    <form id="shopBatchForm">
      ...
      <div class="batch-common-settings">
        ...
        <label>関連予定（全商品共通）</label><select name="event_id"><option value="0">予定なし</option>${events.map(e=>`<option value="${e.id}">${esc(e.title)}${e.start_at?`（${esc(String(e.start_at).slice(0,10))}）`:''}</option>`).join('')}</select>
  ...
  (()=>{
    ...
    form.addEventListener('submit',async e=>{
      ...
      const body={action:'add_batch',csrf:${JSON.stringify(csrf)},products:names.map((name,i)=>({name,quantity:quantities[i]||'1',url:urls[i]||''})),category:form.category.value.trim(),due_date:form.due_date.value,task_id:Number(form.task_id.value||0),assignees:[...form.querySelectorAll('[name="assignees"]:checked')].map(x=>Number(x.value)),memo:form.memo.value.trim(),event_id:Number(form.event_id.value||0)};
...
export async function shopping(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(request.method==='POST'){
    ...
    if(action==='add_batch'){
      ...
      const eventId=Number(b.event_id??0)||null;
      if(eventId){const e=await ctx.env.DB.prepare('SELECT id FROM events WHERE id=? AND family_id=?').bind(eventId,m.family_id).first<Row>();if(!e)throw new BadRequest('関連予定が見つかりません。');}
      if(taskId){
  ...
  const [tasks,members,events]=await Promise.all([ctx.env.DB.prepare("SELECT id,title,start_at,due_at FROM tasks WHERE family_id=? AND status<>'completed' ORDER BY coalesce(start_at,due_at),id LIMIT 200").bind(m.family_id).all<Row>(), ctx.env.DB.prepare('SELECT id,name FROM members WHERE family_id=? AND active=1 ORDER BY id').bind(m.family_id).all<Row>(), ctx.env.DB.prepare("SELECT id,title,start_at FROM events WHERE family_id=? ORDER BY coalesce(start_at,created_at),id DESC LIMIT 200").bind(m.family_id).all<Row>()]);
  ...
  ${shoppingBatchForm(ctx,tasks.results,'',members.results,0,events.results)}
...
export async function taskEdit(request:Request,ctx:AppContext,id:number):Promise<Response>{
  ...
  const [members,shops,items,categories,events]=await Promise.all([
    ...
    ctx.env.DB.prepare("SELECT id,title,start_at FROM events WHERE family_id=? ORDER BY coalesce(start_at,created_at),id DESC LIMIT 200").bind(m.family_id).all<Row>()
  ]);
  ...
  const body=`<div class="card form-card"><h1>📝 タスク編集</h1><form id="taskEditForm">
    ...
    <label>関連予定</label><select name="event_id"><option value="0">予定なし</option>${events.results.map(e=>`<option value="${e.id}" ${Number(task.event_id)===Number(e.id)?'selected':''}>${safe(e.title)}${e.start_at?`（${safe(String(e.start_at).slice(0,10))}）`:''}</option>`).join('')}</select>
...
export async function itemEdit(request:Request,ctx:AppContext,id:number):Promise<Response>{
  ...
  const [tasks,events]=await Promise.all([ctx.env.DB.prepare(`SELECT id,title,start_at,due_at FROM tasks WHERE family_id=? AND status<>'completed' ORDER BY coalesce(start_at,due_at),id`).bind(m.family_id).all<Row>(), ctx.env.DB.prepare(`SELECT id,title,start_at FROM events WHERE family_id=? ORDER BY coalesce(start_at,created_at),id DESC LIMIT 200`).bind(m.family_id).all<Row>()]);
...
export async function shoppingNew(ctx:AppContext,date?:string,selectedTaskId=0):Promise<Response>{
  ...
  const [tasks,members,events]=await Promise.all([ctx.env.DB.prepare("SELECT id,title,start_at,due_at FROM tasks WHERE family_id=? AND status<>'completed' ORDER BY coalesce(start_at,due_at),id LIMIT 200").bind(m.family_id).all<Row>(),ctx.env.DB.prepare('SELECT id,name FROM members WHERE family_id=? AND active=1 ORDER BY id').bind(m.family_id).all<Row>(),ctx.env.DB.prepare("SELECT id,title,start_at FROM events WHERE family_id=? ORDER BY coalesce(start_at,created_at),id DESC LIMIT 200").bind(m.family_id).all<Row>()]);
  const body=`<div class="page-head"><div><div class="eyebrow">Family TODO LINE</div><h1>🛒 買い物を追加</h1></div><a class="btn gray" href="/app/shopping.php">戻る</a></div>${shoppingBatchForm(ctx,tasks.results,d,members.results,selectedTaskId,events.results)}`;
