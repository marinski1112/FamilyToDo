...
export function layout(title: string, body: string, active = ''): string {
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.74-wave55">':''; return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.74-wave55">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
export async function recurring(request: Request, ctx: AppContext): Promise<Response> {
  ...
  const postSuccess = (payload: Record<string, unknown>) => {
    ...
    const response = accepts.includes('text/html') && !accepts.includes('application/json')
      ? redirect('/app/recurring.php?saved=1')
  ...
  function refreshRecurringFields(){
    ...
    document.querySelectorAll('[data-rec-show]').forEach(el=>{const allowed=String(el.dataset.recShow||'').split(',');el.hidden=!allowed.includes(type);});
    const allDay=Boolean(q('all_day')?.checked);document.querySelectorAll('.rec-time-fields').forEach(el=>el.hidden=allDay);
    const cal=Boolean(q('calendar_visible')?.checked);const color=document.getElementById('recCalendarColorWrap');if(color)color.hidden=!cal;
  }
