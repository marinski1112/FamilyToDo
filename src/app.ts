...
export function layout(title: string, body: string, active = ''): string {
  const navItems = [
    ['/app/tasks.php','✅','タスク・イベント'],['/app/calendar.php','📅','カレンダー'],['/app/shopping.php','🛒','買い物'],['/app/messages.php','💬','伝言'],['/app/settings.php','⚙️','管理']
  ];
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.92-wave73">':'';
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.92-wave73">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],shopping:Row[],items:Row[]=[],detailTasks:Row[]=tasks):string{
  ...
  const script='<script src="/assets/calendar.js?v=12.92-wave73"></script>';
...
export async function messages(request:Request,ctx:AppContext):Promise<Response>{
  ...
  <script src="/assets/messages.js?v=12.92-wave73"></script>`;
...
function shoppingBatchForm(ctx:AppContext, tasks:Row[], date='', members:Row[]=[], selectedTaskId=0): string {
  ...
  <script src="/assets/shopping-new.js?v=12.92-wave73"></script>`;
}
...
export async function shopping(request:Request,ctx:AppContext):Promise<Response>{
  ...
  <script type="application/json" id="shoppingPayload">${JSON.stringify({shoppingDetail,csrf:ctx.session.csrfToken??''}).replaceAll('<','\\u003c').replaceAll('>','\\u003e').replaceAll('&','\\u0026')}</script><script src="/assets/shopping.js?v=12.92-wave73"></script>`;
...
export async function home(ctx:AppContext):Promise<Response>{
  ...
  const [todayPhysical,tomorrowPhysical,todayRecurring,tomorrowRecurring,shoppingCount,messageCount]=await Promise.all([
...
export async function taskView(ctx:AppContext, id:number):Promise<Response>{
  ...
  <script type="application/json" id="taskViewPayload">${JSON.stringify({csrf:ctx.session.csrfToken||'',id,occurrenceId,toggleType:isVirtual?'recurrence':'task',returnUrl:'/app/tasks.php?date='+encodeURIComponent(dateForChildren||'')}).replaceAll('<','\\u003c').replaceAll('>','\\u003e').replaceAll('&','\\u0026')}</script><script src="/assets/task-view.js?v=12.92-wave73"></script>`;
...
export async function taskEdit(request:Request,ctx:AppContext,id:number):Promise<Response>{
  ...
  const body=`<div class="card form-card"><h1>📝 タスク・イベント編集</h1><form id="taskEditForm">
    ...
    <script src="/assets/task-edit.js?v=12.92-wave73"></script>`;
...
export async function recurring(request: Request, ctx: AppContext): Promise<Response> {
  ...
  <script src="/assets/recurring.js?v=12.92-wave73"></script>
`;
