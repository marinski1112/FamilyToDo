...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],shopping:Row[],items:Row[]=[],detailTasks:Row[]=tasks,openDate=''):string{
  ...
  const script='<script src="/assets/calendar.js?v=12.138.0-wave119"></script><script src="/assets/occurrence-family-log.js?v=12.101-wave82"></script>';
...
export async function familyLog(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const babyQuickButtons=selectedSubjectRow&&subjectKind==='BABY'?(enabledTypes.includes('MILK')?milkAmountPresets:[]).map(amount=>`<button type="button" class="family-log-quick family-log-one-tap" data-quick-key="MILK" data-milk-amount="${amount}" data-subject-id="${selectedSubjectRow.id}"><span>🍼</span><strong>${amount}</strong></button>`).join('')+[['PEE','💧','おしっこ','DIAPER'],['POOP','💩','うんち','DIAPER'],['BABY_FOOD','🥣','離乳食','MEAL'],['BATH','🛁','お風呂','BATH'],['VOMIT','🤮','吐いた','CONDITION']].filter(x=>enabledTypes.includes(x[3])).map(x=>`<button type="button" class="family-log-quick family-log-one-tap" data-quick-key="${x[0]}" data-subject-id="${selectedSubjectRow.id}"><span>${x[1]}</span><strong>${x[2]}</strong></button>`).join('')+(enabledTypes.includes('SLEEP')?sleepAction(selectedSubjectRow):'')+`<button type="button" class="family-log-quick family-log-other" data-log-type="MEMO" data-subject-id="${selectedSubjectRow.id}"><span>＋</span><strong>その他</strong></button>`:'';
  ...
  <script type="application/json" id="familyLogPayload">${payload}</script><script src="/assets/family-log.js?v=12.138.0-wave119"></script>`;
