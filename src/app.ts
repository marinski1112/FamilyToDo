...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],shopping:Row[],items:Row[]=[],detailTasks:Row[]=tasks):string{
  ...
  const script='<script src="/assets/calendar.js?v=12.101-wave82"></script><script src="/assets/occurrence-family-log.js?v=12.101-wave82"></script>';
...
export async function settingsMembers(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const [members,invitations]=await Promise.all([
    ctx.env.DB.prepare('SELECT id,name,member_type,role,active,deleted_at,created_at FROM members WHERE family_id=? ORDER BY id').bind(m.family_id).all<Row>(),
...
export async function familyLog(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const familyLogIsAdmin=familyLogRole==='OWNER'||familyLogRole==='ADMIN';
  if(request.method==='POST'){
    const b=await bodyJson(request);await ensureCsrf(ctx,b.csrf);
  ...
  const adultAggregate=subjectParam==='adult';
  const selectedSubject=adultAggregate?0:Math.max(0,Number(subjectParam||0)||0);
  ...
  const enabledTypes=selectedSubjectRow
    ...
    :FAMILY_LOG_TYPES.filter(type=>(adultAggregate?adultSubjects:subjects.results).some(s=>familyLogEnabledTypes(s).includes(type)));
  ...
  if(adultAggregate&&recorderFilter){scopeSql.push('l.created_by=?');scopeParams.push(recorderFilter);}
  const dashboardRows=await ctx.env.DB.prepare(`SELECT date(l.occurred_at) day,l.log_type,l.detail_code,COUNT(*) count,
    ...
    GROUP BY date(l.occurred_at),l.log_type,l.detail_code ORDER BY day`).bind(...scopeParams).all<Row>();
  const latestMetrics=await ctx.env.DB.prepare(`SELECT log_type,amount,occurred_at FROM (SELECT l.log_type,l.amount,l.occurred_at,ROW_NUMBER() OVER (PARTITION BY l.log_type ORDER BY l.occurred_at DESC,l.id DESC) rn FROM family_logs l WHERE ${scopeSql.join(' AND ')} AND l.log_type IN ('TEMPERATURE','WEIGHT','HEIGHT') AND l.amount IS NOT NULL) WHERE rn=1`).bind(...scopeParams).all<Row>();
  const vaccineHistory=await ctx.env.DB.prepare(`SELECT l.occurred_at,l.value_text,l.note FROM family_logs l
    WHERE ${scopeSql.join(' AND ')} AND l.log_type='VACCINE' ORDER BY l.occurred_at DESC,l.id DESC LIMIT 50`).bind(...scopeParams).all<Row>();
  ...
  const dashboardHtml=`<details class="card family-log-dashboard" open><summary>📊 まとめ</summary><form method="get" class="family-log-range"><input type="hidden" name="subject" value="${adultAggregate?'adult':selectedSubject||''}"><button name="range" value="today">今日</button><button name="range" value="7">7日</button><button name="range" value="30">30日</button><button name="range" value="custom">期間指定</button><input type="date" name="from" value="${dashboardStart}"><input type="date" name="to" value="${dashboardEnd}"></form><p class="small">${dashboardStart} 〜 ${dashboardEnd}（JST）</p>${cards.length?`<div class="family-log-dashboard-grid">${dashboardCards}</div>${dashboardCharts}${vaccineHtml}`:'<p class="empty">この期間の記録はありません。</p>'}</details>`;
  ...
  if(adultAggregate&&recorderFilter){where.push('l.created_by=?');params.push(recorderFilter);}
  ...
  const timerWhere=selectedSubject?"x.family_id=? AND x.status='running' AND x.subject_id=?":adultAggregate?`x.family_id=? AND x.status='running' AND x.subject_id IN (${adultSubjects.map(()=>'?').join(',')||'NULL'})`:"x.family_id=? AND x.status='running'";
  ...
  const subjectChips=`<div class="family-log-subjects"><a class="${selectedSubject===0&&!adultAggregate?'active':''}" href="/app/family_log.php?date=${selectedDate}${rangeQuery}">すべて</a>${adultSubjects.length>1?`<a class="${adultAggregate?'active':''}" href="/app/family_log.php?date=${selectedDate}&subject=adult">👤 大人</a>`:adultSubjects.map(s=>`<a class="${selectedSubject===Number(s.id)?'active':''}" href="/app/family_log.php?date=${selectedDate}&subject=${s.id}">${esc(familyLogSubjectIcon(s))} ${esc(s.name)}</a>`).join('')}${subjects.results.filter(s=>familyLogSubjectKind(s.subject_kind)!=='ADULT').map(s=>`<a class="${selectedSubject===Number(s.id)?'active':''}" href="/app/family_log.php?date=${selectedDate}&subject=${s.id}">${esc(familyLogSubjectIcon(s))} ${esc(s.name)}</a>`).join('')}</div>`;
  ...
  const payload=JSON.stringify({csrf:ctx.session.csrfToken||'',selectedDate,selectedSubject:adultAggregate?Number(currentAdultSubject?.id||0):selectedSubject,adultAggregate,nowLocal,isAdmin:familyLogIsAdmin,logs:logMap,subjects:subjectMap,quickChores:quickChores.results.map(x=>({id:Number(x.id),name:String(x.name),icon:String(x.icon||'✨'),active:Number(x.active)===1}))}).replaceAll('<','\\u003c').replaceAll('>','\\u003e').replaceAll('&','\\u0026');
  ...
  const body=`<div class="page-head family-log-head"><div><div class="eyebrow">Family TODO LINE</div><h1>🐣 家族ログ</h1><div class="meta">${esc(selectedSubjectLabel)} ・ ${esc(selectedSubjectMode)}</div></div><div class="family-log-head-actions">${familyLogIsAdmin?'<a class="btn gray" href="/app/family_log_import.php">📥 データをインポート</a>':''}${selectedSubject?'<button type="button" class="btn gray" id="familyLogSubjectEdit">⚙️ 対象設定</button>':''}<button type="button" class="btn gray" id="familyLogSubjectOpen">＋ 対象追加</button></div></div>
  ${subjectChips}
  ${dashboardHtml}
  ...
  ${selectedSubject||adultAggregate?'':`<div class="card family-quick-chore-card"><div class="section-head"><div><h2>🧹 ちょこっと家事</h2><p class="small">家族共通・対象なしのHOUSEWORKとして記録します。項目編集はOWNER / ADMINのみ行えます。</p></div>${familyLogIsAdmin?'<button type="button" class="btn gray small" id="familyQuickChoreAdd">＋ 項目</button>':''}</div><div class="family-quick-chore-grid">${quickChoreHtml||'<p class="empty">「玄関を掃く」「タオル交換」など、繰り返し使う家事を追加してください。</p>'}</div></div>`}
  ...
  ${adultAggregate?`<form class="family-log-recorder-filter" method="get"><input type="hidden" name="date" value="${esc(selectedDate)}"><input type="hidden" name="subject" value="adult"><label>記録者 <select name="recorder" onchange="this.form.submit()"><option value="0">すべて</option>${members.results.map(member=>`<option value="${member.id}" ${recorderFilter===Number(member.id)?'selected':''}>${esc(member.name)}</option>`).join('')}</select></label></form>`:''}
  <div class="card family-log-timeline"><div class="section-head"><h2>履歴</h2><span class="meta">1ページ50件</span></div><form method="get" class="family-log-type-filter"><input type="hidden" name="date" value="${selectedDate}"><input type="hidden" name="subject" value="${adultAggregate?'adult':selectedSubject||''}"><select name="type" onchange="this.form.submit()"><option value="">すべて</option>${quickTypes.map(type=>`<option value="${type}" ${timelineType===type?'selected':''}>${FAMILY_LOG_TYPE_META[type].icon} ${FAMILY_LOG_TYPE_META[type].label}</option>`).join('')}<option value="MEMO" ${timelineType==='MEMO'?'selected':''}>📝 その他</option></select></form>${rowHtml||'<p class="empty">この日の記録はありません。</p>'}<nav class="family-log-pagination">${timelinePage>1?`<a class="btn gray" href="?date=${selectedDate}${subjectQuery}&type=${timelineType}&page=${timelinePage-1}${rangeQuery}">前へ</a>`:''}${timelineHasMore?`<a class="btn gray" href="?date=${selectedDate}${subjectQuery}&type=${timelineType}&page=${timelinePage+1}${rangeQuery}">さらに読み込む</a>`:''}</nav></div>
  ...
  <script type="application/json" id="familyLogPayload">${payload}</script><script src="/assets/family-log.js?v=12.110-wave91"></script>`;
...
export async function settingsDiagnostics(ctx:AppContext):Promise<Response>{
  ...
  if(role!=='OWNER'&&role!=='ADMIN') return html(layout('データ診断','<div class="card"><h1>🩺 データ診断</h1><p>管理者権限が必要です。</p><a class="btn gray" href="/app/settings.php">戻る</a></div>','/app/settings.php'));
  const familyId=m.family_id;
  const diagnosticNow=nowJst();
  const results=await Promise.all([
    ctx.env.DB.prepare("SELECT COUNT(*) c FROM (SELECT member_id,target_type,target_id,notify_at,COUNT(*) n FROM notifications WHERE family_id=? AND status IN ('pending','retry') GROUP BY member_id,target_type,target_id,notify_at HAVING COUNT(*)>1)").bind(familyId).first<Row>(),
    ctx.env.DB.prepare("SELECT COUNT(*) c FROM notifications n WHERE n.family_id=? AND n.status IN ('pending','retry') AND ((n.target_type='task' AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=n.target_id AND t.family_id=n.family_id)) OR (n.target_type='message' AND NOT EXISTS(SELECT 1 FROM messages x WHERE x.id=n.target_id AND x.family_id=n.family_id)))").bind(familyId).first<Row>(),
    ctx.env.DB.prepare("SELECT COUNT(*) c FROM recurrence_occurrences o WHERE o.family_id=? AND o.exception_task_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=o.exception_task_id AND t.family_id=o.family_id)").bind(familyId).first<Row>(),
    ctx.env.DB.prepare("SELECT COUNT(*) c FROM recurrence_rules r WHERE r.family_id=? AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=r.task_id AND t.family_id=r.family_id)").bind(familyId).first<Row>(),
    ctx.env.DB.prepare("SELECT COUNT(*) c FROM messages x WHERE x.family_id=? AND ((x.converted_to_task_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=x.converted_to_task_id AND t.family_id=x.family_id)) OR (x.converted_to_shopping_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM shopping_items s WHERE s.id=x.converted_to_shopping_id AND s.family_id=x.family_id)))").bind(familyId).first<Row>(),
    ctx.env.DB.prepare("SELECT (SELECT COUNT(*) FROM shopping_items s WHERE s.family_id=? AND s.task_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=s.task_id AND t.family_id=s.family_id)) + (SELECT COUNT(*) FROM items i WHERE i.family_id=? AND i.task_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=i.task_id AND t.family_id=i.family_id)) c").bind(familyId,familyId).first<Row>(),
    ...
    ctx.env.DB.prepare(`SELECT COUNT(*) c FROM (
      SELECT t.id FROM tasks t WHERE t.family_id=? AND ((t.visibility_scope='PRIVATE' AND (t.private_owner_id IS NULL OR NOT EXISTS(SELECT 1 FROM members om WHERE om.id=t.private_owner_id AND om.family_id=t.family_id))) OR (t.visibility_scope='FAMILY' AND t.private_owner_id IS NOT NULL))
      UNION ALL SELECT t.id FROM tasks t JOIN members om ON om.id=t.private_owner_id AND om.family_id=t.family_id WHERE t.family_id=? AND t.visibility_scope='PRIVATE' AND (om.active=0 OR om.deleted_at IS NOT NULL)
      UNION ALL SELECT ta.task_id FROM task_assignees ta JOIN tasks t ON t.id=ta.task_id WHERE t.family_id=? AND t.visibility_scope='PRIVATE' AND ta.member_id<>t.private_owner_id
      UNION ALL SELECT ia.item_id FROM item_assignees ia JOIN items i ON i.id=ia.item_id JOIN tasks t ON t.id=i.task_id AND t.family_id=i.family_id WHERE t.family_id=? AND t.visibility_scope='PRIVATE' AND ia.member_id<>t.private_owner_id
      UNION ALL SELECT sa.shopping_item_id FROM shopping_assignees sa JOIN shopping_items si ON si.id=sa.shopping_item_id JOIN tasks t ON t.id=si.task_id AND t.family_id=si.family_id WHERE t.family_id=? AND t.visibility_scope='PRIVATE' AND sa.member_id<>t.private_owner_id
      UNION ALL SELECT n.id FROM notifications n JOIN tasks t ON n.target_type='task' AND t.id=n.target_id AND t.family_id=n.family_id WHERE n.family_id=? AND n.status IN ('pending','retry') AND t.visibility_scope='PRIVATE' AND n.member_id<>t.private_owner_id
    )`).bind(familyId,familyId,familyId,familyId,familyId,familyId).first<Row>()
  ]);
  const labels=[
    ['通知の重複グループ','同じ宛先・対象・日時でpending/retryが複数ある状態'],
    ['通知の孤児','削除済みタスク/伝言を指すpending/retry通知'],
    ['定期タスク例外リンクの孤児','存在しない通常タスクをexceptionとして参照'],
    ['定期ルールの孤児','元テンプレートtaskが存在しない定期ルール'],
    ['伝言の変換先リンク切れ','削除済みタスク/買い物を参照'],
    ['買い物・持ち物のtaskリンク切れ','存在しないtaskへの紐付け'],
    ...
    ['定期タスク家族ログ連携の不整合','template/task/subject/log/occurrenceの不存在、家族不一致、無効対象またはprovenance不備'],
    ['PRIVATEデータの整合性','owner/family/担当者/通知はIDとissue typeだけで診断']
  ] as const;
  const counts=results.map(r=>Number(r?.c||0));
  const total=counts.reduce((a,b)=>a+b,0);
  const detailSamples=await Promise.all([
    ctx.env.DB.prepare("SELECT MIN(id) id,member_id,target_type,target_id,notify_at,COUNT(*) n FROM notifications WHERE family_id=? AND status IN ('pending','retry') GROUP BY member_id,target_type,target_id,notify_at HAVING COUNT(*)>1 LIMIT 20").bind(familyId).all<Row>(),
    ctx.env.DB.prepare("SELECT id,target_type,target_id,member_id,notify_at FROM notifications n WHERE n.family_id=? AND n.status IN ('pending','retry') AND ((n.target_type='task' AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=n.target_id AND t.family_id=n.family_id)) OR (n.target_type='message' AND NOT EXISTS(SELECT 1 FROM messages x WHERE x.id=n.target_id AND x.family_id=n.family_id))) LIMIT 20").bind(familyId).all<Row>(),
    ctx.env.DB.prepare("SELECT id,occurrence_date,exception_task_id FROM recurrence_occurrences o WHERE o.family_id=? AND o.exception_task_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=o.exception_task_id AND t.family_id=o.family_id) LIMIT 20").bind(familyId).all<Row>(),
    ctx.env.DB.prepare("SELECT id,task_id,name FROM recurrence_rules r WHERE r.family_id=? AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=r.task_id AND t.family_id=r.family_id) LIMIT 20").bind(familyId).all<Row>(),
    ctx.env.DB.prepare("SELECT id,converted_to_task_id,converted_to_shopping_id FROM messages x WHERE x.family_id=? AND ((x.converted_to_task_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=x.converted_to_task_id AND t.family_id=x.family_id)) OR (x.converted_to_shopping_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM shopping_items s WHERE s.id=x.converted_to_shopping_id AND s.family_id=x.family_id))) LIMIT 20").bind(familyId).all<Row>(),
    ctx.env.DB.prepare("SELECT 'shopping' kind,id,task_id FROM shopping_items s WHERE s.family_id=? AND s.task_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=s.task_id AND t.family_id=s.family_id) UNION ALL SELECT 'item' kind,id,task_id FROM items i WHERE i.family_id=? AND i.task_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM tasks t WHERE t.id=i.task_id AND t.family_id=i.family_id) LIMIT 20").bind(familyId,familyId).all<Row>(),
    ...
    ctx.env.DB.prepare(`SELECT issue_type,entity_id FROM (
      SELECT CASE WHEN t.visibility_scope='PRIVATE' AND t.private_owner_id IS NULL THEN 'private_owner_missing' WHEN t.visibility_scope='FAMILY' AND t.private_owner_id IS NOT NULL THEN 'family_has_private_owner' ELSE 'private_owner_family_mismatch' END issue_type,t.id entity_id FROM tasks t WHERE t.family_id=? AND ((t.visibility_scope='PRIVATE' AND (t.private_owner_id IS NULL OR NOT EXISTS(SELECT 1 FROM members om WHERE om.id=t.private_owner_id AND om.family_id=t.family_id))) OR (t.visibility_scope='FAMILY' AND t.private_owner_id IS NOT NULL))
      UNION ALL SELECT 'private_owner_inactive',t.id FROM tasks t JOIN members om ON om.id=t.private_owner_id AND om.family_id=t.family_id WHERE t.family_id=? AND t.visibility_scope='PRIVATE' AND (om.active=0 OR om.deleted_at IS NOT NULL)
      UNION ALL SELECT 'private_task_assignee',ta.task_id FROM task_assignees ta JOIN tasks t ON t.id=ta.task_id WHERE t.family_id=? AND t.visibility_scope='PRIVATE' AND ta.member_id<>t.private_owner_id
      UNION ALL SELECT 'private_item_assignee',ia.item_id FROM item_assignees ia JOIN items i ON i.id=ia.item_id JOIN tasks t ON t.id=i.task_id WHERE t.family_id=? AND t.visibility_scope='PRIVATE' AND ia.member_id<>t.private_owner_id
      UNION ALL SELECT 'private_shopping_assignee',sa.shopping_item_id FROM shopping_assignees sa JOIN shopping_items si ON si.id=sa.shopping_item_id JOIN tasks t ON t.id=si.task_id WHERE t.family_id=? AND t.visibility_scope='PRIVATE' AND sa.member_id<>t.private_owner_id
      UNION ALL SELECT 'private_notification_recipient',n.id FROM notifications n JOIN tasks t ON n.target_type='task' AND t.id=n.target_id WHERE n.family_id=? AND n.status IN ('pending','retry') AND t.visibility_scope='PRIVATE' AND n.member_id<>t.private_owner_id
    ) LIMIT 20`).bind(familyId,familyId,familyId,familyId,familyId,familyId).all<Row>()
  ]);
  ...
  const pushDiagnostics=await ctx.env.DB.prepare(`SELECT
    SUM(CASE WHEN s.enabled=1 THEN 1 ELSE 0 END) enabled_count,
    SUM(CASE WHEN s.failure_count>0 THEN 1 ELSE 0 END) failure_count,
    SUM(CASE WHEN m.id IS NULL OR m.family_id<>s.family_id THEN 1 ELSE 0 END) family_mismatch,
    SUM(CASE WHEN s.enabled=1 AND m.id IS NOT NULL AND (m.active=0 OR m.deleted_at IS NOT NULL) THEN 1 ELSE 0 END) inactive_enabled,
    SUM(CASE WHEN s.enabled=1 AND (s.last_success_at IS NULL OR s.last_success_at<datetime(?,'-31 days')) THEN 1 ELSE 0 END) stale_success
    FROM web_push_subscriptions s LEFT JOIN members m ON m.id=s.member_id WHERE s.family_id=?`).bind(diagnosticNow,familyId).first<Row>();
  const pushCard=`<div class="card"><h2>Web Push診断</h2><div class="diagnostic-row"><span>有効subscription</span><strong>${Number(pushDiagnostics?.enabled_count||0)}</strong></div><div class="diagnostic-row"><span>失敗端末</span><strong>${Number(pushDiagnostics?.failure_count||0)}</strong></div><div class="diagnostic-row"><span>member/family不一致</span><strong>${Number(pushDiagnostics?.family_mismatch||0)}</strong></div><div class="diagnostic-row"><span>停止・削除memberの有効端末</span><strong>${Number(pushDiagnostics?.inactive_enabled||0)}</strong></div><div class="diagnostic-row"><span>31日以上successなし</span><strong>${Number(pushDiagnostics?.stale_success||0)}</strong></div><p class="small">endpoint・鍵・authは診断に表示しません。</p></div>`;
  const importDiagnostics=await ctx.env.DB.prepare(`SELECT
    (SELECT COUNT(*) FROM family_log_import_batches b WHERE b.family_id=? AND NOT EXISTS(SELECT 1 FROM family_log_subjects s WHERE s.id=b.subject_id)) batch_subject_missing,
    ...
    (SELECT COUNT(*) FROM family_log_import_batches b WHERE b.family_id=? AND (b.processed_count<0 OR b.processed_count>b.record_count)) invalid_processed_count,
    (SELECT COUNT(*) FROM family_log_import_batches b WHERE b.family_id=? AND b.status='COMPLETED' AND b.processed_count<>b.record_count) completed_progress_mismatch,
    (SELECT COUNT(*) FROM family_log_import_batches b WHERE b.family_id=? AND b.status='COMPLETED' AND b.imported_count+b.skipped_count+b.error_count<>b.record_count) completed_outcome_mismatch`).bind(familyId,familyId,familyId,familyId,familyId,familyId,diagnosticNow,familyId,familyId,familyId,familyId).first<Row>();
  const importChecks=[['batch subject missing',importDiagnostics?.batch_subject_missing],['batch subject family mismatch',importDiagnostics?.batch_subject_family_mismatch],['family_log import_batch family mismatch',importDiagnostics?.batch_log_family_mismatch],['import_source_key duplicate anomaly',importDiagnostics?.duplicate_import_keys],['rolled back batch active imported logs',importDiagnostics?.active_logs_after_rollback],['stale IMPORTING batch (>1 hour)',importDiagnostics?.stale_importing],['FAILED batch',importDiagnostics?.failed_batches],['processed_count range anomaly',importDiagnostics?.invalid_processed_count],['COMPLETED progress mismatch',importDiagnostics?.completed_progress_mismatch],['COMPLETED outcome count mismatch',importDiagnostics?.completed_outcome_mismatch]];
