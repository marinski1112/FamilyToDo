...
export async function familyLog(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const managementChores=quickChores.results.map(chore=>`<button type="button" class="btn gray small family-quick-chore-edit" data-id="${chore.id}">${esc(chore.icon||'✨')} ${esc(chore.name)}${Number(chore.active)?'':'（非表示）'}</button>`).join('');
  const managementBody=`<div class="page-head family-log-head"><div><div class="eyebrow">管理</div><h1>🐣 家族ログ管理</h1></div><a class="btn gray" href="/app/settings.php">戻る</a></div><div class="card"><div class="section-head"><h2>記録対象・表示項目</h2>${familyLogIsAdmin?'<button type="button" class="btn small" id="familyLogSubjectOpen">＋ 対象追加</button>':''}</div>${managementSubjects}${selectedSubject&&familyLogIsAdmin?'<button type="button" class="btn secondary" id="familyLogSubjectEdit">この対象の表示項目・設定</button>':''}</div><div class="card"><div class="section-head"><h2>表示設定</h2>${familyLogIsAdmin?'<button type="button" class="btn gray small" id="familyLogSettingsOpen">変更</button>':''}</div><p>大人ログ：<strong>${showAdultLogs?'ON':'OFF'}</strong></p></div><div class="card"><div class="section-head"><h2>🧹 ちょこっと家事</h2>${familyLogIsAdmin?'<button type="button" class="btn small" id="familyQuickChoreAdd">＋ 項目</button>':''}</div><p class="small">項目の追加・編集・非表示・並び替えはOWNER / ADMINのみ行えます。</p><div class="family-log-management-actions">${managementChores}</div></div><div class="card settings-links"><div class="section-link"><div><h2>📥 データインポート</h2><p class="small">データの取込とインポート履歴を管理します。</p></div><a class="btn gray" href="/app/family_log_import.php">開く</a></div></div>`;
