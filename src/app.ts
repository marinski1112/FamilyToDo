...
export function layout(title: string, body: string, active = ''): string {
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.91-wave72">':''; return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.91-wave72">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
function renderDailyPage(ctx:AppContext,date:string,data:{tasks:Row[];items:Row[];shopping:Row[];expiredTasks:Row[]},tomorrow:boolean,unorganized:Row[]=[]):string {
  ...
  const rows=data.tasks.map(t=>{
    ...
    const taskShopping=linked.length?`<details class="task-shopping"><summary>🛒 買い物 ${linked.length}件</summary>${shoppingRows(linked)}</details>`:`<details class="task-shopping"><summary>🛒 買い物</summary><p class="empty">このタスクに紐付く買い物はありません。</p></details>`;
    ...
    const titleHtml=Number(t.id)<0?`<span>${esc(t.title)} <small>(定期)</small></span>`:`<a href="/task/view.php?id=${t.id}">${isEvent?'📌 ':''}${esc(t.title)}</a>`;
    ...
    return `<div class="row task-row ${isEvent?'event-task-row':''}">${mainHtml}<div class="meta">${esc(t.assignees||'')}${t.start_at?' ・ '+esc(String(t.start_at).slice(11,16)):t.due_at?' ・ '+(String(t.due_at).slice(11,16)==='00:00'?'終日':esc(String(t.due_at).slice(11,16))):''}${t.location?' ・ '+esc(t.location):''}</div>${shoppingBlock}</div>`;
  }).join('');
...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],shopping:Row[],items:Row[]=[],detailTasks:Row[]=tasks):string{
  ...
  const script='<script src="/assets/calendar.js?v=12.91-wave72"></script>';
...
export async function messages(request:Request,ctx:AppContext):Promise<Response>{
  ...
  <script src="/assets/messages.js?v=12.91-wave72"></script>`;
...
function shoppingBatchForm(ctx:AppContext, tasks:Row[], date='', members:Row[]=[], selectedTaskId=0): string {
  ...
  <script src="/assets/shopping-new.js?v=12.91-wave72"></script>`;
}
...
export async function taskView(ctx:AppContext, id:number):Promise<Response>{
  ...
  <script type="application/json" id="taskViewPayload">${JSON.stringify({csrf:ctx.session.csrfToken||'',id,occurrenceId,toggleType:isVirtual?'recurrence':'task',returnUrl:'/today.php?date='+encodeURIComponent(dateForChildren||'')}).replaceAll('<','\\u003c').replaceAll('>','\\u003e').replaceAll('&','\\u0026')}</script><script src="/assets/task-view.js?v=12.91-wave72"></script>`;
...
export async function taskEdit(request:Request,ctx:AppContext,id:number):Promise<Response>{
  ...
  const body=`<div class="card form-card"><h1>📝 タスク編集</h1><form id="taskEditForm">
    ...
    <script src="/assets/task-edit.js?v=12.91-wave72"></script>`;
...
export async function settingsMembers(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const [members,invitations]=await Promise.all([
    ...
    ctx.env.DB.prepare('SELECT i.id,i.expires_at,i.used_at,i.created_at,c.name created_by_name,u.name used_by_name FROM family_invitations i LEFT JOIN members c ON c.id=i.created_by LEFT JOIN members u ON u.id=i.used_by WHERE i.family_id=? ORDER BY i.id DESC LIMIT 20').bind(m.family_id).all<Row>()
  ]);
  const now=nowJst();
...
export async function inviteCreate(request: Request, ctx: AppContext): Promise<Response> {
  ...
  if(action==='revoke'){
    ...
    const inv=await ctx.env.DB.prepare('SELECT id,used_at FROM family_invitations WHERE id=? AND family_id=? LIMIT 1').bind(id,m.family_id).first<Row>();
    ...
    await logActivity(ctx,'REVOKED','family_invitation',id);return json({ok:true,id});
  }
...
export async function recurring(request: Request, ctx: AppContext): Promise<Response> {
  ...
  <script src="/assets/recurring.js?v=12.91-wave72"></script>
`;
