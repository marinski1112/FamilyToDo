...
export function layout(title: string, body: string, active = ''): string {
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.36-wave10">':''; return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.36-wave10">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
async function makeViewData(ctx: AppContext, date:string) {
  const [tasks,items,recurring,shopping] = await Promise.all([
    ...
    ctx.env.DB.prepare('SELECT * FROM shopping_items WHERE family_id=? AND (due_date=? OR due_date IS NULL) ORDER BY status,category,name,id').bind(ctx.member!.family_id,date).all<Row>()
  ]);
...
export async function calendar(request:Request,ctx:AppContext,month:string):Promise<Response>{
  ...
  const shopping=await ctx.env.DB.prepare('SELECT * FROM shopping_items WHERE family_id=? AND due_date BETWEEN ? AND ? ORDER BY due_date,category,name,id').bind(fid,from,to).all<Row>();
  return html(renderCalendarPage(ctx,m,start,end,[...tasks.results,...visibleRecur],events.results,shopping.results));
}

function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],events:Row[],shopping:Row[]):string{
  ...
  for(;cursor<=end;cursor.setUTCDate(cursor.getUTCDate()+1)){
    ...
    cells+=`<button type="button" class="${cls}" data-date="${d}" aria-label="${esc(d+(holiday?' '+holiday:''))}"><div class="num">${num}</div>${holiday?`<div class="holiday-name">${esc(holiday)}</div>`:''}<div class="calendar-items">${items.slice(0,4).map(t=>`<div class="calendar-item ${t._event?'event':''}">${esc(t.title)}</div>`).join('')}${items.length>4?`<div class="meta">+${items.length-4}件</div>`:''}${shoppingMap[d]?.length?`<div class="calendar-shopping">🛒 ${shoppingMap[d].length}件</div>`:''}</div></button>`;
  }

  const shoppingDetail=Object.fromEntries(Object.entries(shoppingMap).map(([k,v])=>[k,v.map(t=>({id:t.id,name:t.name,quantity:t.quantity,category:t.category,status:t.status,due_date:t.due_date}))]));
  ...
  const script=`<script>
  const detail=${JSON.stringify(detail)},shoppingDetail=${JSON.stringify(shoppingDetail)},holidays=${JSON.stringify(holidays)};
  ...
  function detailHtml(d){
    ...
    return (h?'<div class="modal-holiday">🎌 '+escJs(h)+'</div>':'')+(rows||'<div class="modal-row">この日の予定はありません。</div>')+(shops?'<div class="modal-subhead">買い物</div>'+shops:'');
  }
  function render(d){
    const x=detail[d]||[],h=holidays[d];
    ...
    box.innerHTML='<h2>'+d+'の予定</h2>'+(h?'<div class="modal-holiday">🎌 '+escJs(h)+'</div>':'')+(x.length?x.map(t=>{
      const time=t.start_at?String(t.start_at).slice(11,16):t.due_at?(String(t.due_at).slice(11,16)==='00:00'?'終日':String(t.due_at).slice(11,16)):'';
      const meta=[t.assignees,time,t.location].filter(Boolean).join(' ・ ');
      const check=!t.event?'<input type="checkbox" class="calendar-task-toggle" data-id="'+t.id+'" data-recurrence="'+(t.recurring?'1':'0')+'" '+(t.status==='completed'?'checked':'')+'> ':''; return '<div class="row"><label class="modal-check-row">'+check+'<strong>'+(t.event?'📅 '+escJs(t.title):'<a href="/task/view.php?id='+encodeURIComponent(t.id)+'">📝 '+escJs(t.title)+'</a>')+'</strong></label>'+(meta?'<div class="meta">'+escJs(meta)+'</div>':'')+'</div>';
    }).join(''):'<p>予定はありません。</p>')+(shops.length?'<div class="modal-subhead">🛒 買い物</div>'+shops.map(i=>'<div class="row"><label class="modal-check-row"><input type="checkbox" class="calendar-shop-toggle" data-id="'+i.id+'" '+(i.status==='completed'?'checked':'')+'> '+escJs(i.name)+(i.quantity&&i.quantity!=='1'?' × '+escJs(i.quantity):'')+'</label></div>').join(''):'')+'<p><a class="btn" href="/task/new.php?date='+d+'&return=calendar">＋ この日にタスクを追加</a> <a class="btn secondary" href="/calendar/event/new?date='+d+'">＋ 予定を追加</a> <a class="btn secondary" href="/app/shopping_new.php?date='+d+'">＋ 買い物を追加</a></p>';
  ...
  document.addEventListener('change',e=>{if(e.target?.classList?.contains('calendar-shop-toggle'))toggleShopping(e.target);if(e.target?.classList?.contains('calendar-task-toggle'))toggleTask(e.target);});
...
export async function settingsNotifications(request:Request,ctx:AppContext):Promise<Response>{
  const m=requireMember(ctx),role=String(m.role||'').toUpperCase(); if(role!=='OWNER'&&role!=='ADMIN')return new Response('管理者権限が必要です。',{status:403});
  const members=await ctx.env.DB.prepare('SELECT id,name,role,active FROM members WHERE family_id=? ORDER BY id').bind(m.family_id).all<Row>();
  if(request.method==='POST'){const b=await bodyJson(request);await ensureCsrf(ctx,b.csrf);for(const x of members.results){const id=Number(x.id);const enabled=Array.isArray(b.enabled_members)&&(b.enabled_members as unknown[]).map(Number).includes(id)?1:0;await ctx.env.DB.prepare(`INSERT INTO notification_settings(family_id,member_id,enabled,before_day,morning,one_hour_before,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(family_id,member_id) DO UPDATE SET enabled=excluded.enabled,updated_at=excluded.updated_at`).bind(m.family_id,id,enabled,Number(b.before_day||1),b.morning?1:0,b.one_hour_before?1:0,nowJst(),nowJst()).run();}return redirect('/app/settings_notifications.php');}
  const settings=await ctx.env.DB.prepare('SELECT * FROM notification_settings WHERE family_id=?').bind(m.family_id).all<Row>();const by=Object.fromEntries(settings.results.map(x=>[Number(x.member_id),x]));
  const body=`<div class="page-head"><div><div class="eyebrow">管理</div><h1>🔔 通知設定</h1></div><a class="btn gray" href="/app/settings.php">戻る</a></div><div class="card form-card"><form method="post"><input type="hidden" name="csrf" value="${esc(ctx.session.csrfToken||'')}"><label>通知するメンバー</label><div class="choice-list">${members.results.map(x=>`<label class="checkrow"><input type="checkbox" name="enabled_members" value="${x.id}" ${Number(by[Number(x.id)]?.enabled??1)?'checked':''}><span>${esc(x.name)}</span></label>`).join('')}</div><label>何日前</label><input type="number" name="before_day" min="0" max="30" value="${esc(by[m.id]?.before_day??1)}"><label class="checkrow"><input type="checkbox" name="morning" ${Number(by[m.id]?.morning??1)?'checked':''}><span>朝通知</span></label><label class="checkrow"><input type="checkbox" name="one_hour_before" ${Number(by[m.id]?.one_hour_before??0)?'checked':''}><span>1時間前通知</span></label><button>保存する</button></form></div>`;
