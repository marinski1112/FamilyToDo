...
export function layout(title: string, body: string, active = ''): string {
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.87-wave68">':''; return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.87-wave68">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],shopping:Row[],items:Row[]=[],detailTasks:Row[]=tasks):string{
  ...
  const script='<script src="/assets/calendar.js?v=12.87-wave68"></script>';
...
export async function messages(request:Request,ctx:AppContext):Promise<Response>{
  ...
  <script src="/assets/messages.js?v=12.87-wave68"></script>`;
...
export async function recurring(request: Request, ctx: AppContext): Promise<Response> {
  ...
  <script src="/assets/recurring.js?v=12.87-wave68"></script>
`;
