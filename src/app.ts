...
export function layout(title: string, body: string, active = ''): string {
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.78-wave59">':''; return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.78-wave59">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],shopping:Row[],items:Row[]=[]):string{
  ...
  calendarCard?.addEventListener('pointerdown',e=>{if(!e.target.closest('.calendar-grid'))return;if(e.pointerType==='mouse'&&e.button!==0)return;swipeX=e.clientX;swipeY=e.clientY;swipeActive=true;swipeMoved=false;});
  calendarCard?.addEventListener('pointermove',e=>{if(!swipeActive)return;const dx=e.clientX-swipeX,dy=e.clientY-swipeY;if(Math.abs(dx)>18||Math.abs(dy)>18)swipeMoved=true;});
  calendarCard?.addEventListener('pointerup',e=>{if(!swipeActive)return;swipeActive=false;const dx=e.clientX-swipeX,dy=e.clientY-swipeY;if(Math.abs(dx)>60&&Math.abs(dx)>Math.abs(dy)){loadMonth(dx<0?currentNext:currentPrev,dx<0?1:-1);return;}if(!swipeMoved){const cell=e.target.closest('.calendar-cell');if(cell?.dataset.date)render(cell.dataset.date);}});
  calendarCard?.addEventListener('pointercancel',()=>{swipeActive=false;});
  calendarCard?.addEventListener('click',e=>{if(swipeMoved)return;const cell=e.target.closest('.calendar-cell');if(cell?.dataset.date)render(cell.dataset.date);});
  ...
  const initialOpenDate=new URLSearchParams(location.search).get('date');if(initialOpenDate&&/^\d{4}-\d{2}-\d{2}$/.test(initialOpenDate))setTimeout(()=>render(initialOpenDate),0);
  </script>`;
