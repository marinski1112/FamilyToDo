...
import { googleCalendarInboundAuthorizationStatus } from './google-calendar-inbound-auth';
import {
...
export async function integrationsSettings(request: Request, ctx: AppContext) {
  ...
  const role = String(ctx.member?.role || '').toUpperCase();
  const canAuthorizeInbound = ['OWNER','ADMIN'].includes(role);
  const inboundStatus = ctx.member ? await googleCalendarInboundAuthorizationStatus(ctx.env.DB,ctx.member.family_id).catch(()=>({linked:false})) : {linked:false};
  const inboundPreview = inboundStatus.linked&&canAuthorizeInbound ? `<div id="googleCalendarInboundPreview"><p class="small">読み取り専用プレビューです。ここではFamilyToDoにもGoogle Calendarにも書き込みません。定期予定は安全なidentity semanticsが完成するまで取り込み候補にしません。</p><div><button type="button" class="btn gray" id="googleCalendarInboundLoad">カレンダー一覧を読み込む</button></div><div style="margin-top:8px"><select id="googleCalendarInboundCalendar" disabled style="max-width:100%"><option value="">取り込み元を選択</option></select></div><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px"><label class="small">開始日 <input type="date" id="googleCalendarInboundFrom"></label><label class="small">終了日 <input type="date" id="googleCalendarInboundTo"></label><button type="button" class="btn" id="googleCalendarInboundPreviewButton" disabled>予定をプレビュー</button></div><div class="small" id="googleCalendarInboundResult" style="margin-top:8px"></div><div id="googleCalendarInboundRows" style="display:grid;gap:6px;margin-top:8px"></div><script src="/assets/google-calendar-inbound.js"></script></div>` : '';
  const inboundPanel = `<section class="calendar-inbound-panel" id="googleCalendarInboundAuth" aria-labelledby="googleCalendarInboundHeading"><h3 id="googleCalendarInboundHeading">Google Calendarから取り込む</h3><p class="small">FamilyToDo → Google Calendar の同期用権限とは分離して、カレンダー一覧と予定の読み取りだけを明示的に許可します。</p><p class="small">${inboundStatus.linked?'読み取り許可済みです。取り込み元カレンダーと期間を選んで、安全性を確認できます。':'まだ読み取り許可されていません。許可してもGoogle側の予定を変更・削除しません。'}</p>${canAuthorizeInbound?'<a class="btn calendar-inbound-auth-btn" href="/oauth/google-calendar/inbound/authorize">Google Calendarの読み取りを許可</a>':'<p class="small">OWNER / ADMIN が読み取りを許可できます。</p>'}${inboundPreview}</section>`;
  source = source.replace('Google CalendarからFamilyToDoへの予定取り込みは行いません。','Google Calendarからの予定取り込みは、明示的な読み取り許可を分離して行います。');
  const googleTasksCardMarker='<div class="card"><h2>Google Tasks</h2>';
  const googleTasksCardStart=source.indexOf(googleTasksCardMarker);
  const googleCalendarCardClose=googleTasksCardStart>=0?source.lastIndexOf('</div>',googleTasksCardStart):-1;
  if(googleCalendarCardClose>=0) source=source.slice(0,googleCalendarCardClose)+inboundPanel+source.slice(googleCalendarCardClose);
  else source=source.replace('</body>',`${inboundPanel}</body>`);
  const enhancement = `<style>.calendar-backfill-limit{display:none!important}.calendar-event-tools{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.calendar-inbound-panel{border-top:1px solid #dfe3eb;margin-top:18px;padding-top:18px}.calendar-inbound-panel h3{margin-top:0}.calendar-inbound-auth-btn{display:flex;align-items:center;justify-content:center;width:100%;min-height:44px;box-sizing:border-box;margin-top:8px}.calendar-inbound-preview-row{border:1px solid #ddd;border-radius:8px;padding:8px}.calendar-inbound-preview-head{display:flex;justify-content:space-between;gap:8px}</style><script>(()=>{const clean=()=>document.querySelectorAll('.calendar-backfill-limit').forEach(el=>el.remove());clean();new MutationObserver(clean).observe(document.body,{childList:true,subtree:true});const history=document.getElementById('calendarHistoryBackfill');if(history&&!document.getElementById('calendarBatchNote')){const note=document.createElement('div');note.id='calendarBatchNote';note.className='small';note.textContent='全履歴EVENTは全件キュー登録できます。Google APIへの送信はWorker上限を避けるため小分けで処理します。';history.insertAdjacentElement('afterend',note);const tools=document.createElement('div');tools.className='calendar-event-tools';tools.innerHTML='<button type="button" class="btn danger" id="calendarEventReset">EVENTを全クリア</button>';note.insertAdjacentElement('afterend',tools);const result=document.getElementById('calendarResult');const call=async(action,extra={})=>{const r=await fetch('/api/google-calendar/backfill',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({csrf,action,...extra})});const d=await r.json();if(!r.ok||d.ok===false)throw new Error(d.error||('HTTP '+r.status));return d};tools.querySelector('#calendarEventReset').addEventListener('click',async e=>{const b=e.currentTarget;b.disabled=true;try{const p=await call('preview_event_reset');if(p.dependency_count){result.textContent='EVENT全クリア不可: 関連データ '+p.dependency_count+'件があります。';return}if(!confirm('Family TODOのEVENT '+p.event_count+'件'+(p.recurring_rule_count?'（定期rule '+p.recurring_rule_count+'件を含む）':'')+'を全削除します。TASKは削除しません。続行しますか？'))return;const d=await call('reset_event_data',{confirm:'RESET_ALL_EVENTS'});result.textContent='EVENT全クリア完了: '+d.deleted_events+'件 / 定期rule '+d.deleted_rules+'件。次に「新しい同期用カレンダーを作成」→ ICS再インポートの順で進めてください。';}catch(error){result.textContent='EVENT全クリア失敗: '+(error instanceof Error?error.message:String(error));}finally{b.disabled=false}})}const old=document.getElementById('calendarSync');if(old){const button=old.cloneNode(true);old.replaceWith(button);button.addEventListener('click',async()=>{const out=document.getElementById('calendarResult');button.disabled=true;try{const r=await fetch('/api/google-calendar/sync',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({csrf})});const d=await r.json().catch(()=>({ok:false,error:'応答を解析できません'}));if(!r.ok||d.ok===false)throw new Error(d.error||('HTTP '+r.status));out.textContent=d.unchanged?'変更はありません':('送信 '+d.sent+'件 / エラー '+d.errors+'件'+(d.more?' / 続きあり（もう一度押してください）':''));}catch(error){out.textContent='同期失敗: '+(error instanceof Error?error.message:String(error));}finally{button.disabled=false}})}})();</script>`;
