...
import { archiveRecurrenceRuleOccurrenceStatements, archiveTaskCompletionStatements } from './lifecycle';
...
function hasRetiredGoodsPayload(body:Record<string,unknown>):boolean{
  const values=(value:unknown):unknown[]=>Array.isArray(value)?value:value===undefined||value===null?[]:[value];
  const nonBlank=(value:unknown)=>String(value??'').trim()!=='';
  const shopping=values(body.shopping);
  if(shopping.some(entry=>{
    if(!entry||typeof entry!=='object')return nonBlank(entry);
    const row=entry as Record<string,unknown>;
    return nonBlank(row.name)||nonBlank(row.url)||nonBlank(row.category)||(nonBlank(row.quantity)&&String(row.quantity).trim()!=='1');
  }))return true;
  if(values(body.items).some(nonBlank))return true;
  if(values(body['shopping_name[]']).some(nonBlank))return true;
  if(values(body['shopping_url[]']).some(nonBlank))return true;
  if(values(body['shopping_quantity[]']).some(v=>nonBlank(v)&&String(v).trim()!=='1'))return true;
  if(values(body['item_name[]']).some(nonBlank))return true;
  return nonBlank(body.shopping_category);
}
...
export async function recurring(request:Request,ctx:AppContext):Promise<Response>{
  ...
  try{
    ...
    if(request.method==='POST'){
      ...
      if((action==='create'||action==='update')&&numberValues(b.assignees,1,2147483647).length)throw new RecurringBadRequest('担当者指定は廃止されました。画面を再読み込みしてください。');
      if((action==='create'||action==='update')&&hasRetiredGoodsPayload(b)){
        throw new RecurringBadRequest('定期タスクと買い物・持ち物の紐づけは廃止されました。画面を再読み込みし、買い物・持ち物はチェックリストから登録してください。');
      }
      ...
      if(action==='delete'){
        ...
        if(taskId){
          statements.unshift(ctx.env.DB.prepare("UPDATE notifications SET status='cancelled',updated_at=? WHERE target_type='task' AND target_id=? AND family_id=? AND status IN ('pending','retry')").bind(deleteNow,taskId,m.family_id));
          statements.push(
            ...archiveTaskCompletionStatements(ctx.env.DB,m.family_id,taskId,deleteNow),
            ctx.env.DB.prepare('DELETE FROM tasks WHERE id=? AND family_id=?').bind(taskId,m.family_id),
          );
        }
      ...
      if(action==='update'){
        ...
        const now=nowJst();
        if(numberValues(b.assignees,1,2147483647).length)throw new RecurringBadRequest('担当者指定は廃止されました。画面を再読み込みしてください。');
        ...
        await ctx.env.DB.prepare(`UPDATE recurrence_occurrences SET
          status=CASE WHEN EXISTS(SELECT 1 FROM recurrence_occurrence_completions c JOIN members am ON am.id=c.member_id AND am.family_id=? AND am.active=1 WHERE c.occurrence_id=recurrence_occurrences.id) THEN 'completed' ELSE 'pending' END,
          completed_by=(SELECT c.member_id FROM recurrence_occurrence_completions c JOIN members am ON am.id=c.member_id AND am.family_id=? AND am.active=1 WHERE c.occurrence_id=recurrence_occurrences.id ORDER BY c.completed_at DESC LIMIT 1),
          completed_at=(SELECT c.completed_at FROM recurrence_occurrence_completions c JOIN members am ON am.id=c.member_id AND am.family_id=? AND am.active=1 WHERE c.occurrence_id=recurrence_occurrences.id ORDER BY c.completed_at DESC LIMIT 1),
          updated_at=? WHERE recurrence_rule_id=? AND family_id=? AND status<>'excluded'`).bind(m.family_id,m.family_id,m.family_id,now,id,m.family_id).run();
    ...
    const ruleJson=rows.results.map(r=>JSON.stringify({id:Number(r.id),title:String(r.title||r.name||''),description:String(r.description||''),recurrence_type:String(r.recurrence_type||'DAILY'),interval_value:Number(r.interval_value||1),start_date:String(r.start_date||''),end_date:String(r.end_date||''),weekdays:parseJsonArray(r.weekdays_json),monthdays:parseJsonArray(r.monthdays_json),week_numbers:parseJsonArray(r.week_numbers_json),week_number:Number(r.week_number||1),business_day_ordinal:Number(r.business_day_ordinal||1),location:String(r.location||''),calendar_color:String(r.calendar_color||'#7c3aed'),all_day:Number(r.all_day??1)===1,calendar_visible:Number(r.calendar_visible??1)===1,start_time:String(r.start_at||'').slice(11,16),end_time:String(r.end_at||'').slice(11,16),family_log_template:templateByTask.get(Number(r.task_id))||null})).map(x=>x.replaceAll('&','&amp;').replaceAll('"','&quot;').replaceAll('<','&lt;').replaceAll('>','&gt;'));
