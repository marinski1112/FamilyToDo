...
const canManageMessage=(memberId:number,memberRole:unknown,senderId:unknown)=>{
  const role=String(memberRole||'').toUpperCase();
  return Number(senderId)===memberId||role==='OWNER'||role==='ADMIN';
};
...
export async function messages(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(request.method==='POST'){
    ...
    if(action==='delete'){
      ...
      if(!canManageMessage(m.id,m.role,msg.sender_id))return json({ok:false,error:'権限がありません。'},403);
    ...
    if(action==='convert_shopping'||action==='convert_task'){
      ...
      if(action==='convert_shopping'){
        ...
        if(Number(b.task_id||0)||Array.isArray(b.assignees)&&b.assignees.length)return bad('買い物のタスク紐づけ・担当者指定は廃止されました。画面を再読み込みしてください。');
        ...
        if(!sid){
          await ctx.env.DB.prepare("INSERT OR IGNORE INTO shopping_items(family_id,name,quantity,category,memo,due_date,status,created_by,created_at,updated_at,url,source_message_id) VALUES(?,?,?,?,?,?,'pending',?,?,?,?,?)").bind(m.family_id,name,quantity,category,memo,dueRaw||null,m.id,now,now,productUrl,id).run();
      ...
      if(b.completion_mode&&b.completion_mode!=='ANY')return bad('完了条件は家族の誰か1人です。画面を再読み込みしてください。');
      ...
      if(Array.isArray(b.assignees)&&b.assignees.length)return bad('担当者指定は廃止されました。画面を再読み込みしてください。');
      ...
      const mutations=[] as D1PreparedStatement[];
      if(reminderAt){
        const rs=await ctx.env.DB.prepare('SELECT id FROM members WHERE family_id=? AND active=1 AND deleted_at IS NULL').bind(m.family_id).all<Row>();
  ...
  <div class="card message-list"><h2>伝言一覧</h2>${rows.results.map(r=>`<div class="row message-row"><div>${esc(r.text)}</div><div class="meta">${esc(r.sender_name||'')} → ${esc(r.recipient_name||'全員')} ・ ${esc(r.created_at||'')}</div>${r.reminder_at?`<div class="meta">🔔 通知 ${esc(String(r.reminder_at).slice(0,16))}</div>`:''}${r.converted_to_shopping_id?`<div class="converted-badge">🛒 買い物：${esc(r.shopping_name||'登録済み')}</div>`:r.converted_to_task_id?`<div class="converted-badge">📝 タスク・イベント：${esc(r.task_title||'登録済み')}</div>`:`<div class="message-actions"><button class="btn small convert-shopping" data-id="${r.id}" data-text="${esc(r.text)}" data-target="${Number(r.target_member_id||0)}">🛒 買い物に追加</button><button class="btn gray small convert-task" data-id="${r.id}" data-text="${esc(r.text)}" data-target="${Number(r.target_member_id||0)}">📝 タスク・イベントに追加</button></div>`}${canManageMessage(m.id,m.role,r.sender_id)?`<div class="message-actions"><button class="btn gray small edit-message" data-id="${r.id}" data-text="${esc(r.text)}" data-target="${Number(r.target_member_id||0)}" data-reminder="${esc(r.reminder_at||'')}">編集</button><button class="btn danger small delete-message" data-id="${r.id}">削除</button></div>`:''}</div>`).join('')||'<p>伝言はありません。</p>'}</div>
  <div class="card form-card"><form id="msgForm"><input type="hidden" name="csrf" value="${esc(ctx.session.csrfToken??'')}"><label for="messageCreateTarget">宛先</label><select id="messageCreateTarget" name="target_member_id"><option value="0">家族全員</option>${members.results.map((r:Row)=>`<option value="${r.id}">${esc(r.name)}</option>`).join('')}</select><label for="messageCreateText">伝言</label><textarea id="messageCreateText" name="text" required></textarea><label for="messageCreateReminder">通知日時（任意）</label><input id="messageCreateReminder" type="datetime-local" name="reminder_at"><p class="small">指定すると宛先へその日時に伝言内容を設定した通知方法で通知します。</p><button type="submit">投稿する</button></form></div>
  <div class="message-shopping-backdrop" id="messageShoppingModal" aria-hidden="true"><div class="message-shopping-dialog"><div class="section-head"><h2>🛒 伝言を買い物に追加</h2><button type="button" class="btn gray small" id="messageShoppingClose">×</button></div><form id="messageShoppingForm"><input type="hidden" name="message_id"><label for="messageShoppingName">商品名</label><input id="messageShoppingName" name="name" maxlength="255" required><label for="messageShoppingQuantity">数量</label><input id="messageShoppingQuantity" name="quantity" value="1"><label for="messageShoppingCategory">カテゴリー</label><input id="messageShoppingCategory" name="category" placeholder="例：食品"><label for="messageShoppingDueDate">期限（任意）</label><input id="messageShoppingDueDate" type="date" name="due_date"><label for="messageShoppingMemo">メモ</label><textarea id="messageShoppingMemo" name="memo"></textarea><label for="messageShoppingUrl">商品URL（任意）</label><input id="messageShoppingUrl" type="url" name="url" placeholder="https://..."><div id="messageShoppingStatus" class="small" aria-live="polite"></div><button type="submit" id="messageShoppingSubmit">買い物に追加</button></form></div></div>
  <div class="message-task-backdrop" id="messageTaskModal" aria-hidden="true"><div class="message-task-dialog"><div class="section-head"><h2>📝 伝言をタスク・イベントに追加</h2><button type="button" class="btn gray small" id="messageTaskClose">×</button></div><form id="messageTaskForm"><input type="hidden" name="message_id"><label for="messageTaskMode">追加方法</label><select name="mode" id="messageTaskMode"><option value="existing">既存タスク・イベントに追加</option><option value="new">新しいタスク・イベントを作成</option></select><div id="existingTaskFields"><label for="messageTaskExistingTarget">追加先タスク・イベント</label><select id="messageTaskExistingTarget" name="task_id"><option value="0">選択してください</option>${tasks.results.map(t=>`<option value="${t.id}">${esc(t.title)}${t.start_at||t.due_at?' ・ '+esc(String(t.start_at||t.due_at).slice(0,10)):''}</option>`).join('')}</select><label class="checkrow"><input type="checkbox" name="append_message" checked><span>伝言本文を説明へ追記する</span></label></div><div id="newTaskFields" style="display:none"><label for="messageTaskTitle">タイトル</label><input id="messageTaskTitle" name="title" maxlength="255"><label for="messageTaskDescription">説明</label><textarea id="messageTaskDescription" name="description"></textarea><div class="date-option-row"><div><span class="small" id="messageTaskStartDateLabel">開始日</span><input id="messageTaskStartDate" aria-labelledby="messageTaskStartDateLabel" type="date" name="date" value="${today}"></div><div><span class="small" id="messageTaskEndDateLabel">終了日</span><input id="messageTaskEndDate" aria-labelledby="messageTaskEndDateLabel" type="date" name="end_date" value="${today}"></div><label class="checkrow"><input type="checkbox" name="no_date"><span>期限なし</span></label></div><label class="checkrow"><input id="messageTaskAllDay" type="checkbox" name="all_day" checked><span>終日</span></label><div id="messageTaskTimeFields" class="task-time-fields message-task-time" style="display:none"><div><label for="messageTaskStartTime">開始時刻</label><input id="messageTaskStartTime" type="time" name="start_time"></div><div><label for="messageTaskEndTime">終了時刻</label><input id="messageTaskEndTime" type="time" name="end_time"></div></div><label for="messageTaskLocation">場所</label><input id="messageTaskLocation" name="location"><label class="checkrow"><input type="checkbox" name="is_event"><span>イベントとして登録（チェック・期限切れ対象なし）</span></label><label for="messageTaskReminder">通知日時（任意）</label><input id="messageTaskReminder" type="datetime-local" name="task_reminder_at"><label class="checkrow"><input id="messageTaskCalendarVisible" type="checkbox" name="calendar_visible" checked><span>カレンダーに表示する</span></label><div id="messageTaskCalendarColorWrap"><label for="messageTaskCalendarColor">カレンダー色</label><select id="messageTaskCalendarColor" name="calendar_color"><option value="#7c3aed">紫</option><option value="#2563eb">青</option><option value="#16a34a">緑</option><option value="#ea580c">橙</option><option value="#dc2626">赤</option><option value="#db2777">ピンク</option><option value="#0891b2">水色</option><option value="#64748b">灰</option></select></div></div><div id="messageTaskStatus" class="small" aria-live="polite"></div><button type="submit" id="messageTaskSubmit">追加する</button></form></div></div>
  <div class="message-edit-backdrop" id="messageEditModal" aria-hidden="true"><div class="message-edit-dialog"><div class="section-head"><h2>✏️ 伝言を編集</h2><button type="button" class="btn gray small" id="messageEditClose">×</button></div><form id="messageEditForm"><input type="hidden" name="message_id"><label for="messageEditTarget">宛先</label><select id="messageEditTarget" name="target_member_id"><option value="0">家族全員</option>${members.results.map((r:Row)=>`<option value="${r.id}">${esc(r.name)}</option>`).join('')}</select><label for="messageEditText">伝言</label><textarea id="messageEditText" name="text" maxlength="5000" required></textarea><label for="messageEditReminder">通知日時（任意）</label><input id="messageEditReminder" type="datetime-local" name="reminder_at"><p class="small">空欄で通知予約を解除します。</p><div id="messageEditStatus" class="small" aria-live="polite"></div><button type="submit" id="messageEditSubmit">保存する</button></form></div></div>
