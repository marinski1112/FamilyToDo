...
export async function shoppingCategoryMutationApi(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(action==='delete_many'){
    ...
    const orderKey=kind==='shopping'?ORDER_KEY:ITEM_ORDER_KEY;
    const currentOrder=await readOrder(ctx,member.family_id,orderKey);
    const removedKeys=new Set(names.map(categoryKey));
    const nextOrder=currentOrder.filter(name=>!removedKeys.has(categoryKey(name)));
    const now=nowJst();
    const statements=[];
    ...
    for(const name of names){
      if(kind==='shopping'){
        ...
        statements.push(ctx.env.DB.prepare('UPDATE shopping_items SET category=NULL,updated_at=? WHERE family_id=? AND category=? COLLATE NOCASE').bind(now,member.family_id,name));
        ...
        statements.push(ctx.env.DB.prepare('UPDATE items SET category=NULL,updated_at=? WHERE family_id=? AND category=? COLLATE NOCASE').bind(now,member.family_id,name));
    ...
    statements.push(ctx.env.DB.prepare(`INSERT INTO family_settings(family_id,setting_key,setting_value,updated_at) VALUES(?,?,?,?)
    ...
    return commitSession(json({ok:true,deleted:names,kind}),ctx.session,ctx.env.APP_SECRET);
  }
