...
export async function reconcileTaskCompletionAfterAssigneeChange(
  ...
  const task=await DB.prepare('SELECT completion_mode FROM tasks WHERE id=? AND family_id=? LIMIT 1').bind(taskId,familyId).first<Row>();
  if(!task)return;

  const assigned=await DB.prepare('SELECT COUNT(*) c FROM task_assignees ta JOIN members am ON am.id=ta.member_id AND am.active=1 WHERE ta.task_id=?').bind(taskId).first<Row>();
  ...
  const done=assignedCount>0
    ?await DB.prepare('SELECT COUNT(*) c FROM task_completions tc JOIN task_assignees ta ON ta.task_id=tc.task_id AND ta.member_id=tc.member_id JOIN members am ON am.id=ta.member_id AND am.active=1 WHERE tc.task_id=?').bind(taskId).first<Row>()
    :await DB.prepare('SELECT COUNT(*) c FROM task_completions tc JOIN members am ON am.id=tc.member_id AND am.family_id=? AND am.active=1 WHERE tc.task_id=?').bind(familyId,taskId).first<Row>();
  ...
  const latest=isComplete
    ?assignedCount>0
      ?await DB.prepare('SELECT tc.member_id,tc.completed_at FROM task_completions tc JOIN task_assignees ta ON ta.task_id=tc.task_id AND ta.member_id=tc.member_id JOIN members am ON am.id=ta.member_id AND am.active=1 WHERE tc.task_id=? ORDER BY tc.completed_at DESC,tc.member_id DESC LIMIT 1').bind(taskId).first<Row>()
      :await DB.prepare('SELECT tc.member_id,tc.completed_at FROM task_completions tc JOIN members am ON am.id=tc.member_id AND am.family_id=? AND am.active=1 WHERE tc.task_id=? ORDER BY tc.completed_at DESC,tc.member_id DESC LIMIT 1').bind(familyId,taskId).first<Row>()
    :null;
