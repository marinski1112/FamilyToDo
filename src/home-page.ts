...
export async function home(ctx:AppContext):Promise<Response>{
  const member=ctx.member;if(!member)return redirect('/liff?next=%2Fapp%2Findex.php');
  const today=dateOnly(),dashboard=await loadHomeDashboard(ctx,today),journal=dashboard.yesterdayJournal;
  const fortune=dailyFortune(Number(member.family_id),Number(member.id),today);
  ...
  const fortuneCard=`<details class="card home-fortune"><summary>🔮 今日の占い <span aria-label="運勢 ${fortune.stars} / 5">${stars}</span></summary><p><strong>${esc(member.name)}さんの今日</strong></p><p>${esc(fortune.headline)}</p><p class="small">🍀 ${esc(fortune.luckyAction)} ・ 🎨 ${esc(fortune.luckyColor)}</p><p class="small">※ 娯楽用です。健康・お金・仕事など大事な判断には使わないでください。</p></details>`;
