...
export async function executeFamilyAiTool(db:D1Database,familyId:number,name:string,a:Args,refs=new Map<string,number>()){
 ...
 if(name==='daily_family_log_aggregate'){
  ...
  const sql=`SELECT substr(occurred_at,1,10) day,${expr} value FROM family_logs WHERE family_id=? AND log_type=? ${sid?'AND subject_id=?':''} AND deleted_at IS NULL AND substr(occurred_at,1,10) BETWEEN ? AND ? GROUP BY day${having} ORDER BY day ${String(a.order)==='ASC'?'ASC':'DESC'} LIMIT ?`;
  return (await db.prepare(sql).bind(...binds).all<Row>()).results;
 }
 if(name==='quick_chore_stats'){
  const mid=resolve(refs,a.member_ref,'MEMBER'),group=String(a.group_by),select=group==='DAY'?"substr(l.occurred_at,1,10) label":group==='CHORE'?'COALESCE(c.name,\'未設定\') label':group==='MEMBER'?'COALESCE(m.name,\'不明\') label':"'合計' label";const binds:any[]=[familyId,from,to];if(mid)binds.push(mid);if(a.chore_id)binds.push(Number(a.chore_id));
  return (await db.prepare(`SELECT ${select},COUNT(*) value FROM family_logs l LEFT JOIN family_quick_chores c ON c.id=l.quick_chore_id AND c.family_id=l.family_id LEFT JOIN members m ON m.id=l.created_by AND m.family_id=l.family_id WHERE l.family_id=? AND l.log_type='HOUSEWORK' AND l.deleted_at IS NULL AND substr(l.occurred_at,1,10) BETWEEN ? AND ? ${mid?'AND l.created_by=?':''} ${a.chore_id?'AND l.quick_chore_id=?':''} GROUP BY label ORDER BY value DESC`).bind(...binds).all<Row>()).results;
 }
...
function answer(name:string,rows:Row[],args:Args){const period=`${String(args.date_from||'全期間')}〜${String(args.date_to||'現在')}`,target=args.subject_ref||args.member_ref?'指定対象':'家族全体',method=name==='daily_family_log_aggregate'?'日単位集計':name==='quick_chore_stats'?'ちょこっと家事件数':name==='schedule_lookup'?'共有予定検索':name==='task_stats'?'共有タスク集計':'最新記録';const provenance=`対象: ${target} / 期間: ${period} / 集計方法: ${method}`;if(!rows.length)return `該当する記録はありません。\n${provenance}`;if(name==='schedule_lookup')return rows.map(r=>`${String(r.start_at||r.due_at).slice(0,16)} ${r.title}`).join('\n')+`\n${provenance}`;const unit=name==='quick_chore_stats'||name==='task_stats'||args.metric==='COUNT'?'件':args.metric==='SUM_DURATION'?'分':args.log_type==='MILK'?'ml':'';const result=rows.length===1&&'value' in rows[0]?`${rows[0].day?`${rows[0].day}: `:''}${rows[0].value}${unit}`:rows.map(r=>`${r.day||r.label||String(r.occurred_at||'').slice(0,16)}: ${r.value??r.amount??r.value_text??''}${'value' in r?unit:''}`).join('\n');return `${result}\n${provenance}`;}
...
export async function familyAiConnectionTest(request:Request,ctx:AppContext){
 if(!ctx.member)return json({ok:false},401);if(!['OWNER','ADMIN'].includes(String(ctx.member.role||'').toUpperCase()))return json({ok:false},403);if(request.method!=='POST')return json({ok:false},405);const body=await request.json().catch(()=>({})) as any;if(String(body.csrf||'')!==String(ctx.session.csrfToken||''))return json({ok:false},403);if(!ctx.env.GEMINI_API_KEY)return json({ok:false,error:'not configured'},503);
 try{const model=ctx.env.GEMINI_MODEL||GEMINI_MODEL_DEFAULT;const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(ctx.env.GEMINI_API_KEY)}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({contents:[{role:'user',parts:[{text:'Reply with OK. This is a synthetic connectivity test; no user data is included.'}]}],generationConfig:{maxOutputTokens:8}})});return json({ok:r.ok},r.ok?200:503);}catch{return json({ok:false},503);}
}
