...
export async function settingsNotifications(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const pushDiagnosticSubscription=isAdmin?await ctx.env.DB.prepare('SELECT endpoint FROM web_push_subscriptions WHERE member_id=? AND family_id=? AND enabled=1 ORDER BY id DESC LIMIT 1').bind(m.id,m.family_id).first<Row>():null;
  ...
  const pushDiagnostic=isAdmin?await diagnoseWebPushVapid(ctx.env,String(pushDiagnosticSubscription?.endpoint||'')):null;
