...
function renderTaskEventsPage(ctx:AppContext,date:string,data:TaskEventsData,unorganized:Row[]):string{
  ...
  const shoppingSection=`<div class="card section-card shopping-checklist-section" id="shopping-checklist"><div class="section-head"><div><h2>🛒 買い物</h2><div class="meta">通常タスクは関連日から期限まで、定期タスクは期限日に表示</div></div><div><a class="btn small" href="/app/shopping_new.php?date=${encodeURIComponent(date)}">＋ 追加</a> <a class="btn small secondary" href="/app/shopping.php">一覧・管理</a></div></div>${shoppingRows(data.shopping)||'<p class="empty">対象日の買い物はありません。</p>'}</div>`;
