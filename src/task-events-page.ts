...
function renderTaskEventsPage(ctx:AppContext,date:string,data:TaskEventsData,unorganized:Row[]):string{
  ...
  const todayDate=dateOnly();const tomorrowCursor=new Date(`${todayDate}T12:00:00Z`);tomorrowCursor.setUTCDate(tomorrowCursor.getUTCDate()+1);const tomorrowDate=tomorrowCursor.toISOString().slice(0,10);
  ...
  const tabs=`<div class="task-event-tabs"><a class="${date===todayDate?'active':''}" href="/app/tasks.php?date=${todayDate}">今日</a><a class="${date===tomorrowDate?'active':''}" href="/app/tasks.php?date=${tomorrowDate}">明日</a><a href="/app/calendar.php?month=${encodeURIComponent(date.slice(0,7))}&date=${encodeURIComponent(date)}">カレンダー</a></div>`;
  const summary=`<div class="task-event-summary meta">タスク・定期 ${checkableTaskCount}件${eventCount?` ・ イベント ${eventCount}件`:''} ・ 買い物 ${data.shopping.length}件</div>`;
