...
.checklist-page .checklist-add{display:flex;min-height:44px;margin:0 0 12px}
...
.checklist-page .checklist-more .meta{font-size:13px;line-height:1.5;margin:8px 0 0}
  ...
  const body=`${checklistStyle}<div class="checklist-page"><div class="daily-head"><div><h1>✅ チェックリスト</h1><div class="date-title">${esc(date)}</div>${summary}</div><div class="date-nav"><a class="btn gray" aria-label="前日を表示" href="/app/tasks.php?date=${prev}">‹</a><a class="btn gray" aria-label="翌日を表示" href="/app/tasks.php?date=${next}">›</a></div></div><a class="btn checklist-add" href="/task/new.php?date=${encodeURIComponent(date)}&return=tasks">✨ まとめて追加（AI入力）</a><div class="card section-card task-section"><div class="section-head"><h2>📝 タスク・イベント</h2><a class="btn small gray" aria-label="タスク・イベントを追加" href="/task/new.php?date=${encodeURIComponent(date)}&return=tasks">＋ 追加</a></div>${taskRows||'<p class="empty">対象日のタスク・イベントはありません。</p>'}</div>${shoppingSection}${expiredShoppingHtml}${unorganizedHtml}${expiredHtml}<div class="card section-card item-section"><div class="section-head"><h2>🎒 持ち物</h2><a class="btn small gray" aria-label="持ち物を追加" href="/item/new.php?date=${encodeURIComponent(date)}">＋ 追加</a></div>${itemRows||'<p class="empty">対象日の持ち物はありません。</p>'}</div></div><script type="application/json" id="dailyPayload">${JSON.stringify({csrf}).replaceAll('<','\\u003c').replaceAll('>','\\u003e').replaceAll('&','\\u0026')}</script><script src="/assets/task-events.js?v=${APP_VERSION}"></script><script src="/assets/occurrence-family-log.js?v=${APP_VERSION}"></script>`;
