...
export class FamilyLogPageBadRequest extends Error {}
...
export function normalizeMilkAmountPresets(value:unknown):number[]{
  if(!Array.isArray(value)||value.length<1||value.length>6)throw new FamilyLogPageBadRequest('ミルク量の候補は1〜6件で指定してください。');
  const values=value.map(Number);
  if(values.some(v=>!Number.isInteger(v)||v<1||v>2000))throw new FamilyLogPageBadRequest('ミルク量は1〜2000mlの整数で指定してください。');
