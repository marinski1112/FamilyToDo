...
export const DIAGNOSTIC_DEFINITIONS:DiagnosticDefinition[]=[
  ...
  {key:'private_integrity',label:'PRIVATEデータの整合性',description:'所有者・通知先の不整合',sql:"SELECT (SELECT COUNT(*) FROM tasks t WHERE t.family_id=?1 AND ((t.visibility_scope='PRIVATE' AND (t.private_owner_id IS NULL OR NOT EXISTS(SELECT 1 FROM members m WHERE m.id=t.private_owner_id AND m.family_id=t.family_id AND m.active=1 AND m.deleted_at IS NULL))) OR (t.visibility_scope='FAMILY' AND t.private_owner_id IS NOT NULL)))+(SELECT COUNT(*) FROM items i WHERE i.family_id=?1 AND ((i.visibility_scope='PRIVATE' AND (i.private_owner_id IS NULL OR NOT EXISTS(SELECT 1 FROM members m WHERE m.id=i.private_owner_id AND m.family_id=i.family_id AND m.active=1 AND m.deleted_at IS NULL))) OR (i.visibility_scope='FAMILY' AND i.private_owner_id IS NOT NULL)))+(SELECT COUNT(*) FROM shopping_items s WHERE s.family_id=?1 AND ((s.visibility_scope='PRIVATE' AND (s.private_owner_id IS NULL OR NOT EXISTS(SELECT 1 FROM members m WHERE m.id=s.private_owner_id AND m.family_id=s.family_id AND m.active=1 AND m.deleted_at IS NULL))) OR (s.visibility_scope='FAMILY' AND s.private_owner_id IS NOT NULL)))+(SELECT COUNT(*) FROM notifications n JOIN tasks t ON n.target_type='task' AND t.id=n.target_id AND t.family_id=n.family_id WHERE n.family_id=?1 AND n.status IN ('pending','retry') AND t.visibility_scope='PRIVATE' AND n.member_id<>t.private_owner_id) c"},
...
export async function settingsDiagnosticsDetail(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(issue==='ai_generation'){
    ...
    return json({ok:true,issue,items:rows.results.map(x=>{
      ...
      const boundedInt=(value:unknown,min:number,max:number)=>{if(value===null||value===undefined)return null;const n=Number(value);return Number.isInteger(n)&&n>=min&&n<=max?n:null;};
      let attempts:Array<{ordinal:number;model:string;status:string;http_status:number|null;reason_code:string|null;failure_stage:string|null;item_ordinal:number|null;source_index:number|null;expected_count:number|null;actual_count:number|null}>=[];
      try{
        ...
        if(Array.isArray(parsed))attempts=parsed.slice(0,4).flatMap((attempt,index)=>{
          ...
          const itemOrdinal=boundedInt(row.itemOrdinal,1,20),sourceIndex=boundedInt(row.sourceIndex,0,19),expectedCount=boundedInt(row.expectedCount,0,100),actualCount=boundedInt(row.actualCount,0,100);
          return [{ordinal:index+1,model,status,http_status:httpStatus,reason_code:reasonCode,failure_stage:failureStage,item_ordinal:itemOrdinal,source_index:sourceIndex,expected_count:expectedCount,actual_count:actualCount}];
        });
      ...
      return {feature:String(x.feature||''),final_status:String(x.final_status||''),ai_called:Number(x.ai_called||0)===1,model:acceptedModel||lastModel,http_status:lastAttempt?.http_status??null,last_attempt_status:lastAttempt?.status??null,last_reason_code:lastAttempt?.reason_code??null,last_failure_stage:lastAttempt?.failure_stage??null,last_item_ordinal:lastAttempt?.item_ordinal??null,last_source_index:lastAttempt?.source_index??null,last_expected_count:lastAttempt?.expected_count??null,last_actual_count:lastAttempt?.actual_count??null,attempt_count:Number(x.attempt_count||0),attempts,item_count:x.item_count==null?null:Number(x.item_count),created_at:String(x.created_at||'')};
    }),limited:20});
  }
