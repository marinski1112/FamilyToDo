...
function strictAiProfilePermissions(value:unknown):AiProfilePermissionKey[]{
  ...
  const allow=new Set<string>(AI_PROFILE_PERMISSION_KEYS),seen=new Set<string>();
  for(const raw of value){const key=String(raw);if(!allow.has(key))throw new Error('AI利用項目の設定が不正です。');seen.add(key);}
  return AI_PROFILE_PERMISSION_KEYS.filter(key=>seen.has(key));
}

function storedAiProfilePermissions(value:unknown):Set<AiProfilePermissionKey>{
  try{const parsed=JSON.parse(String(value||'[]'));if(!Array.isArray(parsed))return new Set();const values=new Set(parsed.map(String));return new Set(AI_PROFILE_PERMISSION_KEYS.filter(key=>values.has(key)));}catch{return new Set();}
}
const canonicalAiProfilePermissions=(value:unknown)=>JSON.stringify(AI_PROFILE_PERMISSION_KEYS.filter(key=>storedAiProfilePermissions(value).has(key)));
...
export async function settingsMembers(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(request.method==='POST'){
    ...
    const hasPermissionSnapshot=Object.prototype.hasOwnProperty.call(b,'ai_profile_permissions');
    let birthDate:string|null,sexGender:string|null,birthplace:string|null,bloodType:string|null,personalityNote:string|null,aiEnabled:0|1,requestedPermissions:AiProfilePermissionKey[]|null=null,expectedPermissions:AiProfilePermissionKey[]|null=null;
    try{
      ...
      if(hasPermissionSnapshot){
        requestedPermissions=strictAiProfilePermissions(b.ai_profile_permissions);
        expectedPermissions=strictAiProfilePermissions(b.ai_profile_permissions_original);
      }
    ...
    return json({ok:true,id:Number(subject.id),ai_profile_permissions:JSON.parse(permissionsJson)});
  }
