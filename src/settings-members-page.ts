...
export async function settingsMembers(request:Request,ctx:AppContext):Promise<Response>{
  ...
  if(request.method==='POST'){
    ...
    let birthDate:string|null,sexGender:string|null,birthplace:string|null,bloodType:string|null,personalityNote:string|null,aiEnabled:0|1;
    try{
    ...
    let subject=await ctx.env.DB.prepare('SELECT id,birth_date,sex_gender,birthplace,blood_type,personality_note,ai_personalization_enabled FROM family_log_subjects WHERE family_id=? AND member_id=? AND active=1 ORDER BY id LIMIT 1').bind(m.family_id,memberId).first<Row>();
    if(!subject){
      const now=nowJst();
      await ctx.env.DB.prepare(`INSERT INTO family_log_subjects(family_id,member_id,name,subject_kind,birth_date,icon,active,created_by,created_at,updated_at,enabled_types_json,auto_complete_linked_task,sex_gender,birthplace,blood_type,personality_note,ai_personalization_enabled)
      ...
      subject=await ctx.env.DB.prepare('SELECT id,birth_date,sex_gender,birthplace,blood_type,personality_note,ai_personalization_enabled FROM family_log_subjects WHERE family_id=? AND member_id=? AND active=1 ORDER BY id LIMIT 1').bind(m.family_id,memberId).first<Row>();
    }
    if(!subject)return json({ok:false,error:'プロフィール対象を作成できませんでした。',code:'CONFLICT'},409);
    const fields:[string,unknown,unknown][]=[
      ['birth_date',subject.birth_date,birthDate],['sex_gender',subject.sex_gender,sexGender],['birthplace',subject.birthplace,birthplace],['blood_type',subject.blood_type,bloodType],['personality_note',subject.personality_note,personalityNote],['ai_personalization_enabled',Number(subject.ai_personalization_enabled||0),aiEnabled],
    ];
    const changedFields=fields.filter(([,before,after])=>String(before??'')!==String(after??'')).map(([name])=>name);
    await ctx.env.DB.prepare('UPDATE family_log_subjects SET birth_date=?,sex_gender=?,birthplace=?,blood_type=?,personality_note=?,ai_personalization_enabled=?,updated_at=? WHERE id=? AND family_id=? AND member_id=? AND active=1').bind(birthDate,sexGender,birthplace,bloodType,personalityNote,aiEnabled,nowJst(),Number(subject.id),m.family_id,memberId).run();
  ...
  const [members,invitations]=await Promise.all([
    ctx.env.DB.prepare(`SELECT m.id,m.name,m.member_type,m.role,m.active,m.deleted_at,m.created_at,
      ...
      s.id profile_subject_id,s.birth_date,s.sex_gender,s.birthplace,s.blood_type,s.personality_note,s.ai_personalization_enabled
      FROM members m
