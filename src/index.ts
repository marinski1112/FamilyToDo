...
async function dbSchemaHealth(env:Env):Promise<Response>{
  const required:Record<string,string[]>= {
    ...
    family_logs:['id','family_id','subject_id','log_type','occurred_at','duration_minutes','linked_task_id','linked_occurrence_id','quick_chore_id','deleted_at'],
...
async function dbRuntimeHealth(env:Env):Promise<Response>{
  const checks:[string,string][]=[
    ...
    ['family_logs','SELECT id,family_id,subject_id,log_type,occurred_at,detail_code,amount,unit,duration_minutes,value_text,note,linked_task_id,linked_occurrence_id,quick_chore_id,created_by,created_at,updated_at,deleted_at FROM family_logs LIMIT 1'],
...
async function taskApi(request:Request,ctx:any):Promise<Response>{
  ...
  let id=0;
  try {
    const r=await ctx.env.DB.prepare('INSERT INTO tasks(family_id,title,description,due_at,status,completion_mode,created_by,created_at,updated_at,start_at,end_at,location,all_day,calendar_visible,calendar_color,task_kind,sort_order,reminder_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(m.family_id,title,String(b.description??'')||null,dueValue,'pending',completionMode,m.id,now,now,start,end,String(b.location??'')||null,allDay?1:0,calendarVisibleFlag(b),calendarColor,isEvent?'EVENT':'TASK',0,reminderAt).run();
