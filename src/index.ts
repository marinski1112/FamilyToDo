...
async function itemNew(ctx:any,date:string,selectedTaskId=0):Promise<Response>{
  ...
  const [members,tasks]=await Promise.all([
    ctx.env.DB.prepare('SELECT id,name FROM members WHERE family_id=? AND active=1 ORDER BY id').bind(ctx.member.family_id).all(),
    ctx.env.DB.prepare(`SELECT id,title,start_at,due_at FROM tasks t WHERE family_id=? AND status<>'completed' AND ${taskVisibilitySql('t')} ORDER BY coalesce(start_at,due_at),id LIMIT 200`).bind(ctx.member.family_id,ctx.member.id).all()
  ]);
...
async function itemApi(request:Request,ctx:any):Promise<Response>{
  ...
  if(taskId){const t=await ctx.env.DB.prepare(`SELECT id,start_at,end_at,due_at FROM tasks t WHERE id=? AND family_id=? AND ${taskVisibilitySql('t')}`).bind(taskId,m.family_id,m.id).first();if(!t)return json({ok:false,error:'関連タスクが見つかりません。'},400);dueDate=String(t.start_at||t.due_at||'').slice(0,10)||dueDate;}
