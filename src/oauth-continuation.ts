...
export async function lineGoogleHomeCallback(r:Request,e:Env){
  ...
  const linePictureUrl=safeLinePictureUrl(verified.picture);
  if(linePictureUrl)await e.DB.prepare('UPDATE members SET line_picture_url=? WHERE id=? AND family_id=? AND line_user_id=?').bind(linePictureUrl,member.id,member.family_id,verified.sub).run();
  const session={iat:Date.now(),lineUserId:verified.sub,lineDisplayName:verified.name||member.name,linePictureUrl:linePictureUrl||undefined,memberId:Number(member.id),familyId:Number(member.family_id),csrfToken:crypto.randomUUID()};
