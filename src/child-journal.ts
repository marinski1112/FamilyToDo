...
export async function childJournalApi(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const subjectId=Number(form.get('subject_id')||0);const occurredDate=String(form.get('occurred_date')||'');const kind=String(form.get('kind')||'').toUpperCase() as JournalInputKind;const note=String(form.get('note')||'').trim().slice(0,2000);const rawValue=String(form.get('value')||'').trim();const title=String(form.get('title')||'').trim().slice(0,120);
...
export async function childJournalPage(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const subjects=await ctx.env.DB.prepare("SELECT id,name,subject_kind,icon FROM family_log_subjects WHERE family_id=? AND active=1 AND subject_kind IN ('BABY','CHILD') ORDER BY id").bind(member.family_id).all<Row>();
  ...
  const form=selected?`<div class="card"><div class="section-head"><h2>＋ 成長を記録</h2></div><form method="post" action="/api/child-journal"><input type="hidden" name="csrf" value="${esc(csrf)}"><input type="hidden" name="kind" value="MEMO"><label>子ども</label><select name="subject_id" required>${subjectOptions}</select><label>日付</label><input type="date" name="occurred_date" value="${today}" required><label>タイトル</label><input type="text" name="title" maxlength="120" required placeholder="例：はじめて完食した"><label>メモ</label><textarea name="note" maxlength="2000" rows="3" placeholder="できたこと、様子など（任意）"></textarea><button type="submit">記録する</button></form></div>`:`<div class="card"><h2>成長日記を始める</h2><p>家族ログ管理で「赤ちゃん」または「子ども」の記録対象を追加してください。</p><a class="btn" href="/app/settings_family_log.php">家族ログ管理を開く</a></div>`;
  const photoEntries=[...entries.results].reverse().map(row=>{const date=String(row.occurred_at||'').slice(0,10);return `<article class="journal-photo-row" data-mitenya-log="${Number(row.log_id)}"><time datetime="${esc(date)}"><strong>${Number(date.slice(8,10))}</strong><small>${['日','月','火','水','木','金','土'][new Date(`${date}T12:00:00Z`).getUTCDay()]}</small></time><div class="journal-entry-text"><strong>${esc(journalLabel(row))}</strong>${row.note&&String(row.note)!==String(row.value_text||'')?`<p>${esc(row.note)}</p>`:''}</div><button type="button" class="btn gray journal-entry-photo" data-journal-photo="${Number(row.log_id)}" aria-label="${esc(date)} ${esc(journalLabel(row))}の写真を追加・確認">📷</button></article>`;}).join('');
  const photoPanel=selected?`<section class="card child-journal-list" aria-label="成長日記の一覧">${photoEntries||'<p class="small">この月の日記はまだありません。「成長を記録」から追加できます。</p>'}</section><div class="card" id="journalPhotoPanel" hidden><form id="familyLogForm"><h2 id="journalPhotoTitle">写真を添付</h2><input type="hidden" name="id" value="0"><input type="hidden" name="subject_id" value="${subjectId}"><input type="hidden" name="log_type" value="MEMO"><input type="hidden" name="detail_code" value="JOURNAL_MEMO"><div id="familyLogAdvanced" hidden></div><button type="submit">保存する</button> <button type="button" class="btn gray" id="journalPhotoClose">閉じる</button></form></div><script type="application/json" id="familyLogPayload">${JSON.stringify({csrf,journalPhotos:true,journalLogIds:entries.results.map(row=>Number(row.log_id)),logs:{}}).replaceAll('<','\\u003c')}</script><script src="/assets/family-log-baby-food-media.js?v=journal-photo1" defer></script><script src="/assets/child-journal-photo.js?v=journal-list1" defer></script><script src="/assets/mitenya-photo-share.js?v=1" defer></script>`:'';
  const body=`<style>.journal-photo-row{display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid #e5e7eb}.journal-photo-row>time{flex:0 0 38px;text-align:center}.journal-photo-row>time strong{font-size:24px}.journal-photo-row>time small{display:block;font-size:11px}.journal-entry-text p{margin:4px 0 0;white-space:pre-wrap;font-size:14px}.journal-entry-photo{width:60px;height:60px;padding:2px!important;overflow:hidden}.journal-entry-photo img{display:block;width:100%;height:100%;object-fit:cover;border-radius:8px}.child-journal-list{padding:8px 12px!important}.child-journal-list .journal-photo-row:first-child{border-top:0}.child-journal-fold>summary{cursor:pointer;font-weight:700;padding:8px}.journal-photo-row>div{flex:1;min-width:0;overflow-wrap:anywhere}.journal-photo-row button{min-width:44px;min-height:44px;flex-shrink:0}#journalPhotoPanel[hidden]{display:none!important}.child-journal-head{display:flex;align-items:center;justify-content:space-between;gap:12px}.child-journal-subjects{display:flex;gap:8px;overflow:auto;padding:4px 0 12px}.child-journal-subjects a{white-space:nowrap;padding:8px 12px;border-radius:999px;background:#fff;text-decoration:none}.child-journal-subjects a.active{font-weight:700;box-shadow:0 0 0 2px currentColor inset}.child-journal-month-nav{display:flex;justify-content:space-between;align-items:center;margin:10px 0}.child-journal-calendar{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:4px}.child-journal-weekday{text-align:center;font-size:12px;font-weight:700;padding:4px}.child-journal-day{min-height:88px;border:1px solid #e5e7eb;border-radius:8px;padding:5px;background:#fff;overflow:hidden}.child-journal-day.today{box-shadow:0 0 0 2px currentColor inset}.child-journal-day.muted{background:transparent;border-color:transparent}.child-journal-entry{margin-top:4px;padding:4px;border-radius:6px;background:#f3f4f6;font-size:11px;line-height:1.25}.child-journal-entry span,.child-journal-entry small{display:block;overflow-wrap:anywhere}@media(max-width:640px){.child-journal-day{min-height:70px;padding:3px}.child-journal-entry{font-size:10px}}</style><div class="child-journal-head"><div><div class="eyebrow">Family Log</div><h1>📔 成長日記</h1></div><a class="btn gray" href="/app/family_log.php">家族ログへ</a></div><div class="child-journal-subjects">${chips}</div><div class="card"><div class="child-journal-month-nav"><a class="btn gray" aria-label="前の月" title="前の月" href="/app/child_journal.php?month=${shiftMonth(month,-1)}${subjectId?`&subject_id=${subjectId}`:''}">‹</a><strong>${esc(month.replace('-','年'))}月</strong><a class="btn gray" aria-label="次の月" title="次の月" href="/app/child_journal.php?month=${shiftMonth(month,1)}${subjectId?`&subject_id=${subjectId}`:''}">›</a></div><details class="child-journal-fold"><summary>カレンダーで見る</summary><div class="child-journal-calendar">${['日','月','火','水','木','金','土'].map(x=>`<div class="child-journal-weekday">${x}</div>`).join('')}${cells.join('')}</div></details></div>${photoPanel}<details class="card child-journal-fold"><summary>＋ 成長を記録</summary>${form}</details>`;
