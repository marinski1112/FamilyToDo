...
export async function recurringForRange(ctx:AppContext,from:string,to:string):Promise<Row[]>{
  ...
  return recurringForFamilyRange(ctx.env.DB,member.family_id,member.id,from,to);
}

/** Scheduled family summaries use memberId=0 for FAMILY-only visibility. */
export async function recurringForFamilyRange(db:D1Database,familyId:number,memberId:number,from:string,to:string):Promise<Row[]>{
  if(!Number.isSafeInteger(familyId)||familyId<=0||!Number.isSafeInteger(memberId)||memberId<0)throw new Error('invalid recurrence scope');
  const fid=familyId,overlapSql=recurrenceOverlapSql('r','t');
  const rules=await db.prepare(`SELECT r.*,t.title,t.description,t.start_at,t.end_at,t.location,t.all_day,t.calendar_visible,t.completion_mode,t.calendar_color,t.visibility_scope,t.private_owner_id,t.task_kind,(SELECT id FROM task_family_log_templates ft WHERE ft.task_id=t.id AND ft.family_id=t.family_id AND ft.active=1 LIMIT 1) family_log_template_id FROM recurrence_rules r JOIN tasks t ON t.id=r.task_id WHERE r.family_id=? AND ${taskVisibilitySql('t')} AND r.active=1 AND r.start_date<=? AND ${overlapSql} ORDER BY r.id`).bind(fid,memberId,to,from).all<Row>();
  ...
  const occurrenceFrom=projected.reduce((earliest,current)=>current.date<earliest?current.date:earliest,from),projectedRuleIds=[...new Set(projected.map(({rule})=>Number(rule.id)).filter(Number.isInteger))],projectedRulePlaceholders=projectedRuleIds.map(()=>'?').join(','),projectedTaskIds=[...new Set(projected.map(({rule})=>Number(rule.task_id)).filter(Number.isInteger))],projectedTaskPlaceholders=projectedTaskIds.map(()=>'?').join(',');
  const loadOccurrences=()=>db.prepare(`SELECT * FROM recurrence_occurrences WHERE family_id=? AND recurrence_rule_id IN (${projectedRulePlaceholders}) AND occurrence_date BETWEEN ? AND ?`).bind(fid,...projectedRuleIds,occurrenceFrom,to).all<Row>();
  ...
  if(missing.length){const statements=missing.map(({rule,date})=>db.prepare('INSERT OR IGNORE INTO recurrence_occurrences(family_id,recurrence_rule_id,occurrence_date,status,created_at,updated_at) VALUES(?,?,?,?,?,?)').bind(fid,Number(rule.id),date,'pending',now,now));for(let i=0;i<statements.length;i+=50){const chunk=statements.slice(i,i+50);if(chunk.length)await db.batch(chunk);}occurrenceRows=await loadOccurrences();occurrenceByKey=new Map(occurrenceRows.results.map(o=>[`${Number(o.recurrence_rule_id)}:${String(o.occurrence_date)}`,o]));}
  const [assigneeRows,completionRows]=await Promise.all([
    db.prepare(`SELECT ta.task_id,GROUP_CONCAT(am.name,'、') assignees,GROUP_CONCAT(ta.member_id,',') assignee_ids,COUNT(am.id) assigned_count FROM task_assignees ta JOIN members am ON am.id=ta.member_id AND am.active=1 WHERE ta.task_id IN (${projectedTaskPlaceholders}) GROUP BY ta.task_id`).bind(...projectedTaskIds).all<Row>(),
    db.prepare(`SELECT c.occurrence_id,COUNT(*) c,MAX(c.completed_at) completed_at FROM recurrence_occurrence_completions c JOIN recurrence_occurrences o ON o.id=c.occurrence_id JOIN recurrence_rules rr ON rr.id=o.recurrence_rule_id JOIN members cm ON cm.id=c.member_id AND cm.family_id=o.family_id AND cm.active=1 WHERE o.family_id=? AND o.recurrence_rule_id IN (${projectedRulePlaceholders}) AND o.occurrence_date BETWEEN ? AND ? AND (NOT EXISTS(SELECT 1 FROM task_assignees ta0 JOIN members am0 ON am0.id=ta0.member_id AND am0.active=1 WHERE ta0.task_id=rr.task_id) OR EXISTS(SELECT 1 FROM task_assignees ta1 JOIN members am1 ON am1.id=ta1.member_id AND am1.active=1 WHERE ta1.task_id=rr.task_id AND ta1.member_id=c.member_id)) GROUP BY c.occurrence_id`).bind(fid,...projectedRuleIds,occurrenceFrom,to).all<Row>()
  ]);
