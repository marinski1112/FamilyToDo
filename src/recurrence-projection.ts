...
export async function recurringForRange(ctx:AppContext,from:string,to:string):Promise<Row[]>{
  ...
  const loadOccurrences=()=>ctx.env.DB.prepare(`SELECT * FROM recurrence_occurrences WHERE family_id=? AND recurrence_rule_id IN (${projectedRulePlaceholders}) AND occurrence_date BETWEEN ? AND ?`).bind(fid,...projectedRuleIds,occurrenceFrom,to).all<Row>();
