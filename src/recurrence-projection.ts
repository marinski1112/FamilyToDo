...
export async function recurringForFamilyRange(db:D1Database,familyId:number,memberId:number,from:string,to:string):Promise<Row[]>{
  ...
  const occurrenceFrom=projected.reduce((earliest,current)=>current.date<earliest?current.date:earliest,from),projectedRuleIds=[...new Set(projected.map(({rule})=>Number(rule.id)).filter(Number.isInteger))],projectedRulePlaceholders=projectedRuleIds.map(()=>'?').join(',');
  ...
  const completionRows=await db.prepare(`SELECT c.occurrence_id,COUNT(*) c,MAX(c.completed_at) completed_at
    FROM recurrence_occurrence_completions c
    JOIN recurrence_occurrences o ON o.id=c.occurrence_id
    JOIN members cm ON cm.id=c.member_id AND cm.family_id=o.family_id AND cm.active=1
    WHERE o.family_id=? AND o.recurrence_rule_id IN (${projectedRulePlaceholders})
      AND o.occurrence_date BETWEEN ? AND ? GROUP BY c.occurrence_id`)
    .bind(fid,...projectedRuleIds,occurrenceFrom,to).all<Row>();
  const completedByOccurrence=new Map(completionRows.results.map(r=>[Number(r.occurrence_id),Number(r.c||0)])),out:Row[]=[];
  for(const {rule,date} of projected){const occ=occurrenceByKey.get(`${Number(rule.id)}:${date}`);if(!occ||String(occ.status||'').toLowerCase()==='excluded'||occ.exception_task_id)continue;const isCompleted=(completedByOccurrence.get(Number(occ.id))||0)>0,baseTime=String(rule.start_at||'').slice(11,19),endTime=String(rule.end_at||'').slice(11,19),endDate=shiftCalendarDateOnly(date,calendarRangeOffsetDays(rule.start_at,rule.end_at));out.push({...rule,id:-Number(occ.id),recurrence_occurrence_id:Number(occ.id),recurrence_rule_id:Number(rule.id),occurrence_date:date,status:isCompleted?'completed':'pending',completed_at:isCompleted?completionRows.results.find(c=>Number(c.occurrence_id)===Number(occ.id))?.completed_at:null,due_at:`${date} ${baseTime||'00:00:00'}`,start_at:baseTime?`${date} ${baseTime}`:null,end_at:endTime?`${endDate} ${endTime}`:null});}
  return out;
}
