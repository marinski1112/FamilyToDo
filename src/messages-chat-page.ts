...
export async function messagesChatPage(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const hasOlder=rows.results.length>40,visible=rows.results.slice(0,40).reverse(),oldest=visible.length?Number(visible[0].id):0;
  let previousDay='';
  const messages=visible.map(r=>{
    ...
    return `${separator}<article class="chat-message ${mine?'mine':'theirs'}" data-message-id="${Number(r.id)}" data-message-type="text" data-text="${esc(r.text)}" data-mine="${mine?'1':'0'}">${mine?'':`<div class="chat-avatar" aria-hidden="true">${esc(String(r.sender_name||'?').slice(0,1))}</div>`}<div class="chat-stack">${mine?'':`<div class="chat-sender">${esc(r.sender_name)}</div>`}<div class="chat-bubble">${esc(r.text)}</div><div class="chat-meta">${scheduled?`<span class="chat-scheduled">予約 ${esc(String(r.reminder_at).slice(5,16))}</span> ・ `:''}${esc(timeLabel(r.created_at))}</div></div></article>`;
  }).join('');
  const payload=JSON.stringify({csrf:ctx.session.csrfToken||'',memberId:m.id,now}).replaceAll('<','\\u003c').replaceAll('>','\\u003e').replaceAll('&','\\u0026');
  const body=`<link rel="stylesheet" href="/assets/messages-chat.css?v=${APP_VERSION}-chat1"><main class="messages-chat-page"><header class="messages-chat-head"><h1>家族</h1><div class="sub">家族グループ</div></header>${hasOlder&&oldest?`<a class="chat-archive-link" href="/app/messages.php?before=${oldest}">以前のメッセージ</a>`:''}<section id="chatMessages">${messages||'<div class="chat-empty">まだメッセージはありません</div>'}</section></main>
  ...
  <div class="chat-tools" id="chatTools"><div class="chat-tools-grid"><button type="button" id="chatStamp">スタンプ</button><button type="button" id="chatSchedule">送信予約</button><button type="button" id="chatImage">画像</button></div><div class="chat-schedule-row" id="chatScheduleRow"><input type="datetime-local" id="chatScheduleAt"><div class="chat-image-note">予約時刻までは他の家族には表示されません</div></div><div class="chat-image-note" id="chatImageNote" hidden>画像送信基盤は準備中です。長押しメニューの「みてにゃに追加」は先行してUIのみ実装します。</div></div>
  ...
  <script type="application/json" id="messagesChatPayload">${payload}</script><script src="/assets/messages-chat.js?v=${APP_VERSION}-chat1"></script>`;
