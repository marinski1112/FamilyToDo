...
export async function messagesChatPage(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const rows=before?await ctx.env.DB.prepare(query).bind(m.family_id,before,now,m.id).all<Row>():await ctx.env.DB.prepare(query).bind(m.family_id,now,m.id).all<Row>();
  ...
  let previousDay='';
  const messages=visible.map(r=>{const day=dayLabel(r.created_at),separator=day&&day!==previousDay?`<div class="chat-day"><span>${esc(day)}</span></div>`:'';previousDay=day;const mine=Number(r.sender_id)===Number(m.id),scheduled=String(r.reminder_at||'')>now;return `${separator}<article class="chat-message ${mine?'mine':'theirs'}" data-message-id="${Number(r.id)}" data-message-type="${r.image_upload_id?'image':'text'}" data-text="${esc(r.text)}" data-updated-at="${esc(r.updated_at)}" data-mine="${mine?'1':'0'}">${mine?'':`<div class="chat-avatar" aria-hidden="true">${esc(String(r.sender_name||'?').slice(0,1))}</div>`}<div class="chat-stack">${mine?'':`<div class="chat-sender">${esc(r.sender_name)}</div>`}<div class="chat-bubble">${r.image_upload_id?`<img class="chat-photo" src="/api/messages?photo=${Number(r.id)}" alt="伝言の写真" loading="lazy" decoding="async" data-photo-share="1">`:''}${esc(r.text)}</div><div class="chat-meta">${scheduled?`<span class="chat-scheduled">予約 ${esc(String(r.reminder_at).slice(5,16))}</span> ・ `:''}${esc(timeLabel(r.created_at))}</div></div></article>`;}).join('');
  ...
  const body=`<script src="/assets/message-photo-upload.js?v=${APP_VERSION}-photo3" defer></script><script src="/assets/mitenya-photo-share.js?v=1" defer></script><link rel="stylesheet" href="/assets/messages-chat.css?v=${APP_VERSION}-chat8"><main class="messages-chat-page"><header class="messages-chat-head"><h1>家族</h1><div class="sub">家族グループ</div></header>${archive}${older}<section id="chatMessages" class="${initialClass.trim()}">${messages||'<div class="chat-empty">まだメッセージはありません</div>'}</section>${back}</main>
