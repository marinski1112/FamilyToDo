...
export async function messagesChatPage(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const body=`<link rel="stylesheet" href="/assets/messages-chat.css?v=${APP_VERSION}-chat2"><main class="messages-chat-page"><header class="messages-chat-head"><h1>家族</h1><div class="sub">家族グループ</div></header>${archive}${older}<section id="chatMessages">${messages||'<div class="chat-empty">まだメッセージはありません</div>'}</section>${back}</main>
  ...
  <script type="application/json" id="messagesChatPayload">${payload}</script><script src="/assets/messages-chat.js?v=${APP_VERSION}-chat2"></script>`;
