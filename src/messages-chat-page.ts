...
export async function messagesChatPage(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const canDeleteAny=['OWNER','ADMIN'].includes(String(m.role||'').toUpperCase());
  ...
  const body=`<script src="/assets/message-photo-upload.js?v=1" defer></script><script src="/assets/mitenya-photo-share.js?v=1" defer></script><link rel="stylesheet" href="/assets/messages-chat.css?v=${APP_VERSION}-chat6"><main class="messages-chat-page"><header class="messages-chat-head"><h1>家族</h1><div class="sub">家族グループ</div></header>${archive}${older}<section id="chatMessages" class="${initialClass.trim()}">${messages||'<div class="chat-empty">まだメッセージはありません</div>'}</section>${back}</main>
