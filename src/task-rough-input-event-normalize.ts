...
function toIsoDate(value:string,referenceDate:string):string|null{
  const text=value.trim();
  let year:number|undefined,month:number|undefined,day:number|undefined;
  let match=text.match(/^(?:(\d{4})[\/.\-])?(\d{1,2})[\/.\-](\d{1,2})$/u);
  if(match){year=match[1]?Number(match[1]):undefined;month=Number(match[2]);day=Number(match[3]);}
  else{
    match=text.match(/^(\d{1,2})\s*月\s*(\d{1,2})\s*日$/u);
    if(match){month=Number(match[1]);day=Number(match[2]);}
  }
  if(!month||!day)return null;
  if(year===undefined){const reference=referenceDate.match(/^(\d{4})-\d{2}-\d{2}$/);if(!reference)return null;year=Number(reference[1]);}
...
export function normalizeEventDateTitleText(value:unknown,referenceDate=''):string{
  ...
  for(let index=0;index<lines.length;index++){
    ...
    const due=current.match(DUE_DATE_LINE);
    if(due&&DATE_ONLY_EVENT_LINE.test(due[2])){
      const normalizedDate=toIsoDate(due[2],referenceDate);
      if(normalizedDate&&normalizedDate!==due[2]){out.push(`${due[1]}${normalizedDate}`);changed=true;continue;}
    }
...
export function normalizeEventRoughInputBody(value:unknown,referenceDate=''):unknown{
  ...
  const fields=body.fields.map(field=>{
    ...
    const text=normalizeEventDateTitleText(row.text,referenceDate);
...
export async function normalizeEventRoughInputRequest(request:Request,referenceDate=''):Promise<Request>{
  ...
  const normalized=normalizeEventRoughInputBody(parsed,referenceDate);
