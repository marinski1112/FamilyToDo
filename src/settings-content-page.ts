...
export async function settingsContent(ctx:AppContext):Promise<Response>{
  ...
  const stampAdmin=admin?`<div class="card content-admin" id="calendarStampSequenceAdmin"><h2>🎞️ アニメーションスタンプ登録</h2><p class="small">配置済みの静的ASSETSにある連番PNGを、1つのアニメーションスタンプとして登録します。1行に <code>assets/stamps/example/001.png,120</code> の形式で2〜48フレームを入力してください。末尾の表示時間(ms)を省略すると120msです。</p><p class="small">現在はASSETS登録のみです。後からR2を作成した際は、アプリ管理メディア（UPLOAD）の保存先だけをR2へ接続する設計なので、ここでバケット名やR2 URLを入力する必要はありません。</p><form id="calendarStampSequenceForm"><input type="hidden" name="csrf" value="${esc(ctx.session.csrfToken||'')}"><label>スタンプ名</label><input name="name" maxlength="80" required placeholder="Happy Birthday"><label>PNGフレーム</label><textarea name="frames" rows="8" required placeholder="assets/stamps/birthday/001.png,120&#10;assets/stamps/birthday/002.png,120"></textarea><label>サムネイル（任意）</label><input name="thumbnailStorageKey" placeholder="assets/stamps/birthday/thumb.png"><div class="form-grid"><label>幅（任意）<input name="width" type="number" min="1" max="4096" inputmode="numeric"></label><label>高さ（任意）<input name="height" type="number" min="1" max="4096" inputmode="numeric"></label></div><button type="submit">スタンプを登録</button><p class="small" id="calendarStampSequenceStatus" role="status" aria-live="polite"></p></form></div><script defer src="/assets/settings-stamps.js"></script>`:'';
  const body=`<div class="page-head"><div><div class="eyebrow">管理</div><h1>📋 投稿管理</h1></div><a class="btn gray" href="/app/settings.php">戻る</a></div>${stampAdmin}${section('タスク','📝',tasks,r=>`/task/view.php?id=${r.id}`,r=>String(r.title||''))}${section('持ち物','🎒',items,r=>`/item/edit.php?id=${r.id}`,r=>String(r.name||''))}${section('買い物','🛒',shops,r=>`/app/shopping_edit.php?id=${r.id}`,r=>String(r.name||''))}${section('伝言','💬',msgs,r=>`/app/messages.php`,r=>String(r.text||''))}${section('家族ログ','🐣',familyLogs,r=>`/app/family_log.php?date=${String(r.occurred_at||'').slice(0,10)}`,r=>`${FAMILY_LOG_TYPE_META[String(r.log_type||'MEMO')]?.label||String(r.log_type||'記録')}${r.subject_name?' / '+String(r.subject_name):''}`)}`;
