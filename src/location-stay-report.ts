...
const STAY_WINDOW_MS=10*60*1000;
const STAY_WINDOW_RADIUS_METERS=50;
...
export function buildLocationStayReport(points:readonly LocationPoint[],places:readonly KnownLocationPlace[]):StayReportEntry[]{
  ...
  let stayAnchor:LocationPoint|undefined,stayAnchorAt=0;
  for(let i=1;i<points.length;i++){
    ...
    const bAt=Date.parse(b.recordedAt);
    if(!stayAnchor||!Number.isFinite(stayAnchorAt)||bAt-stayAnchorAt>STAY_WINDOW_MS||locationDistance(stayAnchor,b)>STAY_WINDOW_RADIUS_METERS){
      if(reliablePoint(a)){stayAnchor=a;stayAnchorAt=Date.parse(a.recordedAt);}
      else if(reliablePoint(b)){stayAnchor=b;stayAnchorAt=bAt;}
    }
    const windowStable=Boolean(stayAnchor&&reliablePoint(b)&&bAt-stayAnchorAt<=STAY_WINDOW_MS&&locationDistance(stayAnchor,b)<=STAY_WINDOW_RADIUS_METERS);
    ...
    if(duration>30*60000){kind='GAP';place='位置記録の空白';stayAnchor=undefined;stayAnchorAt=0;}
    else if(start&&end&&start.key===end.key){kind='STAY';place=start.label;knownPlaceKey=start.key;}
    else if(accurate&&meters>=100){kind='MOVE';place=(start?.label||'未登録地点')+' → '+(end?.label||'未登録地点');stayAnchor=undefined;stayAnchorAt=0;}
    else if((accurate&&locationDistance(a,b)<=50)||windowStable){kind='STAY';place='未登録地点付近';unknownAnchor=stayAnchor||a;}
    const previous=entries.at(-1);
    const bridgeableUncertain=Boolean(previous&&previous.kind==='UNCERTAIN'&&entries.length>=2&&entries.at(-2)?.kind==='STAY'&&duration<=STAY_WINDOW_MS);
    ...
    const sameUnknown=Boolean(contiguous&&!knownPlaceKey&&!stayPrevious?.knownPlaceKey&&stayPrevious?.unknownAnchor&&locationDistance(stayPrevious.unknownAnchor,b)<=UNKNOWN_STAY_CLUSTER_METERS);
