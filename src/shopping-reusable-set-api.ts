...
export async function handleShoppingReusableSetAction(ctx:any,m:any,b:Record<string,unknown>):Promise<Response|null>{
 ...
 if(action==='reusable_set_invoke'){
  ...
  try{await ctx.env.DB.batch(rows.map((x,i)=>ctx.env.DB.prepare("INSERT OR IGNORE INTO shopping_items(family_id,name,quantity,category,memo,due_date,status,created_by,created_at,updated_at,task_id,url,client_request_id) VALUES(?,?,?,?,?,?,'pending',?,?,?,?,?,?)").bind(m.family_id,String(x.name||''),String(x.quantity||'1'),String(x.category||'')||null,String(x.memo||'')||null,date,m.id,t,t,null,String(x.url||'')||null,requestKeys[i])))}catch{return bad('セットから買い物を作成できませんでした。',500)}
