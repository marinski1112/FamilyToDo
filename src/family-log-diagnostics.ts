...
export function familyLogDiagnosticCard(ctx:AppContext):string{
  ...
  return `<section class="card" id="familyLogDiagnosticAdmin" data-family="${familyId}"><h2>子供クイックの診断</h2><p class="small">このLINE内ブラウザで診断開始後、クイックを1回押し、ここへ戻って結果を表示してください。本文・写真・位置・認証情報は記録しません。端末の証拠はこのタブだけで10分、サーバーは最新20件・24時間（削除は毎時）。</p><div class="actions"><button type="button" id="familyLogDiagnosticArm">診断を開始</button><button type="button" class="btn gray" id="familyLogDiagnosticRead">結果を表示</button></div><p id="familyLogDiagnosticStatus" role="status"></p><div id="familyLogDiagnosticResult"></div></section><script src="/assets/family-log-diagnostics.js?v=quick-diag1" data-family="${familyId}"></script>
  <section class="card" id="messageChatDiagnosticAdmin"><h2>伝言（LINE風チャット）の実機診断</h2><p class="small">iPhone/LIFFでしか判定できないスタンプ再生と画面外タップを1回だけ採取します。D1/APIへ診断データは送信しません。本文・スタンプID/URL・画像・token・位置情報は保存せず、段階名と経過時間だけを同一サイト内へ最大10分保存します。</p><div class="actions"><button type="button" id="messageChatDiagnosticStampArm">スタンプ診断を開始</button><button type="button" id="messageChatDiagnosticDismissArm">画面外タップ診断を開始</button><button type="button" class="btn gray" id="messageChatDiagnosticRead">結果を表示</button><button type="button" class="btn gray" id="messageChatDiagnosticClear">診断を破棄</button></div><p id="messageChatDiagnosticStatus" role="status"></p><div id="messageChatDiagnosticResult"></div></section><script src="/assets/messages-chat-diagnostics.js?v=chat-diag1" data-family="${familyId}" data-admin="1"></script>`;
}
