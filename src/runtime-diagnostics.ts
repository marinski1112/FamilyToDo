...
export async function dbSchemaHealth(env:Env):Promise<Response>{
  const required:Record<string,string[]>= {
    ...
    shopping_items:['id','family_id','status','visibility_scope','private_owner_id'],
...
export async function dbRuntimeHealth(env:Env):Promise<Response>{
  const checks:[string,string][]=[
    ...
    ['items','SELECT id,family_id,name,status,completion_mode,due_at,group_key,visibility_scope,private_owner_id FROM items LIMIT 1'],
    ['item_completions','SELECT item_id,member_id,completed_at FROM item_completions LIMIT 1'],
    ['shopping_items','SELECT id,family_id,name,quantity,category,memo,due_date,status,created_by,created_at,updated_at,url,visibility_scope,private_owner_id FROM shopping_items LIMIT 1'],
