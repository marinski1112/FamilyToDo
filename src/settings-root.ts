...
export async function settings(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const body=`<div class="card"><h1>⚙️ 管理</h1><h2>プロフィール</h2><form id="profile"><input type="hidden" name="csrf" value="${esc(ctx.session.csrfToken||'')}"><input name="name" value="${esc(m.name)}" required><button>保存</button></form></div>${isAdmin?`<div class="card"><h2>家族設定</h2><form id="familyTimezone"><label>タイムゾーン</label><select name="timezone">${FAMILY_TIMEZONE_OPTIONS.map(z=>`<option value="${z}" ${String(family?.timezone||DEFAULT_FAMILY_TIMEZONE)===z?'selected':''}>${z==='Asia/Tokyo'?'日本 / ':''}${z}</option>`).join('')}</select><p class="small">タイムゾーンを変更しても、既存の記録日時は自動変換されません。</p><button>保存する</button></form></div>`:''}${isAdmin?`<div class="card"><h2>💬 伝言リアクション</h2><p class="small">家族で使える絵文字を1〜20個設定します。カンマ・読点・改行で区切ってください。</p><form id="messageReactionSettings"><textarea id="messageReactionEmojis" rows="3" maxlength="659" aria-label="伝言リアクション一覧"></textarea><button>保存する</button><p id="messageReactionStatus" role="status" class="small"></p></form></div>`:''}${isAdmin?`<div id="mediaCleanupRecovery" class="card" hidden><h2>Family Log 写真の削除再試行</h2><p class="small">削除に繰り返し失敗した写真を再試行できます。</p><div id="mediaCleanupDead"></div></div>`:''}<div class="card"><h2>🎨 外観</h2><label for="appearanceTheme">表示モード</label><select id="appearanceTheme" aria-describedby="appearanceThemeHint"><option value="LIGHT">ライトモード</option><option value="DARK">ダークモード</option></select><p class="small" id="appearanceThemeHint">この端末の表示に適用します。いつでも切り替えられます。</p><p class="small" id="appearanceThemeStatus" role="status" aria-live="polite"></p></div><div class="card settings-links"><div class="section-link"><div><h2>🔗 外部連携</h2><p class="small">Google Home・Google Calendar・Family AIを分けて管理します。</p></div><a class="btn gray" href="/app/settings_integrations.php">開く</a></div><div class="section-link"><div><h2>👨‍👩‍👧 家族メンバー</h2><p class="small">家族メンバーと招待を管理します。</p></div><a class="btn" href="/app/settings_members.php">開く</a></div><div class="section-link"><div><h2>🐣 家族ログ管理</h2><p class="small">記録対象・表示項目・インポートを管理します。</p></div><a class="btn gray" href="/app/settings_family_log.php">開く</a></div><div class="section-link"><div><h2>📋 投稿管理</h2><p class="small">タスク・持ち物・買い物・伝言を確認します。</p></div><a class="btn gray" href="/app/settings_content.php">開く</a></div><div class="section-link"><div><h2>📅 カレンダーインポート</h2><p class="small">ICS / TimeTreeの予定を安全に確認して取り込みます。</p></div><a class="btn gray" href="/app/calendar_import.php">開く</a></div><div class="section-link"><div><h2>🔔 通知設定</h2><p class="small">LINE / Web Pushの通知方法と対象メンバーを設定します。</p></div><a class="btn gray" href="/app/settings_notifications.php">開く</a></div>${isAdmin?`<div class="section-link"><div><h2>🩺 データ診断</h2><p class="small">通知・定期タスク・削除履歴・紐付けの整合性を確認します。</p></div><a class="btn gray" href="/app/settings_diagnostics.php">開く</a></div>`:''}<div class="section-link"><div><h2>🔁 定期タスク</h2><p class="small">毎日・毎週・毎月などの繰り返しを設定します。</p></div><a class="btn gray" href="/app/recurring.php">開く</a></div>${isAdmin?`<div class="section-link"><div><h2>📊 家族の活動ログ</h2><p class="small">タスク完了や家族ログの記録・編集を、誰がいつ行ったか確認します。</p></div><a class="btn gray" href="/app/logs.php">開く</a></div>`:''}</div><script type="application/json" id="settingsPayload">${JSON.stringify({csrf:ctx.session.csrfToken||''}).replaceAll('<','\\u003c').replaceAll('>','\\u003e').replaceAll('&','\\u0026')}</script><script src="/assets/settings.js?v=${APP_VERSION}"></script>${isAdmin?`<script src="/assets/settings-media-cleanup.js?v=${APP_VERSION}-1"></script>`:''}${isAdmin?`<script src="/assets/settings-message-reactions.js?v=${APP_VERSION}-1"></script>`:''}`;
