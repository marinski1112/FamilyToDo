...
export async function createExternalShoppingItemDomain(env:Env,member:CurrentMember,input:{name:string;quantity:number;visibility?:'FAMILY'|'PRIVATE'}):Promise<{ok:boolean;id?:number}>{
  ...
  const r=await env.DB.prepare("INSERT INTO shopping_items(family_id,name,quantity,category,due_date,status,created_by,created_at,updated_at,url,visibility_scope,private_owner_id) VALUES(?,?,?,NULL,NULL,'pending',?,?,?,NULL,?,?)").bind(member.family_id,name,String(input.quantity),member.id,n,n,visibility,visibility==='PRIVATE'?member.id:null).run();
...
export async function recordExternalBabyQuickDomain(env:Env,member:CurrentMember,subjectId:number,kind:'MILK'|'BATH',milkAmount?:number):Promise<{ok:boolean;id?:number}>{
  ...
  const subject=await env.DB.prepare("SELECT id,subject_kind,enabled_types_json FROM family_log_subjects WHERE id=? AND family_id=? AND active=1 AND subject_kind='BABY' LIMIT 1").bind(subjectId,member.family_id).first<Row>();
  if(!subject||!familyLogEnabledTypes(subject).includes(kind))return {ok:false};
  if(kind==='MILK'){
    if(!Number.isInteger(milkAmount)||Number(milkAmount)<1||Number(milkAmount)>2000)return {ok:false};
    const setting=await env.DB.prepare("SELECT setting_value FROM family_settings WHERE family_id=? AND setting_key='family_log_milk_amount_presets'").bind(member.family_id).first<Row>();
    ...
    if(!presets.includes(milkAmount!))return {ok:false};
  }
  const now=nowJst(),amount=kind==='MILK'?milkAmount:null,detail=kind==='BATH'?'BATH':null,unit=kind==='MILK'?'ml':null;
  const inserted=await env.DB.prepare('INSERT INTO family_logs(family_id,subject_id,log_type,occurred_at,detail_code,amount,unit,duration_minutes,value_text,note,linked_task_id,linked_occurrence_id,created_by,created_at,updated_at,deleted_at) VALUES(?,?,?,?,?,?,?,NULL,NULL,NULL,NULL,NULL,?,?,?,NULL)').bind(member.family_id,subjectId,kind,now,detail,amount,unit,member.id,now,now).run();
...
export async function startDedicatedSleepDomain(env:Env,member:CurrentMember,subjectId:number):Promise<{ok:boolean;id?:number;already?:boolean}>{
  const child=await env.DB.prepare("SELECT id,subject_kind,enabled_types_json FROM family_log_subjects WHERE id=? AND family_id=? AND active=1 AND subject_kind IN ('BABY','CHILD')").bind(subjectId,member.family_id).first<Row>();if(!child||!supportsDedicatedSleep(child.subject_kind))return {ok:false};
...
export async function stopDedicatedSleepDomain(env:Env,member:CurrentMember,subjectId:number,timerId?:number,wakeAt=nowJst()):Promise<{ok:boolean;log_id?:number;duration_minutes?:number;already?:boolean}>{
  const child=await env.DB.prepare("SELECT id,subject_kind,enabled_types_json FROM family_log_subjects WHERE id=? AND family_id=? AND active=1 AND subject_kind IN ('BABY','CHILD')").bind(subjectId,member.family_id).first<Row>();if(!child)return {ok:false};
