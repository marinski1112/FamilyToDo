...
export async function reserveMorningDigestAiRequest(db:D1Database,familyId:number,localDate:string,ignoreCircuit=false):Promise<boolean>{
  const now=utcNow();
  await db.batch([
  ...
  if(!ignoreCircuit&&String(global?.blocked_until||'')>now)return false;
  ...
  const familyReservation=await db.prepare('UPDATE line_daily_digest_ai_family_daily SET request_count=request_count+1,updated_at=? WHERE family_id=? AND local_date=? AND finalized=0 AND request_count<?').bind(now,familyId,localDate,MAX_MORNING_AI_REQUESTS_PER_FAMILY_DAY).run();
  return changed(familyReservation);
}
