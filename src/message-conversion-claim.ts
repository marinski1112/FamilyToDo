...
export async function acquireMessageConversionClaim(db:D1Database,familyId:number,messageId:number,type:MessageConversionType,mode:MessageConversionMode,initialTargetId:number|null=null):Promise<MessageConversionClaim>{
  const token=crypto.randomUUID();
  const inserted=await db.prepare(`INSERT OR IGNORE INTO message_conversion_claims(message_id,family_id,conversion_type,conversion_mode,target_id,status,lease_token,lease_expires_at,created_at,updated_at)
    VALUES(?,?,?,?,?,'PROCESSING',?,datetime('now','+5 minutes'),datetime('now'),datetime('now'))`).bind(messageId,familyId,type,mode,initialTargetId,token).run();
  if(changed(inserted))return{state:'acquired',token,targetId:initialTargetId,mode};

  const existing=await db.prepare('SELECT conversion_mode,target_id,status,lease_expires_at FROM message_conversion_claims WHERE message_id=? AND family_id=? AND conversion_type=? LIMIT 1').bind(messageId,familyId,type).first<Row>();
  ...
  if(storedMode!==mode)return{state:'mismatch',targetId,mode:storedMode};
  ...
  const reclaimed=await db.prepare(`UPDATE message_conversion_claims SET lease_token=?,lease_expires_at=datetime('now','+5 minutes'),updated_at=datetime('now')
    WHERE message_id=? AND family_id=? AND conversion_type=? AND status='PROCESSING' AND COALESCE(lease_expires_at,'')<datetime('now')
    RETURNING target_id,conversion_mode`).bind(token,messageId,familyId,type).first<Row>();
