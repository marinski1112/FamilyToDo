...
const validYear=(v:string)=>/^\d{4}$/.test(v)&&Number(v)>=2000&&Number(v)<=9998;
...
export async function familyDailyJournalPage(request:Request,ctx:AppContext):Promise<Response>{
  ...
  const q=String(url.searchParams.get('q')||'').trim().slice(0,80),pattern=`%${q.replaceAll('\\','\\\\').replaceAll('%','\\%').replaceAll('_','\\_')}%`,requestedSearchYear=String(url.searchParams.get('search_year')||today.slice(0,4)),searchYear=validYear(requestedSearchYear)?requestedSearchYear:today.slice(0,4),searchFrom=`${searchYear}-01-01`,searchTo=`${String(Number(searchYear)+1).padStart(4,'0')}-01-01`;
  const found=q?await ctx.env.DB.prepare(`SELECT journal_date,summary_text FROM family_daily_journals WHERE family_id=? AND journal_date>=? AND journal_date<? AND storage_tier='HOT' AND summary_text LIKE ? ESCAPE '\\' ORDER BY journal_date DESC LIMIT ?`).bind(familyId,searchFrom,searchTo,pattern,MAX_SEARCH).all<Row>():{results:[] as Row[]};
  ...
  const search=q?`<section class="card"><h2>${esc(searchYear)}年「${esc(q)}」の振り返り</h2>${found.results.length?found.results.map(r=>`<a class="journal-result" href="/app/family_journal.php?month=${esc(String(r.journal_date).slice(0,7))}&date=${esc(r.journal_date)}"><strong>${esc(r.journal_date)}</strong><span>${esc(r.summary_text)}</span></a>`).join(''):'<p class="small">一致する総括はありません。</p>'}</section>`:'';
