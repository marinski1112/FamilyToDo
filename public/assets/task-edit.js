...
try{
  ...
  const renderChildSection=()=>{
    ...
    const addHtml=childState.canAddChildren&&!editIsEvent?.checked?`<details id="childTaskAddDetails"><summary class="section-button">＋ 子タスクを追加</summary><div class="compact-form"><label>タイトル<input id="childTaskTitle" maxlength="255" placeholder="子タスク"></label><div class="date-option-row"><label>期限<input id="childTaskDate" type="date"></label><label>時刻<input id="childTaskTime" type="time"></label></div><label>完了条件<select id="childTaskCompletion"><option value="ANY">誰か1人で完了</option><option value="ALL">担当者全員が完了</option></select></label>${assigneeHtml}<button type="button" class="btn" id="childTaskCreate">子タスクを作成</button><p class="small">子タスクは独立して完了できます。親タスクの完了では自動完了しません。</p></div></details>`:editIsEvent?.checked?'':'<p class="small">このタスク自体が子タスクのため、さらに子タスクは追加できません。</p>';
