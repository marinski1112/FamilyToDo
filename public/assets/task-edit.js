...
try{
  ...
  const loadChildren=async()=>{
    if(!parentId)return;
    try{const r=await fetch(`/api/task-children?parent_id=${encodeURIComponent(parentId)}`,{headers:{accept:'application/json'},cache:'no-store'}),d=await r.json().catch(()=>null);if(!r.ok||!d?.ok)throw new Error(d?.error||'子タスクを読み込めませんでした。');childState={loaded:true,parentVisibility:String(d.parent?.visibilityScope)==='PRIVATE'?'PRIVATE':'FAMILY',canAddChildren:Boolean(d.canAddChildren),children:Array.isArray(d.children)?d.children:[]};renderChildSection();}catch(error){childState.loaded=true;renderChildSection();const card=document.getElementById('taskChildTaskCard');if(card)card.innerHTML=`<p class="small">${esc(error?.message||'子タスクを読み込めませんでした。')}</p>`;}
  };
  async function createChildTask(){
    ...
    try{const r=await fetch('/api/task',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)}),d=await r.json().catch(()=>null);if(!r.ok||!d?.ok)throw new Error(d?.error||'子タスクを作成できませんでした。');await loadChildren();const box=document.getElementById('childTaskBox');if(box)box.style.display='block';}catch(error){alert(String(error?.message||'子タスクを作成できませんでした。'));button.disabled=false;button.textContent=old;}
  }
