  ...
  for(const [klass,label] of labels){
    ...
    if(klass==='task-section'){
      ...
      if(!(button instanceof HTMLButtonElement)){
        button=document.createElement('button');button.type='button';button.className='checklist-compact-action task-add-top';button.textContent=section.dataset.kindTab==='event'?'＋ イベント':'＋ タスク';
