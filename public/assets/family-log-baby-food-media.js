...
const journalPhotos=payload.journalPhotos===true;
const journalLogIds=new Set(Array.isArray(payload.journalLogIds)?payload.journalLogIds.map(Number):[]);
const photoLabel=journalPhotos?'成長記録':'離乳食';
...
const isBabyFood=()=>journalPhotos?journalLogIds.has(logId()):String(field('log_type')?.value||'').toUpperCase()==='MEAL'&&String(field('detail_code')?.value||'').toUpperCase()==='BABY_FOOD';
...
const viewer=document.createElement('div');viewer.className='family-log-media-viewer';viewer.setAttribute('aria-hidden','true');viewer.innerHTML='<button type="button" aria-label="写真を閉じる">×</button><img alt="離乳食の写真">';viewer.innerHTML=viewer.innerHTML.replaceAll('離乳食',photoLabel);document.body.appendChild(viewer);
...
const wrap=document.createElement('section');wrap.id='familyLogBabyFoodMedia';wrap.className='family-log-baby-food-media';wrap.hidden=true;wrap.innerHTML='<strong>📷 離乳食の写真（任意）</strong><div class="family-log-media-preview" id="familyLogMediaPreview"></div><div class="family-log-media-picker" id="familyLogMediaPicker"><label>写真から選ぶ<input id="familyLogMediaInput" type="file" accept="image/*"></label><label>カメラで撮る<input id="familyLogMediaCamera" type="file" accept="image/*" capture="environment"></label></div><small class="family-log-media-note">1記録に1枚。写真を選んだ後、下の「保存する」で送信します。最大辺800pxに縮小し、撮影日時を取得できた場合は日時だけ保持します。</small><div class="family-log-media-status small" id="familyLogMediaStatus" aria-live="polite"></div>';
...
async function deleteExisting(){
  if(!existingMedia||!csrf)return;if(!confirm('この写真を削除しますか？'))return;status.textContent='写真を削除しています…';
...
document.addEventListener('submit',async e=>{
  ...
  if(journalPhotos&&!preparing&&!saving&&!pendingBlob){e.preventDefault();e.stopImmediatePropagation();status.textContent=existingMedia?'写真は保存済みです。':'写真を選んでから保存してください。';return;}
  ...
  try{
    let id=logId();
    if(!journalPhotos&&(!id||id!==savedId||signature!==savedSignature)){
...
async function enhanceTimeline(fresh=false){
  ...
  for(const row of logs){if(String(row?.log_type||'').toUpperCase()!=='MEAL'||String(row?.detail_code||'').toUpperCase()!=='BABY_FOOD')continue;const id=Number(row?.id||0),el=document.querySelector(`.family-log-row[data-id="${id}"] .family-log-main`);if(!id||!el)continue;el.querySelector('.family-log-media-row-photo')?.remove();const media=await fetchMedia(id,{fresh});if(!media)continue;const box=document.createElement('div');box.className='family-log-media-row-photo';const button=document.createElement('button');button.type='button';button.className='family-log-media-thumb';button.setAttribute('aria-label',photoLabel+'の写真を拡大');const img=document.createElement('img');img.loading='lazy';img.src=String(media.url||'');img.alt=photoLabel+'の写真';button.append(img);button.addEventListener('click',ev=>{ev.stopPropagation();openViewer(String(media.url||''));});box.append(button);el.append(box);}
}
