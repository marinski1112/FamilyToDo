...
'use strict';
  ...
  document.querySelectorAll('.convert-shopping').forEach(btn=>btn.onclick=()=>{shoppingForm.reset();shoppingForm.message_id.value=btn.dataset.id||'';shoppingForm.name.value=(btn.dataset.text||'').slice(0,255);shoppingForm.quantity.value='1';shoppingModal.classList.add('open');shoppingModal.setAttribute('aria-hidden','false');setTimeout(()=>shoppingForm.name.focus(),60)});
  shoppingForm.onsubmit=async e=>{e.preventDefault();const body={action:'convert_shopping',id:Number(shoppingForm.message_id.value),name:shoppingForm.name.value.trim(),quantity:shoppingForm.quantity.value.trim()||'1',category:shoppingForm.category.value.trim(),due_date:shoppingForm.due_date.value,memo:shoppingForm.memo.value.trim(),url:shoppingForm.url.value.trim(),csrf};if(!body.name){alert('商品名を入力してください。');return;}shoppingSubmit.disabled=true;shoppingStatus.textContent='保存しています…';try{const r=await fetch('/api/messages',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const d=await r.json().catch(()=>null);if(!r.ok||!d?.ok)throw new Error('買い物に追加できませんでした。');location.reload();}catch(_err){shoppingStatus.textContent='';alert('買い物に追加できませんでした。')}finally{shoppingSubmit.disabled=false}};
  ...
  document.querySelectorAll('.convert-task').forEach(btn=>btn.onclick=()=>{taskForm.reset();taskForm.message_id.value=btn.dataset.id||'';taskMode.value='existing';const text=btn.dataset.text||'';taskForm.title.value=text.slice(0,255);taskForm.description.value=text;taskForm.date.value=String(payload.today||'');taskForm.end_date.value=String(payload.today||'');taskForm.all_day.checked=true;taskForm.calendar_visible.checked=true;syncTaskMode();taskModal.classList.add('open');taskModal.setAttribute('aria-hidden','false')});
  taskForm.onsubmit=async e=>{e.preventDefault();if(taskSubmit.disabled||taskForm.dataset.saving==='1')return;const mode=taskMode.value;const body={action:'convert_task',id:Number(taskForm.message_id.value),mode,csrf};if(taskForm.dataset.messageUpdatedAt!==undefined)body.message_updated_at=taskForm.dataset.messageUpdatedAt;if(taskForm.dataset.messageOriginalText!==undefined)body.message_original_text=taskForm.dataset.messageOriginalText;if(mode==='existing'){body.task_id=Number(taskForm.task_id.value||0);body.append_message=taskForm.append_message.checked;}else{body.title=taskForm.title.value.trim();body.description=taskForm.description.value.trim();body.date=taskForm.date.value;body.end_date=taskForm.end_date.value;body.no_date=taskForm.no_date.checked;body.all_day=taskForm.all_day.checked;body.start_time=taskForm.start_time.value;body.end_time=taskForm.end_time.value;body.location=taskForm.location.value.trim();body.is_event=Boolean(taskForm.is_event?.checked);body.reminder_at=taskForm.task_reminder_at.value;body.calendar_color=taskForm.calendar_color.value;body.calendar_visible=taskForm.calendar_visible.checked;}taskForm.dataset.saving='1';taskSubmit.disabled=true;taskStatus.textContent='保存しています…';try{const r=await fetch('/api/messages',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const d=await r.json().catch(()=>null);if(r.status===409){taskStatus.textContent='伝言が変更されました。もう一度AIで整理してください。';return;}if(!r.ok||!d?.ok)throw new Error('タスクに追加できませんでした。');location.href='/task/view.php?id='+d.id;}catch(_err){taskStatus.textContent='';alert('タスクに追加できませんでした。')}finally{taskForm.dataset.saving='0';taskSubmit.disabled=false}};
