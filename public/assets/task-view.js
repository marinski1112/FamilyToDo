(()=>{
  ...
  const loadDirectChildren=async()=>{
    ...
    try{
      ...
      card.innerHTML=`<div class="section-head"><h2>✅ 子タスク <span class="small">(${d.children.length})</span></h2></div>${d.children.map(child=>`<div class="row"><div><strong class="${child.status==='completed'?'done':''}">${esc(child.title)}</strong><div class="meta">${[child.dueDate?`期限 ${child.dueDate}${child.dueTime?' '+child.dueTime:''}`:'期限なし','家族の誰か1人で完了'].map(esc).join(' ・ ')}</div></div><div><a class="btn gray small" href="/task/view.php?id=${Number(child.id)}">詳細</a>${child.canEdit?` <a class="btn gray small" href="/task/edit.php?id=${Number(child.id)}">編集</a>`:''}</div></div>`).join('')}`;
