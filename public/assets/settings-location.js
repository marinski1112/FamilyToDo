...
const overlandEndpoint=`${location.origin}/api/location/overland`;
...
function renderHome(data){
  ...
  if(!data?.configured){homeStatus.textContent='未設定です。自宅にいる家族の共有中の最新位置から設定してください。';if(deleteHome)deleteHome.hidden=true;return;}
  const pieces=['設定済み'];
  ...
  homeStatus.textContent=pieces.join(' ・ ');if(deleteHome)deleteHome.hidden=false;
}

async function loadHome(){
  if(!isAdmin||!homeStatus)return;
  homeStatus.textContent='自宅地点を確認しています…';
  try{renderHome(await homeApi());}catch(error){homeStatus.textContent=error instanceof Error?error.message:'自宅地点を確認できませんでした。';}
}

function renderDevices(devices){
  if(!Array.isArray(devices)||devices.length===0){list.innerHTML='<div class="location-empty">位置連携端末はまだ登録されていません。</div>';return;}
  ...
  list.querySelectorAll('.device-revoke').forEach(button=>button.addEventListener('click',async()=>{
    if(!confirm('この端末を失効しますか？ 同じ接続情報では再接続できなくなります。受信済みの位置履歴は消えません。'))return;
    button.disabled=true;try{await api({action:'revoke',device_id:Number(button.dataset.id)});await loadDevices();}catch(error){alert(error instanceof Error?error.message:'端末を失効できませんでした。');button.disabled=false;}
  }));
}

async function loadDevices(){
  list.innerHTML='<div class="location-empty">端末を確認しています…</div>';
  try{const response=await fetch('/api/location/devices',{headers:{accept:'application/json'},cache:'no-store'});const data=await response.json();if(!response.ok||!data.ok)throw new Error(data.error||'端末一覧を取得できませんでした。');renderDevices(data.devices);}catch(error){list.innerHTML=`<div class="location-empty">${esc(error instanceof Error?error.message:'端末一覧を取得できませんでした。')}</div>`;}
}

captureHome?.addEventListener('click',async()=>{
  const sourceMemberId=Number(homeSourceMember?.value||0);if(!sourceMemberId)return;if(!confirm('このメンバーの最新共有位置を家族の「自宅」として保存しますか？'))return;
  captureHome.disabled=true;try{renderHome(await homeApi({action:'capture',sourceMemberId}));}catch(error){alert(error instanceof Error?error.message:'自宅地点を設定できませんでした。');}finally{captureHome.disabled=false;}
});
