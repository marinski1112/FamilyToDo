...
try{
  ...
  const renderOptions=options=>{
    ...
    for(const option of options){
      ...
      if(option.thumbnailUrl){const image=document.createElement('img');image.src=option.thumbnailUrl;image.alt=option.name||'スタンプ';image.draggable=false;button.appendChild(image);}else{const fallback=document.createElement('span');fallback.className='calendar-stamp-option-fallback';fallback.textContent='🖼️';fallback.setAttribute('aria-hidden','true');button.appendChild(fallback);}pickerGrid.appendChild(button);
    }
  };
