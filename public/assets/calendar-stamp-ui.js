...
try{
  ...
  const viewerImage=viewer.querySelector('.calendar-stamp-viewer-image'),viewerDelete=viewer.querySelector('.calendar-stamp-viewer-delete'),viewerStatus=viewer.querySelector('.calendar-stamp-viewer-status');
  const reducedMotion=()=>window.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches===true;
  let viewerTimer=0,viewerRun=0,currentViewerStamp=null,deleting=false;
  const closeViewer=()=>{if(deleting)return;viewerRun+=1;clearTimeout(viewerTimer);viewerTimer=0;currentViewerStamp=null;viewer.classList.remove('open');if(viewerImage)viewerImage.removeAttribute('src');if(viewerStatus)viewerStatus.textContent='';};
  ...
  const openViewer=stamp=>{
    ...
    currentViewerStamp=stamp;if(viewerStatus)viewerStatus.textContent='';if(viewerDelete instanceof HTMLButtonElement)viewerDelete.hidden=!csrf;
  ...
  viewerDelete?.addEventListener('click',async()=>{
    ...
    deleting=true;if(viewerDelete instanceof HTMLButtonElement)viewerDelete.disabled=true;if(viewerStatus)viewerStatus.textContent='削除しています…';
    try{
      ...
      deleting=false;if(viewerDelete instanceof HTMLButtonElement)viewerDelete.disabled=false;closeViewer();await renderStamps();
    }catch{if(viewerStatus)viewerStatus.textContent='通信に失敗しました。時間をおいて再試行してください。';}
    finally{deleting=false;if(viewerDelete instanceof HTMLButtonElement)viewerDelete.disabled=false;}
  });
