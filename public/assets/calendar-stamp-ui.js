...
try{
  ...
  const closeViewer=()=>{viewer.classList.remove('open');if(viewerImage)viewerImage.removeAttribute('src');};
  const openViewer=stamp=>{if(!viewerImage||!stamp?.fullUrl)return;const url=new URL(String(stamp.fullUrl),location.origin);if(url.origin!==location.origin)return;url.searchParams.set('stamp_play',String(Date.now()));viewerImage.src=url.pathname+url.search;viewer.classList.add('open');viewer.querySelector('.calendar-stamp-viewer-close')?.focus();};
  ...
  async function renderStamps(){
    ...
    try{
      ...
      const firstByDate=new Map();for(const stamp of data.stamps){const date=String(stamp?.date||'');if(safeDate(date)&&!firstByDate.has(date))firstByDate.set(date,stamp);}
      for(const cell of cells){
        ...
        const thumbnailUrl=safeAssetPath(stamp.thumbnailUrl),fullUrl=safeAssetPath(stamp.fullUrl);if(!thumbnailUrl||!fullUrl)continue;
        const image=document.createElement('img');image.className='calendar-stamp-thumb';image.src=thumbnailUrl;image.alt='スタンプ';image.dataset.fullUrl=fullUrl;image.dataset.kind=String(stamp.kind||'STATIC');image.draggable=false;cell.appendChild(image);cell.classList.add('has-calendar-stamp');
      }
    }catch{/* stamp rendering fails closed without breaking Calendar */}
  }
  const stampFromTarget=target=>{const image=target?.closest?.('.calendar-stamp-thumb');if(!image)return null;const fullUrl=safeAssetPath(image.dataset.fullUrl);return fullUrl?{fullUrl}:null;};
