...
try{
  ...
  function resetForm(){f.reset();setVal('action','create');setVal('id','');setVal('start_date',today);setVal('interval_value',1);setVal('business_day_ordinal',1);setVal('effective_date',today);if(editScope)editScope.value='all';if(editScopeWrap)editScopeWrap.style.display='none';refreshEditScope();f.querySelectorAll('[name=week_numbers]').forEach(x=>x.checked=false);heading.textContent='定期タスクを作成';submit.textContent='定期タスクを作成';cancel.style.display='none';status.textContent='';f.querySelectorAll('[name=weekdays]').forEach(x=>x.checked=false);syncCustomColorFromSelect();refreshRecurringFields();}
  ...
  f.addEventListener('submit',async e=>{
    ...
    b.family_log_enabled=Boolean(q('family_log_enabled')?.checked);b.all_day=Boolean(q('all_day')?.checked);b.calendar_visible=true;b.calendar_color=q('calendar_color')?.value||'#7c3aed';
