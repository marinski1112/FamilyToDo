...
const mediaTargets=async externalIds=>{const r=await fetch('/api/family-log-import-media-targets',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({csrf:config.csrf,subject_id:Number(subject.value),external_ids:externalIds})});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'写真の取り込み先を確認できませんでした。');return d};
...
function renderPreview(d){
  ...
  const counts=node('div',undefined,'import-counts');[['全件',d.record_count,''],['新規',d.new_count,'is-new'],['重複',d.duplicate_count,'is-duplicate'],['エラー',d.error_count,'is-error']].forEach(([label,n,c])=>counts.appendChild(node('b',`${label} ${n}`,c)));
...
async function uploadPhotos(button,explicitRetry=false){
  ...
  for(const item of list){
    ...
    try{const result=await uploadPhoto(selected,target);result==='uploaded'?uploaded++:existing++;}
      ...
      if(error instanceof TypeError){uncertain++;}else{failed++;}
    }
  }
...
async function runImport(preview,button,progress){
  const media=manifest(),chosen=selectedFiles(),matched=media.filter(item=>chosen.has(item.file_name)).length;
  ...
  try{
    ...
    latest=await call({action:'finish',batch_id:lastBatch});label.textContent=`記録完了 ${latest.record_count} / ${latest.record_count}`;status.textContent=`取込 ${latest.imported_count} / 重複 ${latest.skipped_count} / エラー ${latest.error_count}`;button.textContent='記録完了';
    if(media.length){label.textContent='離乳食写真を確認しています…';await uploadPhotos(retry,false);label.textContent='完了';}
  }catch(e){label.textContent=`${bar.value||0} / ${preview.record_count} まで完了`;status.textContent=`${e.message}。「再試行」で続きから再開できます。`;button.textContent='再試行';button.disabled=false;}
}

document.getElementById('importPreview').onclick=async()=>{try{const selected=file.files[0];if(!subject.value||!selected)throw new Error('対象とJSONファイルを選択してください。');if(selected.size>config.maxBytes)throw new Error('JSONは3MB以内にしてください。');documentValue=JSON.parse(await selected.text());manifest();status.textContent='検証しています…';const d=await call({action:'preview',subject_id:Number(subject.value),document:documentValue});renderPreview(d);status.textContent='プレビューを確認してください。DBはまだ変更されていません。';}catch(e){status.textContent=e.message;out.replaceChildren();documentValue=null;lastPreview=null;}};
