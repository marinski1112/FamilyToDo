...
try{
  ...
  const mainAdvanced=(item,destination)=>{
    const isEvent=destination==='event',isPrivate=checked('is_private'),calendarVisible=checked('calendar_visible'),completion=String(form.elements.completion_mode?.value||'ANY'),start=item.dueDate||dateValue('dateOnly',''),end=dateValue('endDateOnly',start),allDay=checked('allDay'),startTime=item.dueTime||dateValue('startTime',''),endTime=dateValue('endTime',''),location=dateValue('location',''),reminder=dateValue('reminderAt',''),color=String(form.elements.calendar_color?.value||'');
