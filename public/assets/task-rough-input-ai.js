...
try{
  ...
  const assigneeFieldset=className=>`<fieldset class="${className}"><legend>担当者</legend>${assigneeOptions().map(a=>`<label class="checkrow inline-check"><input type="checkbox" value="${esc(a.value)}" ${a.checked?'checked':''}> ${esc(a.label)}</label>`).join('')}</fieldset>`;
  ...
  const rowBody=(item,index,dests)=>{
    ...
    if(destination==='item')return `${destinationSelect}${title}${item.dueDate?`<div class="rough-draft-summary small">期限 ${esc(item.dueDate)}</div>`:''}<details class="rough-row-details"><summary>詳細設定</summary>${commonDateDetails(item)}${assigneeFieldset('rough-item-assignees')}</details>`;
    if(destination==='child_task')return `${destinationSelect}${title}${item.dueDate?`<div class="rough-draft-summary small">期限 ${esc(item.dueDate)}${item.dueTime?` ${esc(item.dueTime)}`:''}</div>`:''}<details class="rough-row-details"><summary>詳細設定</summary>${commonDateDetails(item)}<label>完了条件<select class="rough-child-completion"><option value="ANY">誰か1人で完了</option><option value="ALL">担当者全員が完了</option></select></label>${assigneeFieldset('rough-child-assignees')}<p class="small">親が自分専用の場合、担当者は自動的に自分だけになります。</p></details>`;
