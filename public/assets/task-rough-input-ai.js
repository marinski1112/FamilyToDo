...
@media(max-width:340px){.rough-draft-heading{grid-template-columns:70px minmax(0,1fr) 44px}.rough-draft-basic-grid,.rough-detail-grid{grid-template-columns:1fr}}`;document.head.appendChild(style);
  ...
  const rowBody=(item,index,dests)=>{
    ...
    if(destination==='item')return `${heading}${item.dueDate?`<div class="rough-draft-summary small">期限 ${esc(item.dueDate)}</div>`:''}<details class="rough-row-details"><summary>詳細設定</summary>${commonDateDetails(item)}</details>`;
    if(destination==='child_task')return `${heading}${item.dueDate?`<div class="rough-draft-summary small">期限 ${esc(item.dueDate)}${item.dueTime?` ${esc(item.dueTime)}`:''}</div>`:''}<details class="rough-row-details"><summary>詳細設定</summary>${commonDateDetails(item)}</details>`;
  ...
  const bindRow=(row,item,index,dests)=>{
    ...
    const syncSummary=()=>{syncShoppingSummary();const summary=row.querySelector('.rough-draft-summary');if(summary&&['task','event'].includes(item.destination)){const date=row.querySelector('.rough-main-start-date')?.value;summary.textContent=[row.querySelector('.rough-main-private')?.checked?'🔒 自分専用':'家族共有',item.destination==='event'?(row.querySelector('.rough-main-calendar-visible')?.checked?'📅 表示':'📅 非表示'):'☑️ チェックリスト',date?'開始 '+date:'日付なし'].join(' ・ ');}};
