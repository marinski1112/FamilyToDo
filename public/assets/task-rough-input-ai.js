...
try{
  ...
  const rowBody=(item,index,dests)=>{
    ...
    if(destination==='item')return `${destinationSelect}${title}${item.dueDate?`<div class="rough-draft-summary small">期限 ${esc(item.dueDate)}</div>`:''}<details class="rough-row-details"><summary>詳細設定</summary>${commonDateDetails(item)}${assigneeFieldset('rough-item-assignees',item.assignees)}</details>`;
    if(destination==='child_task'){const completion=String(item.completion||'ANY');return `${destinationSelect}${title}${item.dueDate?`<div class="rough-draft-summary small">期限 ${esc(item.dueDate)}${item.dueTime?` ${esc(item.dueTime)}`:''}</div>`:''}<details class="rough-row-details"><summary>詳細設定</summary>${commonDateDetails(item)}<label>完了条件<select class="rough-child-completion"><option value="ANY" ${completion==='ANY'?'selected':''}>誰か1人で完了</option><option value="ALL" ${completion==='ALL'?'selected':''}>担当者全員が完了</option></select></label>${assigneeFieldset('rough-child-assignees',item.assignees)}<p class="small">親が自分専用の場合、担当者は自動的に自分だけになります。</p></details>`;}
