(()=>{
  ...
  const quickTypes={MILK:['ミルク','ml'],MEAL:['食事',''],DIAPER:['おむつ',''],BATH:['お風呂',''],CONDITION:['体調',''],TEMPERATURE:['体温','℃'],MEDICINE:['薬',''],WEIGHT:['体重','kg'],HEIGHT:['身長','cm'],MEMO:['その他','']};
  const quickDetails={DIAPER:[['','指定なし'],['WET','おしっこ'],['DIRTY','うんち']],MEAL:[['','指定なし'],['BABY_FOOD','離乳食']],BATH:[['','指定なし'],['BATH','入浴']],CONDITION:[['','指定なし'],['VOMIT','吐いた']]};
  function renderQuickActions(subjectId){const wrap=byId('familyLogQuickRows');if(!wrap)return;const rows=quickActions.filter(q=>Number(q.subject_id)===subjectId&&Number(q.active)!==0);wrap.innerHTML=rows.map((q,i)=>`<div class="family-log-quick-manage-row"><span>${escapeHtml(q.icon||'＋')}</span><strong>${escapeHtml(q.name)}</strong><small>${q.mode==='QUICK'?'ワンタッチ':q.mode==='FORM'?'入力して記録':'睡眠開始/終了'}</small><span class="quick-order"><button type="button" class="btn gray small" data-quick-move="${q.id}" data-direction="up" ${i===0?'disabled':''}>↑</button><button type="button" class="btn gray small" data-quick-move="${q.id}" data-direction="down" ${i===rows.length-1?'disabled':''}>↓</button></span><button type="button" class="btn gray small" data-quick-edit="${q.id}">編集</button></div>`).join('')||'<p class="small">まだありません。</p>';wrap.querySelectorAll('[data-quick-edit]').forEach(button=>button.addEventListener('click',()=>editQuick(quickActions.find(q=>Number(q.id)===Number(button.dataset.quickEdit)))));wrap.querySelectorAll('[data-quick-move]').forEach(button=>button.addEventListener('click',async()=>{await post({action:'quick_action_reorder',id:Number(button.dataset.quickMove),direction:button.dataset.direction});location.reload();}));}
  function editQuick(row){const subjectId=Number(row?.subject_id||subjectField('id')?.value||0);if(!subjectId)return;let modal=byId('familyLogQuickEditor');if(!modal){modal=document.createElement('div');modal.id='familyLogQuickEditor';modal.className='family-log-backdrop';modal.innerHTML=`<div class="family-log-sheet small-sheet"><div class="section-head"><h2>クイック記録を編集</h2><button type="button" class="btn gray small" data-close>×</button></div><form class="compact-form"><input name="id" type="hidden"><input name="subject_id" type="hidden"><div class="field-pair"><div><label>表示名</label><input name="name" maxlength="80" required></div><div><label>アイコン</label><input name="icon" maxlength="8"></div></div><fieldset><legend>動作</legend><label><input type="radio" name="mode" value="QUICK"> ワンタッチ</label><label><input type="radio" name="mode" value="FORM"> 入力して記録</label><label><input type="radio" name="mode" value="SLEEP_TOGGLE"> 睡眠開始 / 終了</label></fieldset><div data-type-wrap><div class="field-pair"><div><label>種類</label><select name="log_type">${Object.entries(quickTypes).map(([code,v])=>`<option value="${code}">${v[0]}</option>`).join('')}</select></div><div><label>詳細</label><select name="detail_code"></select></div></div><div class="field-pair"><div data-amount-wrap><label>量 / 値</label><input name="amount" type="number" step="any" inputmode="decimal"></div><div><label>単位</label><input name="unit" readonly></div></div></div><button type="submit">保存する</button></form></div>`;document.body.append(modal);modal.querySelector('[data-close]').onclick=()=>modal.classList.remove('open');const form=modal.querySelector('form'),refresh=()=>{const sleep=form.elements.mode.value==='SLEEP_TOGGLE',type=form.elements.log_type.value;modal.querySelector('[data-type-wrap]').hidden=sleep;form.elements.unit.value=quickTypes[type]?.[1]||'';form.elements.detail_code.innerHTML=(quickDetails[type]||[['','指定なし']]).map(([v,l])=>`<option value="${v}">${l}</option>`).join('');modal.querySelector('[data-amount-wrap]').hidden=!['MILK','TEMPERATURE','WEIGHT','HEIGHT'].includes(type);};form.addEventListener('change',refresh);form.addEventListener('submit',async e=>{e.preventDefault();const data=Object.fromEntries(new FormData(form));await post({action:'quick_action_save',...data,id:Number(data.id||0),subject_id:Number(data.subject_id),log_type:data.mode==='SLEEP_TOGGLE'?'SLEEP':data.log_type,active:true});location.reload();});modal._refresh=refresh;}const form=modal.querySelector('form');form.elements.id.value=String(row?.id||'');form.elements.subject_id.value=String(subjectId);form.elements.name.value=String(row?.name||'');form.elements.icon.value=String(row?.icon||'＋');form.querySelector(`[name="mode"][value="${row?.mode||'QUICK'}"]`).checked=true;form.elements.log_type.value=String(row?.log_type==='SLEEP'?'MEMO':row?.log_type||'MEMO');modal._refresh();form.elements.detail_code.value=String(row?.detail_code||'');form.elements.amount.value=String(row?.amount??'');modal.classList.add('open');}
  ...
  document.querySelectorAll('.family-log-form-action').forEach(btn=>btn.addEventListener('click',()=>{const diagnostic=window.familyLogDiagnostic?.begin();diagnostic?.mark('UI_UPDATE_START');openNew(String(btn.dataset.logType||'MEMO'),Number(btn.dataset.subjectId||0));formField('detail_code').value=String(btn.dataset.detail||'');formField('amount').value=String(btn.dataset.amount||'');formField('unit').value=String(btn.dataset.unit||'');formField('value_text').value=String(btn.dataset.valueText||'');refreshDynamicFields();diagnostic?.mark('EDITOR_READY');}));
