...
const bindInlineEditor=title=>{
  ...
  title.dataset.flowInlineBound='1';title.classList.add('checklist-inline-title');title.title='タップして編集';
  const begin=()=>{
...
const addTaskRow=(id,title)=>{
  ...
  clearEmpty(section);const row=document.createElement('div');row.className='row task-row checklist-new-row';
...
const addItemRow=(id,name)=>{
  ...
  clearEmpty(section);const row=document.createElement('div');row.className='row checklist-new-row';
