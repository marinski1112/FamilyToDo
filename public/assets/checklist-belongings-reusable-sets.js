...
const boot=()=>{
  ...
  const build=toolbar=>{
    ...
    const open=document.createElement('button');open.id='belongingsReusableSetButton';open.className='belongings-reusable-set-button';open.type='button';open.textContent='セット';open.setAttribute('aria-label','現在の持ち物からセット保存・呼び出し');toolbar.prepend(open);
