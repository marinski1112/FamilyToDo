...
const installCategoryUi=async(section,kind,categories,canManage)=>{
 ...
 trash.addEventListener('click',async()=>{if(!section.classList.contains('category-delete-mode-active')){setMode(true);trash.textContent='完了';trash.classList.add('text-button');return;}const names=[...selected];if(!names.length){setMode(false);trash.textContent='🗑️';trash.classList.remove('text-button');return;}const policy=await deleteChoice(names,kind);if(!policy)return;try{await requestJson('/api/shopping-category-mutation',{csrf,action:'delete_many',kind,names,item_policy:policy});location.reload();}catch(err){alert(err?.message||String(err));}});
};
...
const setup=async()=>{
 ...
 const task=document.querySelector('.task-section');if(task instanceof HTMLElement){installStatusTabs(task,'task');installSearch(task);const add=[...document.querySelectorAll('a,button')].find(n=>/^＋?\s*タスクを追加$/.test(String(n.textContent||'').trim()));if(add instanceof HTMLElement)task.append(add);}
