...
const installUnifiedChecklist=()=>{
 ...
 if(task instanceof HTMLElement){
  ...
  const add=tools?.querySelector('.task-add-top');if(add instanceof HTMLAnchorElement){add.dataset.baseHref=add.href;add.dataset.inputKind='task';}
 ...
 if(shopping instanceof HTMLElement&&items instanceof HTMLElement){
  ...
  const itemCategoryAdd=[...items.querySelectorAll('.belongings-add-category,.checklist-compact-action')].find(b=>/カテゴリ/.test(String(b.textContent||'')));if(itemCategoryAdd instanceof HTMLElement){itemCategoryAdd.dataset.inputKind='item';tools?.append(itemCategoryAdd);}  const shoppingCategoryAdd=[...shopping.querySelectorAll('.shopping-category-add,.checklist-compact-action')].find(b=>/カテゴリ/.test(String(b.textContent||''))&&b!==itemCategoryAdd);if(shoppingCategoryAdd instanceof HTMLElement)shoppingCategoryAdd.dataset.inputKind='shopping';  const catButtons=[shoppingCategoryAdd,itemCategoryAdd].filter(Boolean);
  let active='shopping';let apply=kind=>{active=kind;shopping.dataset.inputKind=kind;tabs.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.kind===kind));catButtons.forEach(b=>{if(!(b instanceof HTMLElement))return;const belongs=b.dataset.inputKind==='item';b.hidden=kind==='item'?!belongs:belongs;});if(shoppingUn instanceof HTMLElement)shoppingUn.hidden=kind!=='shopping';if(itemUn instanceof HTMLElement)itemUn.hidden=kind!=='item';if(ai instanceof HTMLAnchorElement){const u=new URL(ai.href,location.href);u.searchParams.set('type',kind);ai.href=u.pathname+u.search;}};
