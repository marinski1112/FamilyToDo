...
const decorateParents=()=>{
 document.querySelectorAll('.task-row[data-task-id],.unorganized-task-row[data-task-id]').forEach(row=>{
  ...
  row.classList.add('task-parent-row');children.hidden=true;const composer=children.querySelector('.task-child-composer');if(composer instanceof HTMLElement){composer.classList.add('task-child-composer-deferred');const reveal=document.createElement('button');reveal.type='button';reveal.className='task-child-add-reveal';reveal.textContent='＋ 子タスクを追加';composer.before(reveal);reveal.addEventListener('click',()=>{reveal.hidden=true;composer.classList.add('is-open');composer.querySelector('input,textarea')?.focus();});}
