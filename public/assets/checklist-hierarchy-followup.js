...
const installCategoryUi=async(section,kind,categories,canManage)=>{
 ...
 for(const g of groups){const name=categoryName(g),head=g.querySelector(kind==='shopping'?':scope>.shopping-category-title':':scope>.belongings-category-head');const rows=g.querySelectorAll(kind==='shopping'?':scope>.linked-shopping-row':':scope>.belongings-category-body>.belongings-category-row');const count=rows.length;if(count)active.add(key(name));
  if(head instanceof HTMLElement){const nameNode=head.querySelector(kind==='shopping'?'.shopping-category-name':'.belongings-category-name');if(nameNode instanceof HTMLElement&&name!==U){nameNode.classList.add('category-name-editable');nameNode.title='カテゴリ名を編集';nameNode.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();void renameCategory(kind,name,nameNode);});}
 ...
 cluster=document.createElement('details');cluster.className='zero-category-cluster';cluster.innerHTML=`<summary><span>空のカテゴリ</span><span>${zero.length}</span></summary><div class="zero-category-cluster-list"></div>`;const list=cluster.querySelector('.zero-category-cluster-list');zero.forEach(name=>{const row=document.createElement('div');row.className='zero-category-cluster-row';const title=document.createElement('button');title.type='button';title.className='zero-category-name';title.textContent=name;title.addEventListener('click',()=>void renameCategory(kind,name,title));row.append(title);list.append(row);});const add=document.createElement('button');add.type='button';add.className='zero-unclassified-add';add.textContent=kind==='shopping'?'＋ 未分類に買い物を追加':'＋ 未分類に持ち物を追加';add.addEventListener('click',()=>{const live=[...section.querySelectorAll(kind==='shopping'?':scope>.shopping-category-group':':scope>.belongings-category-group')].find(g=>g instanceof HTMLElement&&categoryName(g)===U);if(live instanceof HTMLElement){live.hidden=false;live.classList.remove('category-collapsed');if(kind==='shopping'){const trigger=live.querySelector('.shopping-category-add-item');if(trigger instanceof HTMLButtonElement)trigger.click();}else{const input=live.querySelector('.belongings-composer-name');if(input instanceof HTMLElement)input.focus({preventScroll:false});}live.scrollIntoView({behavior:'smooth',block:'center'});}});list.append(add);section.append(cluster);
