...
const decorateParents=()=>{
 document.querySelectorAll('.task-row[data-task-id],.unorganized-task-row[data-task-id]').forEach(row=>{
  ...
  row.classList.toggle('task-parent-row',childRows.length>0);children.hidden=true;
...
const installCategoryUi=async(section,kind,categories,canManage)=>{
 ...
 const selected=new Set(),trash=document.createElement('button');trash.type='button';trash.className='checklist-icon-button category-delete-mode';trash.innerHTML='<span class="checklist-trash-icon" aria-hidden="true"></span>';trash.setAttribute('aria-label','カテゴリ削除');trash.title='カテゴリ削除';if(canManage)tools.insertBefore(trash,tools.firstChild);
 ...
 trash.addEventListener('click',async()=>{if(!section.classList.contains('category-delete-mode-active')){setMode(true);trash.textContent='完了';trash.classList.add('text-button');return;}const names=[...selected];if(!names.length){setMode(false);trash.innerHTML='<span class="checklist-trash-icon" aria-hidden="true"></span>';trash.classList.remove('text-button');return;}const policy=await deleteChoice(names,kind);if(!policy)return;try{await requestJson('/api/shopping-category-mutation',{csrf,action:'delete_many',kind,names,item_policy:policy});location.reload();}catch(err){alert(err?.message||String(err));}});
};
