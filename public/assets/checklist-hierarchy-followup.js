...
const installCategoryUi=async(section,kind,categories,canManage)=>{
 ...
 if(zero.length){cluster=document.createElement('details');cluster.className='zero-category-cluster';cluster.innerHTML=`<summary><span>空のカテゴリ</span><span>${zero.length}</span></summary><div class="zero-category-cluster-list"></div>`;const list=cluster.querySelector('.zero-category-cluster-list');zero.forEach(name=>{const row=document.createElement('div');row.className='zero-category-cluster-row';const title=document.createElement('button');title.type='button';title.className='zero-category-name';title.textContent=name;row.append(title);list.append(row);});section.append(cluster);}const add=document.createElement('button');add.type='button';add.className='zero-unclassified-add';add.textContent=kind==='shopping'?'＋ 未分類に買い物を追加':'＋ 未分類に持ち物を追加';add.addEventListener('click',()=>{const live=[...section.querySelectorAll(kind==='shopping'?':scope>.shopping-category-group':':scope>.belongings-category-group')].find(g=>g instanceof HTMLElement&&categoryName(g)===U);if(live instanceof HTMLElement){live.hidden=false;live.classList.remove('category-collapsed','checklist-status-group-empty','checklist-search-hidden');if(kind==='shopping'){const trigger=live.querySelector('.shopping-category-add-item');if(trigger instanceof HTMLButtonElement)trigger.click();}else{const input=live.querySelector('.belongings-composer-name');if(input instanceof HTMLElement)input.focus({preventScroll:false});}live.scrollIntoView({behavior:'smooth',block:'center'});}});section.append(add);
