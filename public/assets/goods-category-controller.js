...
const boot=async()=>{
 ...
 const tabs=document.createElement('div');tabs.className='checklist-kind-tabs goods-kind-tabs';tabs.setAttribute('role','tablist');tabs.setAttribute('aria-label','追加する種類');
 ...
 const refresh=()=>{
  ...
  for(const kind of KINDS)for(const g of groups(kind)){
   ...
   const visibleCount=content.filter(row=>Boolean(row.querySelector('input.toggle')?.checked)===completedView).length;
   const composing=Boolean(g.querySelector('.shopping-continuous-composer:not([hidden]),.belongings-composer:not([hidden])'));
   const emptyUnclassified=name(g)===U&&visibleCount===0&&!openedEmpty.has(g)&&!composing;
   g.hidden=state==='DISABLED'||emptyUnclassified||(!completedView&&name(g)!==U&&state==='ARCHIVED_EMPTY'&&!openedEmpty.has(g)&&!composing);
   const collapsed=g.classList.contains('category-collapsed'),h=g.querySelector(':scope>.shopping-category-title');
   const count=h?.querySelector('.checklist-category-count-toggle'),toggle=h?.querySelector('.shopping-category-toggle');setText(count,String(visibleCount));setText(toggle,collapsed?'›':'⌄');
   ...
   for(const row of content){const done=Boolean(row.querySelector('input.toggle')?.checked);if(done)completed++;row.classList.toggle('checklist-status-hidden',host.dataset.statusTab==='completed'?!done:done);}
   const minus=h?.querySelector('.category-delete-minus');if(deleteMode&&name(g)!==U){if(!minus){const b=document.createElement('button');b.type='button';b.className='category-delete-minus';b.dataset.goodsAction='delete';b.textContent='−';b.setAttribute('aria-label',name(g)+'を削除');h.prepend(b);}}else minus?.remove();
   if(!completedView&&name(g)!==U&&state==='ARCHIVED_EMPTY'&&!openedEmpty.has(g)&&!composing)archived.push(g);
  }
  ...
  const signature=JSON.stringify([deleteMode,archived.map(g=>[g.dataset.goodsKind,name(g)])]);
  if(zero.dataset.signature!==signature){zero.dataset.signature=signature;const list=zero.querySelector('.zero-category-cluster-list');list.replaceChildren();for(const g of archived){const row=document.createElement('div');row.className='zero-category-cluster-row';row.dataset.goodsKind=g.dataset.goodsKind;row.dataset.category=name(g);row.dataset.categoryState='ARCHIVED_EMPTY';const icon=document.createElement('span');icon.textContent=g.dataset.goodsKind==='shopping'?'🛒':'🎒';const title=document.createElement('span');title.tabIndex=0;title.setAttribute('role','button');title.className='zero-category-name';title.dataset.goodsAction='rename';title.textContent=name(g);const plus=document.createElement('button');plus.type='button';plus.className='zero-category-add-item';plus.dataset.goodsAction='compose';plus.textContent='＋ 追加';row.append(icon,title,plus);if(deleteMode){const minus=document.createElement('button');minus.type='button';minus.className='category-delete-minus';minus.dataset.goodsAction='delete';minus.textContent='−';minus.setAttribute('aria-label',name(g)+'を削除');row.prepend(minus)}list.append(row)}}
