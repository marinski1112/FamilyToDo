...
try {
  ...
  form.onsubmit=async e=>{
    ...
    const body={action:'add_batch',csrf:String(payload.csrf||''),products:names.map((name,j)=>({name,quantity:quantities[j]||'1',url:safeUrls[j]||''})),category:String(fd.get('category')||'').trim(),due_date:dueDate,task_id:taskId,assignees,memo:String(fd.get('memo')||'').trim()};
