...
try {
  ...
  const jumpPanel=document.getElementById('calendarJumpPanel'),monthJump=document.getElementById('calendarMonthJump'),dateJump=document.getElementById('calendarDateJump'),monthPicker=document.getElementById('calendarMonthPicker');
  if(monthLabel)monthLabel.textContent=currentMonth.slice(0,4)+'年'+Number(currentMonth.slice(5))+'月';
  const jumpToPickedMonth=()=>{const value=String(monthPicker?.value||'');if(!/^(?:20\\d{2}|2100)-(?:0[1-9]|1[0-2])$/.test(value)||value===currentMonth)return;location.assign('/app/calendar.php?view='+encodeURIComponent(currentView)+'&month='+encodeURIComponent(value));};monthPicker?.addEventListener('change',jumpToPickedMonth);monthPicker?.addEventListener('input',()=>{if(monthPicker.value&&monthPicker.value.length===7)jumpToPickedMonth();});
