...
try {
  ...
  function render(d){location.assign('/app/tasks.php?date='+encodeURIComponent(d));}
  ...
  calendarCard?.addEventListener('click',e=>{if(Date.now()<suppressCalendarClickUntil)return;const cell=e.target.closest('.calendar-cell');if(cell?.dataset.date)location.assign('/app/tasks.php?date='+encodeURIComponent(cell.dataset.date));});
