...
try {
  ...
  const jumpToPickedMonth=()=>{const value=String(monthPicker?.value||'');if(!/^(?:20\d{2}|2100)-(?:0[1-9]|1[0-2])$/.test(value)||value===currentMonth)return;location.assign('/app/calendar.php?view='+encodeURIComponent(currentView)+'&month='+encodeURIComponent(value));};monthPicker?.addEventListener('change',jumpToPickedMonth);
