...
try {
  ...
  let journalDates=new Set(Array.isArray(payload.journalDates)?payload.journalDates:[]);
  ...
  const checklistLink=document.createElement('a');checklistLink.className='calendar-day-checklist-link';checklistLink.textContent='☑ この日のチェックリストを開く';modalBody?.parentElement?.before(checklistLink);
  const checklistStyle=document.createElement('style');checklistStyle.textContent='.calendar-day-checklist-link{display:block;margin:6px 14px;padding:9px 12px;border:1px solid #b8d8ff;border-radius:9px;color:#007aff;background:#fff;text-decoration:none;font-size:14px;font-weight:700}.calendar-day-checklist-link:focus-visible{outline:2px solid #007aff;outline-offset:2px}';document.head.append(checklistStyle);
  ...
  function detailHtml(d){
    ...
    const journal=journalDates.has(d)?'<div class="modal-subhead">家族日誌</div><div class="modal-row"><a class="btn gray small calendar-journal-link" href="/app/family_journal.php?month='+encodeURIComponent(d.slice(0,7))+'&date='+encodeURIComponent(d)+'">📘 この日の家族日誌を見る</a></div>':'';
    return (h?'<div class="modal-holiday">🎌 '+escJs(h)+'</div>':'')+(rows||'<div class="modal-row">この日のタスクはありません。</div>')+(carry?'<div class="modal-subhead">持ち物</div>'+carry:'')+(shops?'<div class="modal-subhead">買い物</div>'+shops:'')+journal;
  }
  ...
  function rememberGoodsStatus(groups,id,status){for(const rows of Object.values(groups))for(const row of rows)if(Number(row.id)===id)row.status=status;}
  async function toggleItem(el){const checked=el.checked,id=Number(el.dataset.id);el.disabled=true;try{const r=await fetch('/api/toggle',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({type:'item',id,completed:checked,csrf:payload.csrf||''})});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'持ち物の更新に失敗しました');rememberGoodsStatus(itemDetail,id,String(d.status|| (checked?'completed':'pending')));}catch(e){el.checked=!checked;alert(e.message);}finally{el.disabled=false;}}
  async function toggleShopping(el){const checked=el.checked,id=Number(el.dataset.id);el.disabled=true;try{const r=await fetch('/api/shopping',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'toggle',id,completed:checked,csrf:payload.csrf||''})});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'買い物の更新に失敗しました');rememberGoodsStatus(shoppingDetail,id,String(d.status|| (checked?'completed':'pending')));}catch(e){el.checked=!checked;alert(e.message);}finally{el.disabled=false;}}
