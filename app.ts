...
export function layout(title: string, body: string, active = ''): string {
  ...
  const extra=active==='/app/calendar.php'?'<link rel="stylesheet" href="/assets/calendar.css?v=12.73-wave54">':''; return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${esc(title)} - Family TODO LINE</title><link rel="stylesheet" href="/assets/family.css?v=12.73-wave54">${extra}</head><body><div class="wrap">${body}</div>${nav}</body></html>`;
}
...
function renderCalendarPage(ctx:AppContext,month:string,start:Date,end:Date,tasks:Row[],shopping:Row[],items:Row[]=[]):string{
  ...
  function render(d){
    ...
    selectedDate=d;modalTitle.textContent=d+' の詳細';modalBody.innerHTML=detailHtml(d);if(modalAdd)modalAdd.href='/task/new.php?date='+d+'&return=calendar';if(modalPrev)modalPrev.setAttribute('aria-label',shiftDate(d,-1)+' の詳細');if(modalNext)modalNext.setAttribute('aria-label',shiftDate(d,1)+' の詳細');openModal();
  }
  // Wave37: pointer-based delegated interaction.  A tap opens every date, while a horizontal swipe changes month.
  const grid=document.querySelector('.calendar-grid');
  let swipeX=0,swipeY=0,swipeActive=false,swipeMoved=false;
  grid?.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'&&e.button!==0)return;swipeX=e.clientX;swipeY=e.clientY;swipeActive=true;swipeMoved=false;});
  grid?.addEventListener('pointermove',e=>{if(!swipeActive)return;const dx=e.clientX-swipeX,dy=e.clientY-swipeY;if(Math.abs(dx)>18||Math.abs(dy)>18)swipeMoved=true;});
  grid?.addEventListener('pointerup',e=>{if(!swipeActive)return;swipeActive=false;const dx=e.clientX-swipeX,dy=e.clientY-swipeY;if(Math.abs(dx)>60&&Math.abs(dx)>Math.abs(dy)){location.href='/app/calendar.php?month='+(dx<0?${JSON.stringify(next)}:${JSON.stringify(prev)});return;}if(!swipeMoved){const cell=e.target.closest('.calendar-cell');if(cell?.dataset.date)render(cell.dataset.date);}});
  grid?.addEventListener('pointercancel',()=>{swipeActive=false;});
  grid?.addEventListener('click',e=>{if(swipeMoved)return;const cell=e.target.closest('.calendar-cell');if(cell?.dataset.date)render(cell.dataset.date);});
  ...
  document.addEventListener('keydown',e=>{if(!modal.classList.contains('open'))return;if(e.key==='Escape')closeModal();if(e.key==='ArrowLeft'&&selectedDate)render(shiftDate(selectedDate,-1));if(e.key==='ArrowRight'&&selectedDate)render(shiftDate(selectedDate,1));});
...
export async function taskEdit(request:Request,ctx:AppContext,id:number):Promise<Response>{
  ...
  const body=`<div class="card form-card"><h1>📝 タスク編集</h1><form id="taskEditForm">
    ...
    <script>
    const f=document.getElementById('taskEditForm'),editDate=document.getElementById('editTaskDate'),editEndDate=document.getElementById('editTaskEndDate'),editNoDate=document.getElementById('editNoDate');const syncEditDate=()=>{editDate.disabled=editNoDate.checked;if(editNoDate.checked){editDate.value='';if(editEndDate)editEndDate.value='';f.querySelectorAll('[name=start_time],[name=end_time]').forEach(x=>x.value='');}};editNoDate.onchange=syncEditDate;syncEditDate();
...
export async function recurring(request: Request, ctx: AppContext): Promise<Response> {
  ...
  function resetForm(){f.reset();setVal('action','create');setVal('id','');setVal('start_date',${JSON.stringify(dateOnly())});setVal('interval_value',1);setVal('business_day_ordinal',1);f.querySelectorAll('[name=week_numbers]').forEach(x=>x.checked=false);heading.textContent='定期タスクを作成';submit.textContent='定期タスクを作成';cancel.style.display='none';status.textContent='';f.querySelectorAll('[name=weekdays]').forEach(x=>x.checked=false);resetChildren();}
  ...
  f.onsubmit=async e=>{e.preventDefault();const type=String(q('recurrence_type')?.value||'DAILY');const weekdays=[...f.querySelectorAll('[name=weekdays]:checked')].map(x=>Number(x.value));const weekNumbers=[...f.querySelectorAll('[name=week_numbers]:checked')].map(x=>Number(x.value));const monthdays=String(q('monthdays')?.value||'').split(',').map(x=>Number(x.trim())).filter(x=>Number.isInteger(x)&&x>=1&&x<=31);if((type==='WEEKLY'||type==='INTERVAL_WEEKS'||type==='MONTHLY_WEEKDAY')&&!weekdays.length){alert('曜日を1つ以上選択してください。');return}if(type==='MONTHLY_WEEKDAY'&&!weekNumbers.length){alert('第1〜第5週を1つ以上選択してください。');return}if(type==='MONTHLY_DAY'&&!monthdays.length){alert('毎月指定日を1つ以上入力してください。');return}const b=Object.fromEntries(new FormData(f));b.csrf=csrf;b.weekdays=weekdays;b.week_numbers=weekNumbers;b.monthdays=monthdays;b.assignees=[...f.querySelectorAll('[name=assignees]:checked')].map(x=>Number(x.value));const shopNames=[...f.querySelectorAll('[name="shopping_name[]"]')],shopQty=[...f.querySelectorAll('[name="shopping_quantity[]"]')],shopUrl=[...f.querySelectorAll('[name="shopping_url[]"]')];b.shopping=shopNames.map((x,i)=>({name:x.value.trim(),quantity:shopQty[i]?.value.trim()||'1',url:shopUrl[i]?.value.trim()||'',category:q('shopping_category')?.value||''})).filter(x=>x.name);b.items=[...f.querySelectorAll('[name="item_name[]"]')].map(x=>x.value.trim()).filter(Boolean);b.shopping_category=q('shopping_category')?.value||'';b.all_day=Boolean(q('all_day')?.checked);b.calendar_visible=Boolean(q('calendar_visible')?.checked);b.calendar_color=q('calendar_color')?.value||'#7c3aed';submit.disabled=true;status.textContent='保存しています…';try{const r=await fetch('/app/recurring.php',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(b)});const d=await r.json().catch(()=>({ok:false,error:'応答を読み取れませんでした'}));if(!r.ok||!d.ok)throw new Error(d.error||'保存に失敗しました');status.textContent='保存しました。';location.reload();}catch(err){status.textContent='';alert(err.message||String(err));}finally{submit.disabled=false}};
  document.querySelectorAll('.rec-edit').forEach(b=>b.onclick=()=>{const d=JSON.parse(b.dataset.rule);setVal('action','update');setVal('id',d.id);setVal('title',d.title);setVal('description',d.description);setVal('recurrence_type',d.recurrence_type);setVal('interval_value',d.interval_value);setVal('start_date',d.start_date);setVal('end_date',d.end_date);setVal('business_day_ordinal',d.business_day_ordinal);f.querySelectorAll('[name=week_numbers]').forEach(x=>x.checked=d.week_numbers.includes(Number(x.value)));setVal('monthdays',d.monthdays.join(','));setVal('start_time',d.start_time);setVal('end_time',d.end_time);setVal('location',d.location);setVal('completion_mode',d.completion_mode);setVal('calendar_color',d.calendar_color);f.querySelectorAll('[name=assignees]').forEach(x=>x.checked=d.assignees.includes(Number(x.value)));q('all_day').checked=d.all_day;q('calendar_visible').checked=d.calendar_visible;f.querySelectorAll('[name=weekdays]').forEach(x=>x.checked=d.weekdays.includes(Number(x.value)));heading.textContent='定期タスクを編集';submit.textContent='変更を保存';cancel.style.display='inline-block';status.textContent='';window.scrollTo({top:0,behavior:'smooth'});});
  cancel.onclick=resetForm;
  document.querySelectorAll('.rec-toggle').forEach(b=>b.onclick=async()=>{const active=b.dataset.active==='1';b.disabled=true;try{const r=await fetch('/app/recurring.php',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'toggle',id:Number(b.dataset.id),active:!active,csrf})});const d=await r.json();if(d.ok)location.reload();else alert(d.error||'更新に失敗しました');}finally{b.disabled=false}});
  document.querySelectorAll('.rec-delete').forEach(b=>b.onclick=async()=>{if(!confirm('この定期タスクを削除しますか？\n過去の発生日記録も削除されます。'))return;b.disabled=true;try{const r=await fetch('/app/recurring.php',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'delete',id:Number(b.dataset.id),csrf})});const d=await r.json();if(d.ok)location.reload();else alert(d.error||'削除に失敗しました');}finally{b.disabled=false}});
  </script>`;
