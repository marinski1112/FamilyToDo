...
-- lives on tasks rather than shopping_items.
...
CREATE INDEX idx_shopping_overdue_parent_fallback
  ON shopping_items(family_id,task_id,(category IS NOT NULL),COALESCE(category,''),name,id)
  WHERE status<>'completed'
    AND task_id IS NOT NULL
