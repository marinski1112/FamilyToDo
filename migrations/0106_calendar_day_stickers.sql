...
CREATE TABLE calendar_sticker_assets (
...
CREATE INDEX idx_calendar_sticker_assets_family ON calendar_sticker_assets(family_id,active,id);
CREATE TABLE calendar_sticker_days (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  family_id INTEGER NOT NULL REFERENCES families(id),
...
CREATE INDEX idx_calendar_sticker_days_range ON calendar_sticker_days(family_id,sticker_date,owner_id);
CREATE TRIGGER calendar_sticker_assets_tenant BEFORE INSERT ON calendar_sticker_assets BEGIN
...
END;
CREATE TRIGGER calendar_sticker_days_tenant BEFORE INSERT ON calendar_sticker_days BEGIN
...
END;
CREATE TRIGGER calendar_sticker_days_tenant_update BEFORE UPDATE OF asset_id,family_id,owner_id ON calendar_sticker_days BEGIN
  SELECT CASE WHEN NOT EXISTS(SELECT 1 FROM calendar_sticker_assets WHERE id=NEW.asset_id AND family_id=NEW.family_id AND active=1)
    OR (NEW.owner_id<>0 AND NOT EXISTS(SELECT 1 FROM members WHERE id=NEW.owner_id AND family_id=NEW.family_id AND active=1))
    THEN RAISE(ABORT,'sticker placement family mismatch') END;
END;
